-- phpMyAdmin SQL Dump
-- version 3.4.10.1deb1
-- http://www.phpmyadmin.net
--
-- Host: localhost
-- Generation Time: Mar 20, 2013 at 07:07 AM
-- Server version: 5.5.29
-- PHP Version: 5.3.10-1ubuntu3.6

SET SQL_MODE="NO_AUTO_VALUE_ON_ZERO";
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8 */;

--
-- Database: `prefeitura`
--

-- --------------------------------------------------------

--
-- Table structure for table `aidfe_solicitacoes`
--

CREATE TABLE IF NOT EXISTS `aidfe_solicitacoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `solicitante` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `aidf_docs`
--

CREATE TABLE IF NOT EXISTS `aidf_docs` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codsolicitacao` int(10) DEFAULT NULL,
  `especie` varchar(50) DEFAULT NULL,
  `serie` varchar(10) DEFAULT NULL,
  `subserie` varchar(10) DEFAULT NULL,
  `nroinicial` int(10) DEFAULT NULL,
  `nrofinal` int(10) DEFAULT NULL,
  `quantidade` int(10) DEFAULT NULL,
  `tipo` varchar(10) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='Tabela de documentos a serem impressos.' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `aidf_especie`
--

CREATE TABLE IF NOT EXISTS `aidf_especie` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `especie` text,
  `serie` varchar(10) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `aidf_solicitacoes`
--

CREATE TABLE IF NOT EXISTS `aidf_solicitacoes` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codemissor` int(10) DEFAULT NULL COMMENT 'tabela cadastro',
  `codgrafica` int(10) DEFAULT NULL COMMENT 'tabela cadastro',
  `observacoes` text,
  `confirmar` char(1) DEFAULT 'n' COMMENT 's para sim, n para nao',
  `data` date DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `autos_infracao`
--

CREATE TABLE IF NOT EXISTS `autos_infracao` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `origem` varchar(255) DEFAULT NULL,
  `assunto` varchar(255) DEFAULT NULL,
  `data_hora` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=87 ;

-- --------------------------------------------------------

--
-- Table structure for table `bancos`
--

CREATE TABLE IF NOT EXISTS `bancos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `banco` varchar(60) DEFAULT NULL,
  `boleto` varchar(60) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=20 ;

--
-- Dumping data for table `bancos`
--

INSERT INTO `bancos` (`codigo`, `banco`, `boleto`) VALUES
(1, 'Banco do Brasil', 'boleto_bb.php'),
(2, 'Bancoob', 'boleto_bancoob.php'),
(3, 'Banespa', 'boleto_banespa.php'),
(4, 'Banestes', 'boleto_banestes.php'),
(5, 'Besc', 'boleto_besc.php'),
(6, 'Bradesco', 'boleto_bradesco.php'),
(7, 'Caixa Econômica Federal', 'boleto_cef.php'),
(8, 'HSBC', 'boleto_hsbc.php'),
(9, 'Itaú', 'boleto_itau.php'),
(10, 'Nossa caixa', 'boleto_nossacaixa.php'),
(11, 'Real', 'boleto_real.php'),
(12, 'Santander', 'boleto_santander_banespa.php'),
(13, 'Sicredi', 'boleto_sicredi.php'),
(14, 'Sudameris', 'boleto_sudameris.php'),
(15, 'Unibanco', 'boleto_unibanco.php'),
(16, 'Caixa Econômica Federal (SINCO)', 'boleto_cef_sinco.php'),
(17, 'Caixa Econômica Federal(SIGCB)', 'boleto_cef_sigcb.php'),
(18, 'Santander Banespa', 'boleto_banespa.php'),
(19, 'Banrisul', 'boleto_banrisul.php');

-- --------------------------------------------------------

--
-- Table structure for table `boleto`
--

CREATE TABLE IF NOT EXISTS `boleto` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `tipo` char(1) DEFAULT 'R' COMMENT 'P para  pagamento e R para recebimento',
  `codbanco` int(10) DEFAULT NULL,
  `agencia` varchar(60) DEFAULT NULL,
  `contacorrente` varchar(60) DEFAULT NULL,
  `convenio` varchar(60) DEFAULT NULL,
  `contrato` varchar(60) DEFAULT NULL,
  `carteira` varchar(60) DEFAULT NULL,
  `codfebraban` int(11) DEFAULT NULL,
  `instrucoes` text,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=2 ;

--
-- Dumping data for table `boleto`
--

INSERT INTO `boleto` (`codigo`, `tipo`, `codbanco`, `agencia`, `contacorrente`, `convenio`, `contrato`, `carteira`, `codfebraban`, `instrucoes`) VALUES
(1, 'R', 1, '', '', '', '', '', 2222, '');

-- --------------------------------------------------------

--
-- Table structure for table `cadastro`
--

CREATE TABLE IF NOT EXISTS `cadastro` (
  `codigo` bigint(11) NOT NULL AUTO_INCREMENT,
  `codtipo` int(11) DEFAULT NULL,
  `codtipodeclaracao` int(11) DEFAULT NULL,
  `nome` varchar(200) DEFAULT NULL,
  `razaosocial` varchar(200) DEFAULT NULL,
  `cnpj` varchar(20) DEFAULT NULL,
  `cpf` varchar(20) DEFAULT NULL,
  `senha` varchar(255) DEFAULT NULL,
  `inscrmunicipal` varchar(20) DEFAULT NULL,
  `logradouro` varchar(200) DEFAULT NULL,
  `numero` varchar(15) DEFAULT NULL,
  `complemento` varchar(50) DEFAULT NULL,
  `bairro` varchar(50) DEFAULT NULL,
  `cep` varchar(20) DEFAULT NULL,
  `municipio` varchar(100) DEFAULT NULL,
  `uf` char(2) DEFAULT NULL,
  `logo` varchar(100) DEFAULT NULL,
  `email` varchar(100) DEFAULT NULL,
  `ultimanota` int(10) DEFAULT '0',
  `notalimite` int(10) DEFAULT '0',
  `estado` char(2) DEFAULT 'NL' COMMENT 'NL nao liberado, A ativo, I inativo',
  `codcontador` int(10) DEFAULT NULL,
  `nfe` char(1) DEFAULT 'N' COMMENT 'Determina se emissor emite nfe',
  `fonecomercial` varchar(15) DEFAULT NULL,
  `fonecelular` varchar(15) DEFAULT NULL,
  `pispasep` varchar(20) DEFAULT NULL,
  `datainicio` date DEFAULT NULL,
  `datafim` date DEFAULT '0000-00-00',
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=2 ;

--
-- Dumping data for table `cadastro`
--

INSERT INTO `cadastro` (`codigo`, `codtipo`, `codtipodeclaracao`, `nome`, `razaosocial`, `cnpj`, `cpf`, `senha`, `inscrmunicipal`, `logradouro`, `numero`, `complemento`, `bairro`, `cep`, `municipio`, `uf`, `logo`, `email`, `ultimanota`, `notalimite`, `estado`, `codcontador`, `nfe`, `fonecomercial`, `fonecelular`, `pispasep`, `datainicio`, `datafim`) VALUES
(1, 1, 1, 'RAFAEL', 'RAFA LTDA', '01.000.000/0000-00', NULL, '827ccb0eea8a706c4c34a16891f84e7b', 'TESTE', 'TESTE', '12', 'TESTE', 'TESTE', '76767-678', 'NORMANDIA', 'RR', NULL, 'rafael@portalpublico.com.br', 2, 0, 'A', NULL, 'S', '(12) 1212-1212', '', '', NULL, '2010-10-11');

-- --------------------------------------------------------

--
-- Table structure for table `cadastro_resp`
--

CREATE TABLE IF NOT EXISTS `cadastro_resp` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codemissor` int(10) DEFAULT NULL COMMENT 'tabela cadastro',
  `codcargo` int(11) DEFAULT NULL,
  `nome` varchar(100) DEFAULT NULL,
  `cpf` varchar(20) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `cadastro_servicos`
--

CREATE TABLE IF NOT EXISTS `cadastro_servicos` (
  `codigo` bigint(11) NOT NULL AUTO_INCREMENT,
  `codservico` bigint(11) DEFAULT NULL,
  `codemissor` bigint(11) DEFAULT NULL COMMENT 'tabela cadastro',
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `cargos`
--

CREATE TABLE IF NOT EXISTS `cargos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `cargo` varchar(60) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=5 ;

--
-- Dumping data for table `cargos`
--

INSERT INTO `cargos` (`codigo`, `cargo`) VALUES
(1, 'Gerente'),
(2, 'Responsável'),
(3, 'Sócio'),
(4, 'Diretor');

-- --------------------------------------------------------

--
-- Table structure for table `cartorios`
--

CREATE TABLE IF NOT EXISTS `cartorios` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `admpublica` char(1) DEFAULT NULL COMMENT 'D direta ou I indireta',
  `nivel` char(1) DEFAULT NULL COMMENT 'M municipal, E estadual, F federal',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `cartorios_des`
--

CREATE TABLE IF NOT EXISTS `cartorios_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcartorio` int(11) DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `total` decimal(10,2) DEFAULT NULL,
  `codverificacao` varchar(9) DEFAULT NULL,
  `iss_emo` decimal(10,2) DEFAULT NULL COMMENT 'emo=emolumento',
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `cartorios_des_notas`
--

CREATE TABLE IF NOT EXISTS `cartorios_des_notas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `coddec_des` int(11) DEFAULT NULL,
  `codservico` int(11) DEFAULT NULL,
  `valornota` decimal(10,2) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  `emolumento` decimal(10,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `cartorios_servicos`
--

CREATE TABLE IF NOT EXISTS `cartorios_servicos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codtipo` int(11) DEFAULT NULL,
  `servicos` varchar(255) DEFAULT NULL,
  `aliquota` float(10,2) DEFAULT NULL,
  `estado` char(1) DEFAULT 'I',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `cartorios_tipo`
--

CREATE TABLE IF NOT EXISTS `cartorios_tipo` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `tipocartorio` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=3 ;

--
-- Dumping data for table `cartorios_tipo`
--

INSERT INTO `cartorios_tipo` (`codigo`, `tipocartorio`) VALUES
(1, 'Processos'),
(2, 'serviços gerais');

-- --------------------------------------------------------

--
-- Table structure for table `certidoes_negativas`
--

CREATE TABLE IF NOT EXISTS `certidoes_negativas` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codemissor` int(10) DEFAULT NULL,
  `codverificacao` varchar(9) DEFAULT NULL,
  `dataemissao` date DEFAULT NULL,
  `datavalidade` date DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `certidoes_pagamento`
--

CREATE TABLE IF NOT EXISTS `certidoes_pagamento` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codemissor` int(10) DEFAULT NULL,
  `codverificacao` varchar(9) DEFAULT NULL,
  `dataemissao` date DEFAULT NULL,
  `datavalidade` date DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `configuracoes`
--

CREATE TABLE IF NOT EXISTS `configuracoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `endereco` varchar(200) DEFAULT NULL,
  `cidade` varchar(200) DEFAULT NULL,
  `estado` varchar(2) DEFAULT NULL,
  `cnpj` varchar(30) DEFAULT NULL,
  `email` varchar(100) DEFAULT NULL,
  `secretaria` varchar(100) DEFAULT NULL,
  `secretario` varchar(100) DEFAULT NULL COMMENT 'Nome do sr secretario',
  `chefetributos` varchar(100) DEFAULT NULL COMMENT 'Nome do sr chefe de tributos',
  `lei` varchar(100) DEFAULT NULL,
  `decreto` varchar(100) DEFAULT NULL,
  `topo` varchar(100) DEFAULT NULL,
  `topo_nfe` varchar(100) DEFAULT NULL,
  `logo` varchar(100) DEFAULT NULL,
  `logo_nfe` varchar(100) DEFAULT NULL,
  `brasao` varchar(60) DEFAULT NULL,
  `brasao_nfe` varchar(60) DEFAULT NULL,
  `codlayout` int(10) DEFAULT NULL,
  `taxacorrecao` decimal(10,2) DEFAULT NULL,
  `taxamulta` decimal(10,2) DEFAULT NULL,
  `taxajuros` decimal(10,2) DEFAULT NULL,
  `data_tributacao` int(2) DEFAULT '10' COMMENT 'dia do mes para cobranca de tributacao (0 = ultimo dia do mes)',
  `declaracoes_atrazadas` enum('s','n') DEFAULT 's' COMMENT '(s para sim, n para nao)se a prefeitura permite declaracoes pelo site atrazadas',
  `gerar_guia_site` enum('t','i') DEFAULT 'i' COMMENT 't para todos, i para individual',
  `ativar_creditos` char(1) DEFAULT 's' COMMENT 's para creditos ativatos, n para creditos desativados',
  `site` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=2 ;

-- --------------------------------------------------------

--
-- Table structure for table `ddp_contas`
--

CREATE TABLE IF NOT EXISTS `ddp_contas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `conta` varchar(20) DEFAULT NULL,
  `descricao` varchar(100) DEFAULT NULL,
  `aliquota` float(10,2) DEFAULT NULL,
  `estado` char(1) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `ddp_des`
--

CREATE TABLE IF NOT EXISTS `ddp_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codddp` int(11) DEFAULT NULL,
  `data` date DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `total` decimal(10,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  `codverificacao` varchar(30) DEFAULT NULL,
  `estado` char(3) DEFAULT 'N',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `ddp_des_contas`
--

CREATE TABLE IF NOT EXISTS `ddp_des_contas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codddp_des` int(11) DEFAULT NULL,
  `contaoficial` varchar(20) DEFAULT NULL,
  `contacontabil` varchar(50) DEFAULT NULL,
  `titulo` varchar(50) DEFAULT NULL,
  `item` int(5) DEFAULT NULL,
  `saldo_mesanterior` decimal(10,2) DEFAULT NULL,
  `debito` decimal(10,2) DEFAULT NULL,
  `credito` decimal(10,2) DEFAULT NULL,
  `saldo_mesatual` decimal(10,2) DEFAULT NULL,
  `receita` decimal(10,2) DEFAULT NULL,
  `aliquota` decimal(10,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `ddp_des_notas`
--

CREATE TABLE IF NOT EXISTS `ddp_des_notas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codddp_des` int(11) DEFAULT NULL,
  `codemissor` int(11) DEFAULT NULL,
  `codservico` int(11) DEFAULT NULL,
  `valornota` decimal(10,2) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `decc_des`
--

CREATE TABLE IF NOT EXISTS `decc_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codempreiteira` int(11) DEFAULT NULL COMMENT 'tabela cadastro',
  `codobra` int(11) DEFAULT NULL,
  `data` date DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `total` decimal(10,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  `codverificacao` varchar(9) DEFAULT NULL,
  `estado` char(1) DEFAULT 'N' COMMENT 'N para normal B para boleto C para cancelada e E para escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `decc_des_notas`
--

CREATE TABLE IF NOT EXISTS `decc_des_notas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `coddecc_des` int(11) DEFAULT NULL,
  `codservico` int(11) DEFAULT NULL,
  `valornota` decimal(10,2) DEFAULT NULL,
  `nronota` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `declaracoes`
--

CREATE TABLE IF NOT EXISTS `declaracoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `declaracao` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='tipos de declaracoes' AUTO_INCREMENT=5 ;

--
-- Dumping data for table `declaracoes`
--

INSERT INTO `declaracoes` (`codigo`, `declaracao`) VALUES
(1, 'DES Consolidada'),
(2, 'DES Simplificada'),
(3, 'Simples Nacional'),
(4, 'MEI');

-- --------------------------------------------------------

--
-- Table structure for table `des`
--

CREATE TABLE IF NOT EXISTS `des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(10) DEFAULT NULL,
  `codcontador` int(11) DEFAULT NULL COMMENT 'contador que fez a declaracao, se for NULL foi a propria empresa',
  `competencia` date DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `data` date DEFAULT NULL,
  `total` float(10,2) DEFAULT NULL,
  `iss_retido` decimal(10,2) DEFAULT NULL COMMENT 'valor de iss retido na fonte',
  `iss` decimal(10,2) DEFAULT NULL,
  `tomador` char(1) DEFAULT 'n',
  `codverificacao` char(9) DEFAULT NULL COMMENT 'Codigo de verificacao para a des',
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `des_issretido`
--

CREATE TABLE IF NOT EXISTS `des_issretido` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL COMMENT 'tomador',
  `total` decimal(10,2) DEFAULT NULL,
  `valor` decimal(10,2) DEFAULT NULL,
  `multa` decimal(10,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `codverificacao` char(9) DEFAULT NULL,
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `des_issretido_notas`
--

CREATE TABLE IF NOT EXISTS `des_issretido_notas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `coddes_issretido` int(11) DEFAULT NULL,
  `valor_nota` decimal(20,2) DEFAULT NULL,
  `codemissor` int(11) DEFAULT NULL COMMENT 'codigo da tabela cadastro',
  `issretido` decimal(10,2) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `des_multas_atraso`
--

CREATE TABLE IF NOT EXISTS `des_multas_atraso` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `dias` int(5) DEFAULT NULL COMMENT 'contagem de dias do pagamento',
  `multa` decimal(5,2) DEFAULT NULL COMMENT 'valor da multa em reais',
  `juros_mora` decimal(5,2) DEFAULT NULL COMMENT 'porcentagem por atrazo',
  `estado` char(1) DEFAULT 'A',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `des_servicos`
--

CREATE TABLE IF NOT EXISTS `des_servicos` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `coddes` int(10) DEFAULT NULL,
  `codservico` bigint(11) DEFAULT NULL,
  `basedecalculo` float(10,2) DEFAULT NULL,
  `iss_retido` decimal(10,2) DEFAULT NULL COMMENT 'valor iss retido na fonte da nota',
  `iss` decimal(10,2) DEFAULT NULL COMMENT 'valor do imposto de iss da nota',
  `tomador_cnpjcpf` varchar(20) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  `codmunicipio` varchar(100) DEFAULT NULL COMMENT 'Local onde foi emitida a nota',
  `cancelada` char(1) DEFAULT 'N' COMMENT 'N não e S para sim',
  `complementar` int(11) DEFAULT NULL,
  `observacao` text,
  `motivo_cancelamento` text,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `des_temp`
--

CREATE TABLE IF NOT EXISTS `des_temp` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codemissores_temp` int(11) DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `base` decimal(10,2) DEFAULT NULL,
  `aliq` decimal(10,2) DEFAULT NULL,
  `codverificacao` varchar(20) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='tabela temporaria para cadastro de emissores nao cadastrados' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `des_tomadores_notas`
--

CREATE TABLE IF NOT EXISTS `des_tomadores_notas` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `cod_tomador` int(10) DEFAULT NULL,
  `nota` varchar(20) DEFAULT NULL,
  `dataemissao` date DEFAULT NULL,
  `cod_emissor` int(11) DEFAULT NULL COMMENT 'codigo do emissor',
  `valor` float(10,2) DEFAULT NULL,
  `credito` float(10,2) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='tabela para gerar creditos das declaracoes' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `dif_contas`
--

CREATE TABLE IF NOT EXISTS `dif_contas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `conta` varchar(20) DEFAULT NULL,
  `descricao` varchar(100) DEFAULT NULL,
  `aliquota` float(10,2) DEFAULT NULL,
  `estado` char(1) DEFAULT 'I',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `dif_des`
--

CREATE TABLE IF NOT EXISTS `dif_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codinst_financeira` int(11) DEFAULT NULL,
  `data` date DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `total` decimal(10,2) DEFAULT NULL,
  `codverificacao` varchar(30) DEFAULT NULL,
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `dif_des_contas`
--

CREATE TABLE IF NOT EXISTS `dif_des_contas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `coddif_des` int(11) DEFAULT NULL,
  `contaoficial` varchar(20) DEFAULT NULL,
  `contacontabil` varchar(50) DEFAULT NULL,
  `titulo` varchar(50) DEFAULT NULL,
  `item` int(5) DEFAULT NULL,
  `saldo_mesanterior` decimal(10,2) DEFAULT NULL,
  `debito` decimal(10,2) DEFAULT NULL,
  `credito` decimal(10,2) DEFAULT NULL,
  `saldo_mesatual` decimal(10,2) DEFAULT NULL,
  `receita` decimal(10,2) DEFAULT NULL,
  `aliquota` decimal(4,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `doc_contas`
--

CREATE TABLE IF NOT EXISTS `doc_contas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `conta` varchar(20) DEFAULT NULL,
  `descricao` varchar(100) DEFAULT NULL,
  `aliquota` float(10,2) DEFAULT NULL,
  `estado` char(1) DEFAULT 'I',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `doc_des`
--

CREATE TABLE IF NOT EXISTS `doc_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codopr_credito` int(11) DEFAULT NULL,
  `data` datetime DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `total` decimal(10,2) DEFAULT NULL,
  `codverificacao` varchar(30) DEFAULT NULL,
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `doc_des_contas`
--

CREATE TABLE IF NOT EXISTS `doc_des_contas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `coddoc_des` int(11) DEFAULT NULL,
  `contaoficial` varchar(20) DEFAULT NULL,
  `contacontabil` varchar(50) DEFAULT NULL,
  `titulo` varchar(50) DEFAULT NULL,
  `item` int(5) DEFAULT NULL,
  `saldo_mesanterior` decimal(10,2) DEFAULT NULL,
  `debito` decimal(10,2) DEFAULT NULL,
  `credito` decimal(10,2) DEFAULT NULL,
  `saldo_mesatual` decimal(10,2) DEFAULT NULL,
  `receita` decimal(10,2) DEFAULT NULL,
  `aliquota` decimal(4,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `dop_des`
--

CREATE TABLE IF NOT EXISTS `dop_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codorgaopublico` int(11) DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `total` float(10,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  `codverificacao` varchar(9) DEFAULT NULL,
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `dop_des_notas`
--

CREATE TABLE IF NOT EXISTS `dop_des_notas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `coddop_des` int(11) DEFAULT NULL,
  `codemissor` int(11) DEFAULT NULL,
  `codservico` int(11) DEFAULT NULL,
  `valornota` decimal(10,2) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `empreiteiras_servicos`
--

CREATE TABLE IF NOT EXISTS `empreiteiras_servicos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `codservico` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `empreiteiras_socios`
--

CREATE TABLE IF NOT EXISTS `empreiteiras_socios` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `nome` varchar(100) DEFAULT NULL,
  `cpf` char(14) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `faq_isencoes`
--

CREATE TABLE IF NOT EXISTS `faq_isencoes` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `tipo` varchar(200) DEFAULT NULL COMMENT 'Se isencao ou imunidade',
  `texto` text COMMENT 'Texto',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='Tabela de faq para isencoes da prefeitura' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `fiscais`
--

CREATE TABLE IF NOT EXISTS `fiscais` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nome` varchar(100) DEFAULT NULL,
  `estado` char(1) DEFAULT 'A' COMMENT 'A para ativado e D para desativado',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `guias_cadastro_pg`
--

CREATE TABLE IF NOT EXISTS `guias_cadastro_pg` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `codguia` int(11) DEFAULT NULL,
  `codverificacao` char(9) COLLATE latin1_general_ci DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `total` float(10,2) DEFAULT NULL,
  `estado` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `guias_declaracoes`
--

CREATE TABLE IF NOT EXISTS `guias_declaracoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codguia` int(11) DEFAULT NULL,
  `codrelacionamento` int(11) DEFAULT NULL,
  `relacionamento` varchar(15) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=3 ;

-- --------------------------------------------------------

--
-- Table structure for table `guia_pagamento`
--

CREATE TABLE IF NOT EXISTS `guia_pagamento` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `dataemissao` date DEFAULT NULL,
  `datavencimento` date DEFAULT NULL,
  `valor` float(11,2) DEFAULT NULL,
  `valormulta` decimal(10,2) DEFAULT NULL,
  `nossonumero` char(25) DEFAULT NULL,
  `chavecontroledoc` bigint(20) DEFAULT NULL,
  `pago` char(1) DEFAULT 'N',
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal e C cancelada',
  `motivo_cancelamento` varchar(70) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=3 ;

-- --------------------------------------------------------

--
-- Table structure for table `inconsistencias`
--

CREATE TABLE IF NOT EXISTS `inconsistencias` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codemissor` int(11) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  `codtomador` int(11) DEFAULT NULL,
  `estado` char(1) DEFAULT NULL,
  `tipo` varchar(30) DEFAULT NULL,
  `datahorainconsistencia` datetime DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `inst_financeiras`
--

CREATE TABLE IF NOT EXISTS `inst_financeiras` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `codbanco` int(11) DEFAULT NULL,
  `agencia` varchar(20) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `legislacao`
--

CREATE TABLE IF NOT EXISTS `legislacao` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `titulo` varchar(200) DEFAULT NULL,
  `texto` text,
  `data` date DEFAULT NULL,
  `arquivo` varchar(255) DEFAULT NULL,
  `estado` char(1) DEFAULT 'A' COMMENT 'A=ativo;I=inativo;',
  `tipo` char(1) DEFAULT 'I' COMMENT 'N=nfe,I=iss;T=todos',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `livro`
--

CREATE TABLE IF NOT EXISTS `livro` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `periodo` varchar(255) DEFAULT '10',
  `vencimento` date DEFAULT NULL,
  `geracao` date DEFAULT NULL,
  `basecalculo` float(10,2) DEFAULT NULL,
  `reducaobc` float(10,2) DEFAULT NULL,
  `valoriss` float(10,2) DEFAULT NULL,
  `valorissretido` float(10,2) DEFAULT NULL,
  `valorisstotal` float(10,2) DEFAULT NULL,
  `obs` varchar(200) DEFAULT NULL COMMENT 'E notas emitidas T notas tomadas',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `livro_notas`
--

CREATE TABLE IF NOT EXISTS `livro_notas` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codlivro` int(11) DEFAULT NULL,
  `codnota` int(11) DEFAULT NULL,
  `tipo` char(1) DEFAULT NULL COMMENT '(E) para notas emitidas e (T) para notas tomadas',
  `nfe` char(1) DEFAULT NULL COMMENT 'S para nfe N para issdigital',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `logs`
--

CREATE TABLE IF NOT EXISTS `logs` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codusuario` int(11) DEFAULT NULL,
  `ip` varchar(100) DEFAULT NULL,
  `data` datetime DEFAULT NULL,
  `acao` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=27 ;

-- --------------------------------------------------------

--
-- Table structure for table `mei_des`
--

CREATE TABLE IF NOT EXISTS `mei_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codemissor` int(11) DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `total` decimal(10,2) DEFAULT NULL,
  `tomador` char(1) DEFAULT 'N',
  `codverificacao` char(9) DEFAULT NULL,
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `mei_des_servicos`
--

CREATE TABLE IF NOT EXISTS `mei_des_servicos` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codmei_des` int(10) DEFAULT NULL,
  `codservico` bigint(11) DEFAULT NULL,
  `basedecalculo` float(10,2) DEFAULT NULL,
  `tomador_cnpjcpf` varchar(20) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `mensagem_tipos`
--

CREATE TABLE IF NOT EXISTS `mensagem_tipos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `tipo` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `mensagens`
--

CREATE TABLE IF NOT EXISTS `mensagens` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `assunto` varchar(255) DEFAULT NULL,
  `mensagem` text,
  `codtipo_mensagem` int(11) DEFAULT NULL,
  `estado` char(1) DEFAULT NULL COMMENT 'E = Em análise, L = Liberado e R = Recusado',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=4 ;

--
-- Dumping data for table `mensagens`
--

INSERT INTO `mensagens` (`codigo`, `assunto`, `mensagem`, `codtipo_mensagem`, `estado`) VALUES
(1, 'Estado de Aidf: Em análise', 'Está é uma notificação da prefeitura, informando-lhe que seu pedido de liberação de AIDF está sob análise da Prefeitura!', 1, 'E'),
(2, 'Estado de Aidf: Liberado', 'Está é uma notificação da prefeitura, para informar-lhe que foi liberado seu pedido de AIDF!', 1, 'L'),
(3, 'Estado de Aidf: Recusado', 'Está é uma notificação da prefeitura, para informar que por algum eventual erro em sua solicitação, seu pedido de liberamento de AIDF foi recusado!', 1, 'R');

-- --------------------------------------------------------

--
-- Table structure for table `menus_cadastro`
--

CREATE TABLE IF NOT EXISTS `menus_cadastro` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `menu` varchar(100) DEFAULT NULL,
  `tipo` varchar(100) DEFAULT NULL,
  `ordem` int(11) DEFAULT NULL,
  `link` varchar(100) DEFAULT NULL,
  `visivel` char(1) DEFAULT 'S',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=70 ;

--
-- Dumping data for table `menus_cadastro`
--

INSERT INTO `menus_cadastro` (`codigo`, `menu`, `tipo`, `ordem`, `link`, `visivel`) VALUES
(1, 'DES Consolidada', 'des', 1, 'comtomador', 'S'),
(2, 'DES Simplificada', 'des', 3, 'semtomador', 'S'),
(3, 'Declaração', 'dif', 1, 'declaracao/principal', 'S'),
(5, 'Guia Pagamento', 'des', 4, 'guia_pagamento', 'S'),
(7, 'Declaração DOP', 'dop', 1, 'declaracao', 'S'),
(8, 'Atualizar Cadastro', 'dop', 2, 'atualizarcadastro', 'S'),
(10, 'Retificação DOP', 'dop', 5, 'retificar', 'S'),
(11, 'Guia de Pagamento', 'dop', 3, 'guia_pagamento', 'N'),
(12, 'Segunda via', 'dop', 4, 'segundavia', 'S'),
(13, 'Atualizar Cadastro', 'dec', 2, 'atualizarcadastro', 'S'),
(14, 'Atualizar Cadastro', 'decc', 1, 'atualizar/atualizarcadastro', 'S'),
(15, 'Atualizar Cadastro', 'dif', 2, 'atualizacao/atualizarcadastro', 'S'),
(16, 'Guia de pagamento', 'dif', 3, 'guia_pagamento/index', 'S'),
(17, 'Declaração DECC', 'decc', 3, 'declaracoes/declarar', 'S'),
(18, 'Obras', 'decc', 2, 'obras/obras', 'S'),
(19, 'Retificação DIF', 'dif', 5, 'cancelamento/cancelar', 'S'),
(20, 'Relatórios', 'dif', 7, 'historico/historico', 'S'),
(21, 'Segunda Via', 'des', 5, 'segundavia_prestador', 'S'),
(22, 'Declaração', 'dec', 1, 'declarar', 'S'),
(23, 'Relatórios', 'dec', 6, 'historico', 'S'),
(24, 'Declaração', 'doc', 1, 'declarar', 'S'),
(25, 'Atualizar Cadastro', 'doc', 2, 'atualizarcadastro', 'S'),
(26, 'Guia de recolhimento', 'doc', 3, 'recolhimento', 'S'),
(27, 'Retificação DOC', 'doc', 5, 'retificacao', 'S'),
(28, 'Segunda Via', 'doc', 4, 'segundavia', 'S'),
(29, 'Relatórios', 'doc', 7, 'historico', 'S'),
(30, 'Retificação', 'dec', 3, 'retificar', 'S'),
(31, 'Retificação DES', 'des', 6, 'prestadores_cancelardes', 'S'),
(32, 'Relatórios', 'des', 9, 'historicodes', 'S'),
(33, 'Declaração', 'simples', 1, 'desn', 'S'),
(34, 'Atualizar Cadastro', 'simples', 2, 'alterar_form', 'S'),
(35, 'Retificação', 'simples', 3, 'desn_retificacao', 'S'),
(36, 'Relatórios', 'simples', 4, 'desn_historico', 'S'),
(37, 'Portal SN', 'simples', 5, 'portal_simples', 'S'),
(38, 'Guia Pagamento', 'decc', 4, 'guias/geraguia', 'S'),
(39, 'Segunda Via', 'decc', 5, 'guias/segundavia', 'S'),
(40, 'Guia de Pagamento', 'dec', 4, 'gerarguia', 'S'),
(41, 'Remessa DES', 'des', 7, 'importar', 'S'),
(42, 'Segunda Via', 'dif', 4, 'guia_pagamento/segundavia', 'S'),
(43, 'Segunda Via', 'dec', 5, 'segundavia', 'S'),
(45, 'Atualizar Cadastro', 'des', 10, 'atualizarcadastro', 'S'),
(46, 'Retificação Guia', 'des', 8, 'prestadores_cancelarguia', 'S'),
(47, 'Clientes (Contador)', 'des', 0, 'clientes', 'S'),
(48, 'Definir Contador', 'des', 11, 'definircontador', 'S'),
(50, 'Relatórios', 'mei', 4, 'mei_historico', 'S'),
(51, 'Retificação', 'mei', 3, 'mei_retificacao', 'S'),
(52, 'Declaração', 'mei', 1, 'declaracao', 'S'),
(53, 'Retificação Guia', 'dif', 6, 'retificacaoguia/dif_cancelarguia', 'S'),
(54, 'Retificação Guia', 'doc', 6, 'doc_cancelarguia', 'S'),
(55, 'Retificação DECC', 'decc', 6, 'retificacao/cancelar', 'S'),
(56, 'Retificação Guia', 'decc', 7, 'retificacao_guia/decc_cancelarguia', 'S'),
(58, 'Retificação Guia', 'dop', 6, 'dop_cancelarguia', 'N'),
(59, 'Definir Contador', 'mei', 10, 'definircontador', 'S'),
(60, 'Definir Contador', 'simples', 10, 'definircontador', 'S'),
(61, 'Definir Contador', 'dif', 8, 'definircontador', 'S'),
(62, 'Declaração Simples', 'des', 0, 'contsimples', 'N'),
(63, 'DES Tomadas', 'des', 2, 'des_tomadas', 'S'),
(64, 'Declaração', 'ddp', 1, 'declarar', 'S'),
(65, 'Atualizar Cadastro', 'ddp', 2, 'atualizar', 'S'),
(66, 'Guia de Pagamento', 'ddp', 3, 'guia_pagamento', 'S'),
(67, 'Retificação de Guia', 'ddp', 4, 'retificacao', 'S'),
(68, 'Relatórios', 'ddp', 5, 'relatorios', 'S'),
(69, 'Segunda Via', 'ddp', 6, 'segunda_via', 'S');

-- --------------------------------------------------------

--
-- Table structure for table `menus_prefeitura`
--

CREATE TABLE IF NOT EXISTS `menus_prefeitura` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `menu` varchar(100) DEFAULT NULL,
  `ordem` int(11) DEFAULT NULL,
  `link` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=31 ;

--
-- Dumping data for table `menus_prefeitura`
--

INSERT INTO `menus_prefeitura` (`codigo`, `menu`, `ordem`, `link`) VALUES
(2, 'Serviços', 2, 'servicos'),
(3, 'Cadastro', 1, 'cadastro'),
(6, 'NFe', 5, 'nfe'),
(15, 'Ajuda', 20, 'ajuda'),
(18, 'Fiscalização', 4, 'fiscalizacao'),
(20, 'Declarações', 3, 'declaracoes'),
(22, 'Utilitários', 8, 'utilitarios'),
(29, 'Relatórios', 6, 'relatorios'),
(30, 'Livro Digital', 7, 'livro');

-- --------------------------------------------------------

--
-- Table structure for table `menus_prefeitura_submenus`
--

CREATE TABLE IF NOT EXISTS `menus_prefeitura_submenus` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codmenu` int(11) DEFAULT NULL,
  `codsubmenu` int(11) DEFAULT NULL,
  `visivel` char(1) DEFAULT NULL COMMENT 'S = sim N = nao',
  `ordem` int(11) DEFAULT NULL,
  `nivel` char(1) DEFAULT 'M' COMMENT 'A para alto ,M para medio e B para baixo',
  `iss` char(1) DEFAULT 'N' COMMENT 'S=sim N=nao',
  `nfe` char(1) DEFAULT 'N' COMMENT 'S=sim N=nao',
  `iptu` char(1) DEFAULT 'N' COMMENT 'S=sim N=nao',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=121 ;

--
-- Dumping data for table `menus_prefeitura_submenus`
--

INSERT INTO `menus_prefeitura_submenus` (`codigo`, `codmenu`, `codsubmenu`, `visivel`, `ordem`, `nivel`, `iss`, `nfe`, `iptu`) VALUES
(3, 2, 1, 'S', 1, 'B', 'S', 'S', 'N'),
(4, 2, 5, 'S', 2, 'B', 'S', 'S', 'N'),
(6, 3, 16, 'S', 2, 'M', 'S', 'N', 'N'),
(7, 3, 6, 'S', 3, 'M', 'S', 'N', 'N'),
(13, 7, 3, 'S', NULL, 'M', 'S', 'N', 'N'),
(14, 8, 26, 'S', NULL, 'M', 'S', 'N', 'N'),
(15, 8, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(16, 6, 11, 'S', 3, 'B', 'N', 'S', 'N'),
(17, 7, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(24, 15, 10, 'S', NULL, 'B', 'S', 'S', 'N'),
(25, 17, 1, 'S', NULL, 'M', 'S', 'N', 'N'),
(26, 17, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(27, 18, 3, 'S', 1, 'M', 'S', 'N', 'N'),
(28, 18, 4, 'S', 2, 'A', 'S', 'N', 'N'),
(39, 17, 19, 'S', NULL, 'M', 'S', 'N', 'N'),
(42, 22, 19, 'S', NULL, 'M', 'S', 'N', 'N'),
(43, 22, 20, 'S', 1, 'A', 'S', 'S', 'N'),
(44, 22, 21, 'S', 5, 'A', 'S', 'S', 'N'),
(45, 22, 22, 'S', 4, 'M', 'S', 'S', 'N'),
(46, 22, 23, 'S', 2, 'A', 'S', 'S', 'N'),
(47, 22, 24, 'S', 3, 'B', 'S', 'S', 'N'),
(51, 22, 29, 'S', 10, 'B', 'S', 'S', 'N'),
(52, 22, 30, 'S', 8, 'B', 'S', 'S', 'N'),
(53, 23, 1, 'S', NULL, 'M', 'S', 'N', 'N'),
(54, 23, 6, 'S', NULL, 'M', 'S', 'N', 'N'),
(55, 23, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(56, 23, 31, 'S', NULL, 'M', 'S', 'N', 'N'),
(57, 23, 32, 'S', NULL, 'M', 'S', 'N', 'N'),
(58, 24, 1, 'S', NULL, 'M', 'S', 'N', 'N'),
(59, 24, 6, 'S', NULL, 'M', 'S', 'N', 'N'),
(60, 24, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(61, 24, 31, 'S', NULL, 'M', 'S', 'N', 'N'),
(62, 24, 32, 'S', NULL, 'M', 'S', 'N', 'N'),
(65, 25, 1, 'S', NULL, 'M', 'S', 'N', 'N'),
(66, 25, 6, 'S', NULL, 'M', 'S', 'N', 'N'),
(67, 25, 31, 'S', NULL, 'M', 'S', 'N', 'N'),
(68, 27, 1, 'S', NULL, 'M', 'S', 'N', 'N'),
(70, 25, 32, 'S', NULL, 'M', 'S', 'N', 'N'),
(71, 25, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(72, 27, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(73, 27, 31, 'S', NULL, 'M', 'S', 'N', 'N'),
(74, 27, 32, 'S', NULL, 'M', 'S', 'N', 'N'),
(75, 27, 6, 'S', NULL, 'M', 'S', 'N', 'N'),
(76, 27, 35, 'S', NULL, 'M', 'S', 'N', 'N'),
(77, 28, 31, 'S', NULL, 'M', 'S', 'N', 'N'),
(78, 28, 2, 'S', NULL, 'M', 'S', 'N', 'N'),
(79, 28, 32, 'S', NULL, 'M', 'S', 'N', 'N'),
(80, 28, 6, 'S', NULL, 'M', 'S', 'N', 'N'),
(82, 28, 1, 'S', NULL, 'M', 'S', 'N', 'N'),
(83, 20, 8, 'S', 8, 'B', 'S', 'N', 'N'),
(84, 20, 13, 'S', 5, 'M', 'S', 'N', 'N'),
(85, 20, 14, 'S', 4, 'M', 'S', 'N', 'N'),
(86, 20, 28, 'S', 7, 'M', 'S', 'N', 'N'),
(87, 20, 27, 'S', 1, 'M', 'S', 'N', 'N'),
(88, 20, 38, 'S', 2, 'M', 'S', 'N', 'N'),
(89, 20, 15, 'S', 6, 'M', 'S', 'N', 'N'),
(90, 20, 39, 'S', 3, 'A', 'S', 'N', 'N'),
(91, 3, 12, 'S', 5, 'M', 'S', 'S', 'N'),
(92, 3, 36, 'S', 6, 'B', 'S', 'S', 'N'),
(93, 6, 17, 'S', 2, 'M', 'N', 'S', 'N'),
(95, 6, 6, 'S', 1, 'M', 'N', 'S', 'N'),
(96, 18, 31, 'S', 3, 'B', 'S', 'N', 'N'),
(97, 18, 37, 'S', 4, 'M', 'S', 'N', 'N'),
(99, 15, 42, 'S', NULL, 'B', 'S', 'S', 'N'),
(102, 22, 26, 'S', 9, 'B', 'S', 'S', 'N'),
(103, 22, 44, 'S', 6, 'A', 'S', 'S', 'N'),
(104, 20, 45, 'S', 9, 'B', 'S', 'N', 'N'),
(105, 3, 46, 'S', 4, 'M', 'S', 'S', 'N'),
(106, 2, 47, 'S', 3, 'M', 'S', 'N', 'N'),
(107, 2, 48, 'S', 4, 'M', 'S', 'N', 'N'),
(108, 2, 49, 'S', 5, 'M', 'S', 'N', 'N'),
(109, 2, 50, 'S', 6, 'A', 'S', 'N', 'N'),
(110, 3, 51, 'S', 3, 'M', 'S', 'N', 'N'),
(111, 29, 52, 'S', 1, 'M', 'S', 'S', 'N'),
(112, 29, 53, 'S', 3, 'M', 'S', 'S', 'N'),
(113, 20, 54, 'S', 3, 'M', 'S', 'N', 'N'),
(114, 22, 55, 'S', 7, 'M', 'S', 'S', 'N'),
(115, 6, 56, 'S', 4, 'M', 'N', 'S', 'N'),
(116, 29, 57, 'S', 4, 'M', 'N', 'S', 'N'),
(117, 6, 58, 'S', 5, 'M', 'N', 'S', 'N'),
(118, 30, 59, 'S', 2, 'M', 'N', 'S', 'N'),
(119, 30, 60, 'S', 1, 'M', 'N', 'S', 'N'),
(120, 30, 61, 'S', 3, 'M', 'N', 'S', 'N');

-- --------------------------------------------------------

--
-- Table structure for table `menus_site`
--

CREATE TABLE IF NOT EXISTS `menus_site` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `menu` varchar(100) DEFAULT NULL,
  `link` varchar(100) DEFAULT NULL,
  `codservico` int(10) DEFAULT NULL,
  `ordem` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=3 ;

--
-- Dumping data for table `menus_site`
--

INSERT INTO `menus_site` (`codigo`, `menu`, `link`, `codservico`, `ordem`) VALUES
(1, 'Declaração Eletrônica de Serviços', 'des.php', 5, 1),
(2, 'NFeletrônica', 'nfe.php', 3, 2);

-- --------------------------------------------------------

--
-- Table structure for table `municipios`
--

CREATE TABLE IF NOT EXISTS `municipios` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nome` varchar(80) DEFAULT NULL,
  `uf` varchar(2) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=5352 ;

--
-- Dumping data for table `municipios`
--

INSERT INTO `municipios` (`codigo`, `nome`, `uf`) VALUES
(1, 'ABAETÉ', 'MG'),
(2, 'ABAETÉTUBA', 'PA'),
(3, 'ABAIARA', 'CE'),
(4, 'ABAIRA', 'BA'),
(5, 'ABARE', 'BA'),
(6, 'ABATIA', 'PR'),
(7, 'ABDON BATISTA', 'SC'),
(8, 'ABEL FIGUEIREDO', 'PA'),
(9, 'ABELARDO LUZ', 'SC'),
(10, 'ABRE CAMPO', 'MG'),
(11, 'ABREU E LIMA', 'PE'),
(12, 'ABREULÂNDIA', 'TO'),
(13, 'ACAIACA', 'MG'),
(14, 'AÇAILÂNDIA', 'MA'),
(15, 'ACAJUTIBA', 'BA'),
(16, 'ACARÁ', 'PA'),
(17, 'ACARAPE', 'CE'),
(18, 'ACARAU', 'CE'),
(19, 'ACARI', 'RN'),
(20, 'ACAUA', 'PI'),
(21, 'ACEGUÁ', 'RS'),
(22, 'ACOPIARA', 'CE'),
(23, 'ACORIZAL', 'MT'),
(24, 'ACREUNA', 'GO'),
(25, 'AÇU', 'RN'),
(26, 'AÇUCENA', 'MG'),
(27, 'ADAMANTINA', 'SP'),
(28, 'ADELÂNDIA', 'GO'),
(29, 'ADOLFO', 'SP'),
(30, 'ADRIANÓPOLIS', 'PR'),
(31, 'ADUSTINA', 'BA'),
(32, 'AFOGADOS DA INGAZEIRA', 'PE'),
(33, 'AFONSO BEZERRA', 'RN'),
(34, 'AFONSO CLAUDIO', 'ES'),
(35, 'AFONSO CUNHA', 'MA'),
(36, 'AFRANIO', 'PE'),
(37, 'AFUA', 'PA'),
(38, 'AGRESTINA', 'PE'),
(39, 'AGRICOLÂNDIA', 'PI'),
(40, 'AGROLÂNDIA', 'SC'),
(41, 'AGRONÔMICA', 'SC'),
(42, 'ÁGUA AZUL DO NORTE', 'PA'),
(43, 'ÁGUA BOA', 'MG'),
(44, 'ÁGUA BOA', 'MT'),
(45, 'ÁGUA BRANCA', 'PB'),
(46, 'ÁGUA CLARA', 'MS'),
(47, 'ÁGUA COMPRIDA', 'MG'),
(48, 'ÁGUA DOCE', 'SC'),
(49, 'ÁGUA DOCE DO MARANHÃO', 'MA'),
(50, 'ÁGUA DOCE DO NORTE', 'ES'),
(51, 'ÁGUA FRIA', 'BA'),
(52, 'ÁGUA FRIA DE GOIÁS', 'GO'),
(53, 'ÁGUA LIMPA', 'GO'),
(54, 'ÁGUA NOVA', 'RN'),
(55, 'ÁGUA PRETA', 'PE'),
(56, 'ÁGUA SANTA', 'RS'),
(57, 'ÁGUAI', 'SP'),
(58, 'AGUANIL', 'MG'),
(59, 'ÁGUAS BELAS', 'PE'),
(60, 'ÁGUAS DA PRATA', 'SP'),
(61, 'ÁGUAS DE CHAPECÓ', 'SC'),
(62, 'ÁGUAS DE LINDÓIA', 'SP'),
(63, 'ÁGUAS DE SANTA BÁRBARA', 'SP'),
(64, 'ÁGUAS DE SÃO PEDRO', 'SP'),
(65, 'ÁGUAS FORMOSAS', 'MG'),
(66, 'ÁGUAS FRIAS', 'SC'),
(67, 'ÁGUAS LINDAS', 'GO'),
(68, 'ÁGUAS MORNAS', 'SC'),
(69, 'ÁGUAS VERMELHAS', 'MG'),
(70, 'AGUDO', 'RS'),
(71, 'AGUDOS', 'SP'),
(72, 'AGUDOS  DO SUL', 'PR'),
(73, 'AGUIA BRANCA', 'ES'),
(74, 'AGUIAR', 'PB'),
(75, 'AGUIARNÓPOLIS', 'TO'),
(76, 'AIMORES', 'MG'),
(77, 'AIQUARA', 'BA'),
(78, 'AIUABA', 'CE'),
(79, 'AIURUOCA', 'MG'),
(80, 'AJURICABA', 'RS'),
(81, 'ALAGOA GRANDE', 'PB'),
(82, 'ALAGOA NOVA', 'PB'),
(83, 'ALAGOINHA', 'PB'),
(84, 'ALAGOINHA', 'PE'),
(85, 'ALAGOINHA DO PIAUÍ', 'PI'),
(86, 'ALAGOINHAS', 'BA'),
(87, 'ALAMBARI', 'SP'),
(88, 'ALBERTINA', 'MG'),
(89, 'ALCANTARA', 'MA'),
(90, 'ALCÂNTARAS', 'CE'),
(91, 'ALCANTIL', 'PB'),
(92, 'ALCINÓPOLIS', 'MS'),
(93, 'ALCOBAÇA', 'BA'),
(94, 'ALDEIAS ALTAS', 'MA'),
(95, 'ALECRIM', 'RS'),
(96, 'ALEGRE', 'ES'),
(97, 'ALEGRETE', 'RS'),
(98, 'ALEGRETE DO PIAUÍ', 'PI'),
(99, 'ALEGRIA', 'RS'),
(100, 'ALÉM PARAÍBA', 'MG'),
(101, 'ALENQUER', 'PA'),
(102, 'ALEXANDRIA', 'RN'),
(103, 'ALEXANIA', 'GO'),
(104, 'ALFENAS', 'MG'),
(105, 'ALFREDO CHAVES', 'ES'),
(106, 'ALFREDO MARCONDES', 'SP'),
(107, 'ALFREDO VASCONCELOS', 'MG'),
(108, 'ALFREDO WAGNER', 'SC'),
(109, 'ALGODÃO DE JANDAIRA', 'PB'),
(110, 'ALHANDRA', 'PB'),
(111, 'ALIANÇA', 'PE'),
(112, 'ALIANÇA DO TOCANTINS', 'TO'),
(113, 'ALMADINA', 'BA'),
(114, 'ALMAGE', 'BA'),
(115, 'ALMAS', 'TO'),
(116, 'ALMEIRIM', 'PA'),
(117, 'ALMENARA', 'MG'),
(118, 'ALMINO AFONSO', 'RN'),
(119, 'ALMIRANTE TAMANDARE', 'PR'),
(120, 'ALOÂNDIA', 'GO'),
(121, 'ALPERCATA', 'MG'),
(122, 'ALPESTRE', 'RS'),
(123, 'ALPINÓPOLIS', 'MG'),
(124, 'ALTA FLORESTA', 'MT'),
(125, 'ALTA FLORESTA DO OESTE', 'RO'),
(126, 'ALTAIR', 'SP'),
(127, 'ALTAMIRA', 'PA'),
(128, 'ALTAMIRA DO MARANHÃO', 'MA'),
(129, 'ALTAMIRA DO PARANÁ', 'PR'),
(130, 'ALTANEIRA', 'CE'),
(131, 'ALTEROSA', 'MG'),
(132, 'ALTINHO', 'PE'),
(133, 'ALTINÓPOLIS', 'SP'),
(134, 'ALTO ALEGRE', 'RS'),
(135, 'ALTO ALEGRE DO MARANHÃO', 'MA'),
(136, 'ALTO ARAGUAIA', 'MT'),
(137, 'ALTO BELA VISTA', 'SC'),
(138, 'ALTO BOA VISTA', 'MT'),
(139, 'ALTO CAPARAÓ', 'MG'),
(140, 'ALTO DO RODRIGUES', 'RN'),
(141, 'ALTO FELIZ', 'RS'),
(142, 'ALTO GARÇAS', 'MT'),
(143, 'ALTO HORIZONTE', 'GO'),
(144, 'ALTO JEQUITIBÁ', 'MG'),
(145, 'ALTO LONGA', 'PI'),
(146, 'ALTO PARAGUAI', 'MT'),
(147, 'ALTO PARAISO', 'RO'),
(148, 'ALTO PARAÍSO DE GOIÁS', 'GO'),
(149, 'ALTO PARANÁ', 'PR'),
(150, 'ALTO PARNAÍBA', 'MA'),
(151, 'ALTO PIQUIRI', 'PR'),
(152, 'ALTO RIO DOCE', 'MG'),
(153, 'ALTO RIO NOVO', 'ES'),
(154, 'ALTO SANTO', 'CE'),
(155, 'ALTO TAQUARI', 'MT'),
(156, 'ALTONIA', 'PR'),
(157, 'ALTOS', 'PI'),
(158, 'ALUMINIO', 'SP'),
(159, 'ALVARÃES', 'AM'),
(160, 'ALVARENGA', 'MG'),
(161, 'ALVARES FLORENCE', 'SP'),
(162, 'ÁLVARES MACHADO', 'SP'),
(163, 'ÁLVARO DE CARVALHO', 'SP'),
(164, 'ALVINLÂNDIA', 'SP'),
(165, 'ALVINÓPOLIS', 'MG'),
(166, 'ALVORADA', 'RS'),
(167, 'ALVORADA DE MINAS', 'MG'),
(168, 'ALVORADA DO GURGUEIRA', 'PI'),
(169, 'ALVORADA DO NORTE', 'GO'),
(170, 'ALVORADA DO SUL', 'PR'),
(171, 'ALVORADA DO OESTE', 'RO'),
(172, 'AMAJARI', 'RR'),
(173, 'AMAMBAÍ', 'MS'),
(174, 'AMAPÁ', 'AP'),
(175, 'AMAPÁ DO MARANHÃO', 'MA'),
(176, 'AMAPORÃ', 'PR'),
(177, 'AMARAJI', 'PE'),
(178, 'AMARAL FERRADOR', 'RS'),
(179, 'AMARALINA', 'GO'),
(180, 'AMARANTE', 'PI'),
(181, 'AMARANTE DO MARANHÃO', 'MA'),
(182, 'AMARGOSA', 'BA'),
(183, 'AMATURÁ', 'AM'),
(184, 'AMAZONAS', 'PR'),
(185, 'AMÉLIA RODRIGUES', 'BA'),
(186, 'AMÉRICA DOURADA', 'BA'),
(187, 'AMÉRICANA', 'SP'),
(188, 'AMÉRICANO DO BRASIL', 'GO'),
(189, 'AMÉRICO BRASILIENSE', 'SP'),
(190, 'AMÉRICO DE CAMPOS', 'SP'),
(191, 'AMETISTA DO SUL', 'RS'),
(192, 'AMONTADA', 'CE'),
(193, 'AMORINÓPOLIS', 'GO'),
(194, 'AMPARO', 'PB'),
(195, 'AMPARO', 'SP'),
(196, 'AMPARO DA SERRA', 'MG'),
(197, 'AMPARO DE SÃO FRANCISCO', 'SE'),
(198, 'AMPÉRE', 'PR'),
(199, 'ANADIA', 'AL'),
(200, 'ANAGÉ', 'BA'),
(201, 'ANAHY', 'PR'),
(202, 'ANAJÁS', 'PA'),
(203, 'ANAJATUBA', 'MA'),
(204, 'ANALÂNDIA', 'SP'),
(205, 'ANAMA', 'AM'),
(206, 'ANANÁS', 'TO'),
(207, 'ANANINDEUA', 'PA'),
(208, 'ANÁPOLIS', 'GO'),
(209, 'ANAPU', 'PA'),
(210, 'ANAPURUS', 'MA'),
(211, 'ANASTÁCIO', 'MS'),
(212, 'ANAURILÂNDIA', 'MS'),
(213, 'ANCHIETA', 'ES'),
(214, 'ANDARAI', 'BA'),
(215, 'ANDIRA', 'PR'),
(216, 'ANDORINHA', 'BA'),
(217, 'ANDRADAS', 'MG'),
(218, 'ANDRADINA', 'SP'),
(219, 'ANDRÉ DA ROCHA', 'RS'),
(220, 'ANDRELÂNDIA', 'MG'),
(221, 'ANGATUBA', 'SP'),
(222, 'ANGELÂNDIA', 'MG'),
(223, 'ANGÉLICA', 'MS'),
(224, 'ANGELIM', 'PE'),
(225, 'ANGELINA', 'SC'),
(226, 'ANGICAL', 'BA'),
(227, 'ANGICAL DO PIAUÍ', 'PI'),
(228, 'ANGICO', 'TO'),
(229, 'ANGICOS', 'RN'),
(230, 'ANGRA DOS REIS', 'RJ'),
(231, 'ANGUERA', 'BA'),
(232, 'ANGULO', 'PR'),
(233, 'ANHANGUERA', 'GO'),
(234, 'ANHEMBI', 'SP'),
(235, 'ANHUMAS', 'SP'),
(236, 'ANICUNS', 'GO'),
(237, 'ANISIO DE ABREU', 'PI'),
(238, 'ANITA GARIBALDI', 'SC'),
(239, 'ANITAPOLIS', 'SC'),
(240, 'ANORI', 'AM'),
(241, 'ANTA GORDA', 'RS'),
(242, 'ANTAS', 'BA'),
(243, 'ANTONINA', 'PR'),
(244, 'ANTONINA DO NORTE', 'CE'),
(245, 'ANTÔNIO ALMEIDA', 'PI'),
(246, 'ANTÔNIO CARDOSO', 'BA'),
(247, 'ANTÔNIO CARLOS', 'MG'),
(248, 'ANTÔNIO DIAS', 'MG'),
(249, 'ANTÔNIO GONÇALVES', 'BA'),
(250, 'ANTÔNIO JOÃO', 'MS'),
(251, 'ANTÔNIO MARTINS', 'RN'),
(252, 'ANTÔNIO OLINTO', 'PR'),
(253, 'ANTÔNIO PRADO', 'RS'),
(254, 'ANTÔNIO PRADO DE MINAS', 'MG'),
(255, 'APARECIDA', 'PB'),
(256, 'APARECIDA', 'SP'),
(257, 'APARECIDA DE GOIANIA', 'GO'),
(258, 'APARECIDA DO RIO DOCE', 'GO'),
(259, 'APARECIDA DO RIO NEGRO', 'TO'),
(260, 'APARECIDA DO TABOADO', 'MS'),
(261, 'APARECIDA DO OESTE', 'SP'),
(262, 'APERIBÉ', 'RJ'),
(263, 'APIACA', 'ES'),
(264, 'APIACÁS', 'MT'),
(265, 'APIAÍ', 'SP'),
(266, 'APICUM AÇU', 'MA'),
(267, 'APIUNA', 'SC'),
(268, 'APODI', 'RN'),
(269, 'APORA', 'BA'),
(270, 'APORE', 'GO'),
(271, 'APUAREMA', 'BA'),
(272, 'APUCARANA', 'PR'),
(273, 'APUI', 'AM'),
(274, 'APUIARÉS', 'CE'),
(275, 'AQUIDABÃ', 'SE'),
(276, 'AQUIDAUANA', 'MS'),
(277, 'AQUIRAZ', 'CE'),
(278, 'ARABUTÃ', 'SC'),
(279, 'ARAÇAGI', 'PB'),
(280, 'ARAÇAI', 'MG'),
(281, 'ARACAJU', 'SE'),
(282, 'ARAÇARIGUAMA', 'SP'),
(283, 'ARAÇAS', 'BA'),
(284, 'ARAÇATI', 'CE'),
(285, 'ARAÇATU', 'BA'),
(286, 'ARAÇATUBA', 'SP'),
(287, 'ARACI', 'BA'),
(288, 'ARACITABA', 'MG'),
(289, 'ARACOIABA', 'CE'),
(290, 'ARAÇOIABA', 'PE'),
(291, 'ARAÇOIABA DA SERRA', 'SP'),
(292, 'ARACRUZ', 'ES'),
(293, 'ARAÇU', 'GO'),
(294, 'ARAÇUAI', 'MG'),
(295, 'ARAGARÇAS', 'GO'),
(296, 'ARAGOIANIA', 'GO'),
(297, 'ARAGOMINAS', 'TO'),
(298, 'ARÁGUACEMA', 'TO'),
(299, 'ARAGUAÇU', 'TO'),
(300, 'ARÁGUAIANA', 'MT'),
(301, 'ARÁGUAINA', 'TO'),
(302, 'ARAGUANÃ', 'MA'),
(303, 'ARÁGUAPAZ', 'GO'),
(304, 'ARÁGUARI', 'MG'),
(305, 'ARÁGUATINS', 'TO'),
(306, 'ARAIOSES', 'MA'),
(307, 'ARAL MOREIRA', 'MS'),
(308, 'ARAMARI', 'BA'),
(309, 'ARAMBARÉ', 'RS'),
(310, 'ARAME', 'MA'),
(311, 'ARAMINA', 'SP'),
(312, 'ARANDU', 'SP'),
(313, 'ARANTINA', 'MG'),
(314, 'ARAPEI', 'SP'),
(315, 'ARAPIRACA', 'AL'),
(316, 'ARAPIRINA', 'PE'),
(317, 'ARAPOEMA', 'TO'),
(318, 'ARAPONGA', 'MG'),
(319, 'ARAPONGAS', 'PR'),
(320, 'ARAPORA', 'MG'),
(321, 'ARAPOTI', 'PR'),
(322, 'ARAPUA', 'MG'),
(323, 'ARAPUTANGA', 'MT'),
(324, 'ARAQUARI', 'SC'),
(325, 'ARARA', 'PB'),
(326, 'ARARANGUA', 'SC'),
(327, 'ARARAQUARA', 'SP'),
(328, 'ARARAS', 'SP'),
(329, 'ARARENDÁ', 'CE'),
(330, 'ARARICÁ', 'RS'),
(331, 'ARARIPE', 'CE'),
(332, 'ARARIPINA', 'PE'),
(333, 'ARARUAMA', 'RJ'),
(334, 'ARARUNA', 'PR'),
(335, 'ARATACA', 'BA'),
(336, 'ARATIBA', 'RS'),
(337, 'ARATUBA', 'CE'),
(338, 'ARATUIPE', 'BA'),
(339, 'ARAUÁ', 'SE'),
(340, 'ARAUCARIA', 'PR'),
(341, 'ARAUJOS', 'MG'),
(342, 'ARAXA', 'MG'),
(343, 'ARCEBURGO', 'MG'),
(344, 'ARCO IRIS', 'SP'),
(345, 'ARCOS', 'MG'),
(346, 'ARCOVERDE', 'PE'),
(347, 'AREADO', 'MG'),
(348, 'AREAL', 'RJ'),
(349, 'AREALVA', 'SP'),
(350, 'AREIA', 'PB'),
(351, 'AREIA BRANCA', 'RN'),
(352, 'AREIA BRANCA', 'SE'),
(353, 'AREIA DE BARAÚNAS', 'PB'),
(354, 'AREIAL', 'PB'),
(355, 'AREIAS', 'SP'),
(356, 'AREIÓPOLIS', 'SP'),
(357, 'ARENÁPOLIS', 'MT'),
(358, 'ARENÓPOLIS', 'GO'),
(359, 'ARES', 'RN'),
(360, 'ARGIRITA', 'MG'),
(361, 'ARICANDUVA', 'MG'),
(362, 'ARINOS', 'MG'),
(363, 'ARIPUANÃ', 'MT'),
(364, 'ARIQUEMES', 'RO'),
(365, 'ARIRANHA', 'SP'),
(366, 'ARIRANHA DO IVAI', 'PR'),
(367, 'ARMAÇÃO DE BÚZIOS', 'RJ'),
(368, 'ARMAZEM', 'SC'),
(369, 'ARNEIROZ', 'CE'),
(370, 'AROAZES', 'PI'),
(371, 'AROEIRAS', 'PB'),
(372, 'ARRAIAL', 'PI'),
(373, 'ARRAIAL DO CABO', 'RJ'),
(374, 'ARRAIAS', 'TO'),
(375, 'ARROIO DO MEIO', 'RS'),
(376, 'ARROIO DO PADRE', 'RS'),
(377, 'ARROIO DO SAL', 'RS'),
(378, 'ARROIO DO TIGRE', 'RS'),
(379, 'ARROIO DOS RATOS', 'RS'),
(380, 'ARROIO GRANDE', 'RS'),
(381, 'ARROIO TRINTA', 'SC'),
(382, 'ARTUR NOGUEIRA', 'SP'),
(383, 'ARUANA', 'GO'),
(384, 'ARUJA', 'SP'),
(385, 'ARUNA', 'GO'),
(386, 'ARVOREDO', 'SC'),
(387, 'ARVOREZINHA', 'RS'),
(388, 'ASCURRA', 'SC'),
(389, 'ASPÁSIA', 'SP'),
(390, 'ASSAÍ', 'PR'),
(391, 'ASSARE', 'CE'),
(392, 'ASSIS', 'SP'),
(393, 'ASSIS BRASIL', 'AC'),
(394, 'ASSIS CHATEAUBRIAND', 'PR'),
(395, 'ASSUNCÃO', 'PB'),
(396, 'ASSUNÇÃO DO PIAUÍ', 'PI'),
(397, 'ASTOLFO DUTRA', 'MG'),
(398, 'ASTORGA', 'PR'),
(399, 'ATALAIA', 'AL'),
(400, 'ATALAIA', 'PR'),
(401, 'ATALAIA DO NORTE', 'AM'),
(402, 'ATALANTA', 'SC'),
(403, 'ATALEIA', 'MG'),
(404, 'ATIBAIA', 'SP'),
(405, 'ATILIO VIVACQUA', 'ES'),
(406, 'AUGUSTINÓPOLIS', 'TO'),
(407, 'AUGUSTO CORRÊA', 'PA'),
(408, 'AUGUSTO DE LIMA', 'MG'),
(409, 'AUGUSTO PESTANA', 'RS'),
(410, 'AUREA', 'RS'),
(411, 'AURELINO LEAL', 'BA'),
(412, 'AURIFLAMA', 'SP'),
(413, 'AURILÂNDIA', 'GO'),
(414, 'AURORA', 'CE'),
(415, 'AURORA', 'SC'),
(416, 'AURORA DO PARÁ', 'PA'),
(417, 'AURORA DO TOCANTINS', 'TO'),
(418, 'AUTAZES', 'AM'),
(419, 'AVAI', 'SP'),
(420, 'AVANHANDAVA', 'SP'),
(421, 'AVARE', 'SP'),
(422, 'AVEIRO', 'PA'),
(423, 'AVELINO LOPES', 'PI'),
(424, 'AVELINÓPOLIS', 'GO'),
(425, 'AXIXA', 'MA'),
(426, 'AXIXÁ DO TOCANTINS', 'TO'),
(427, 'BABAÇULÂNDIA', 'TO'),
(428, 'BACABAL', 'MA'),
(429, 'BACURI', 'MA'),
(430, 'BAÇURITUBA', 'MA'),
(431, 'BADY BASSITT', 'SP'),
(432, 'BAEPENDI', 'MG'),
(433, 'BAGÉ', 'RS'),
(434, 'BAGRE', 'PA'),
(435, 'BAIA DA TRAICÃO', 'PB'),
(436, 'BAIA FORMOSA', 'RN'),
(437, 'BAIANÓPOLIS', 'BA'),
(438, 'BAIÃO', 'PA'),
(439, 'BAIRRA DALCANTARA', 'PI'),
(440, 'BAIXA GRANDE', 'BA'),
(441, 'BAIXA GRANDE DO RIBEIRO', 'PI'),
(442, 'BAIXIO', 'CE'),
(443, 'BAIXO GUANDU', 'ES'),
(444, 'BALBINOS', 'SP'),
(445, 'BALDIM', 'MG'),
(446, 'BALIZA', 'GO'),
(447, 'BALNEÁRIO BARRA DO SUL', 'SC'),
(448, 'BALNEÁRIO CAMBORIÚ', 'SC'),
(449, 'BALNEÁRIO GAIVOTA', 'SC'),
(450, 'BALNEÁRIO PINHAL', 'RS'),
(451, 'BALSA NOVA', 'PR'),
(452, 'BALSAMO', 'SP'),
(453, 'BALSAS', 'MA'),
(454, 'BAMBUI', 'MG'),
(455, 'BANABUIU', 'CE'),
(456, 'BANANAL', 'SP'),
(457, 'BANANEIRAS', 'PB'),
(458, 'BANDEIRA', 'MG'),
(459, 'BANDEIRA DO SUL', 'MG'),
(460, 'BANDEIRANTE', 'TO'),
(461, 'BANDEIRANTES', 'MS'),
(462, 'BANNACH', 'PA'),
(463, 'BANZAE', 'BA'),
(464, 'BARÃO', 'RS'),
(465, 'BARÃO DE ANTONINA', 'SP'),
(466, 'BARÃO DE COCAIS', 'MG'),
(467, 'BARÃO DE COTEGIPE', 'RS'),
(468, 'BARÃO DE MELGAÇO', 'MT'),
(469, 'BARÃO DE MONTE ALTO', 'MG'),
(470, 'BARÃO DO GRAJAÚ', 'MA'),
(471, 'BARÃO DO TRIUNFO', 'RS'),
(472, 'BARAÚNA', 'PB'),
(473, 'BARBACENA', 'MG'),
(474, 'BARBALHA', 'CE'),
(475, 'BARBOSA', 'SP'),
(476, 'BARBOSA FERRAZ', 'PR'),
(477, 'BARCELONA', 'RN'),
(478, 'BARCELOS', 'AM'),
(479, 'BARIRI', 'SP'),
(480, 'BARRA', 'BA'),
(481, 'BARRA BONITA', 'SC'),
(482, 'BARRA DA ESTIVA', 'BA'),
(483, 'BARRA DE GUABIRABA', 'PE'),
(484, 'BARRA DE SANTA ROSA', 'PB'),
(485, 'BARRA DE SANTANA', 'PB'),
(486, 'BARRA DE SANTO ANTÔNIO', 'AL'),
(487, 'BARRA DE SÃO FRANCISCO', 'ES'),
(488, 'BARRA DE SÃO MIGUEL', 'AL'),
(489, 'BARRA DO BUGRES', 'MT'),
(490, 'BARRA DO CHAPÉU', 'SP'),
(491, 'BARRA DO CHOCA', 'BA'),
(492, 'BARRA DO CORDA', 'MA'),
(493, 'BARRA DO GARÇAS', 'MT'),
(494, 'BARRA DO GUARITA', 'RS'),
(495, 'BARRA DO JACARÉ', 'PR'),
(496, 'BARRA DO MENDES', 'BA'),
(497, 'BARRA DO OURO', 'TO'),
(498, 'BARRA DO PIRAÍ', 'RJ'),
(499, 'BARRA DO QUARAI', 'RS'),
(500, 'BARRA DO RIBEIRO', 'RS'),
(501, 'BARRA DO RIO AZUL', 'RS'),
(502, 'BARRA DO ROCHA', 'BA'),
(503, 'BARRA DO TURVO', 'SP'),
(504, 'BARRA DOS COQUEIROS', 'SE'),
(505, 'BARRA FUNDA', 'RS'),
(506, 'BARRA LONGA', 'MG'),
(507, 'BARRA MANSA', 'RJ'),
(508, 'BARRA VELHA', 'SC'),
(509, 'BARRACÃO', 'PR'),
(510, 'BARRAÇÃO', 'RS'),
(511, 'BARRAS', 'PI'),
(512, 'BARREIRA', 'CE'),
(513, 'BARREIRAS', 'BA'),
(514, 'BARREIRAS DO PIAUÍ', 'PI'),
(515, 'BARREIRINHA', 'AM'),
(516, 'BARREIRINHAS', 'MA'),
(517, 'BARREIROS', 'PE'),
(518, 'BARRERAS', 'PE'),
(519, 'BARRETOS', 'SP'),
(520, 'BARRINHA', 'SP'),
(521, 'BARRO', 'CE'),
(522, 'BARRO ALTO', 'BA'),
(523, 'BARRO DURO', 'PI'),
(524, 'BARRO PRETO', 'BA'),
(525, 'BARROLÂNDIA', 'TO'),
(526, 'BARROQUINHA', 'CE'),
(527, 'BARROS CASSAL', 'RS'),
(528, 'BARROSO', 'MG'),
(529, 'BARUERI', 'SP'),
(530, 'BASTOS', 'SP'),
(531, 'BATÁGUASSU', 'MS'),
(532, 'BATAIPORA', 'MS'),
(533, 'BATALHA', 'AL'),
(534, 'BATATAIS', 'SP'),
(535, 'BATURITE', 'CE'),
(536, 'BAURU', 'SP'),
(537, 'BAYEUX', 'PB'),
(538, 'BEBEDOURO', 'SP'),
(539, 'BEBERIBE', 'CE'),
(540, 'BELA CRUZ', 'CE'),
(541, 'BELA VISTA', 'MG'),
(542, 'BELA VISTA', 'MS'),
(543, 'BELA VISTA DA CAROBA', 'PR'),
(544, 'BELA VISTA DE GOIÁS', 'GO'),
(545, 'BELA VISTA DE MINAS', 'MG'),
(546, 'BELA VISTA DO PIAUÍ', 'PI'),
(547, 'BELA VISTA DO TOLDO', 'SC'),
(548, 'BELA VISTA DO PARAÍSO', 'PR'),
(549, 'BELÁGUA', 'MA'),
(550, 'BELEM', 'PA'),
(551, 'BELÉM', 'PB'),
(552, 'BELÉM', 'AL'),
(553, 'BELÉM DE MARIA', 'PE'),
(554, 'BELÉM DE SÃO FRANCISCO', 'PE'),
(555, 'BELÉM DO BREJO DO CRUZ', 'PB'),
(556, 'BELÉM DO PIAUÍ', 'PI'),
(557, 'BELFORD ROXO', 'RJ'),
(558, 'BELMIRO BRAGA', 'MG'),
(559, 'BELMONTE', 'BA'),
(560, 'BELO CAMPO', 'BA'),
(561, 'BELO HORIZONTE', 'MG'),
(562, 'BELO JARDIM', 'PE'),
(563, 'BELO MONTE', 'AL'),
(564, 'BELO ORIENTE', 'MG'),
(565, 'BELO VALE', 'MG'),
(566, 'BELTERRA', 'PA'),
(567, 'BENEDITINOS', 'PI'),
(568, 'BENEDITO LEITE', 'MA'),
(569, 'BENEDITO NOVO', 'SC'),
(570, 'BENEVIDES', 'PA'),
(571, 'BENJAMIN CONSTANT DO SUL', 'RS'),
(572, 'BENJAMIN CONSTANT', 'AM'),
(573, 'BENTO DE ABREU', 'SP'),
(574, 'BENTO FERNANDES', 'RN'),
(575, 'BENTO GONÇALVES', 'RS'),
(576, 'BEQUIMÃO', 'MA'),
(577, 'BERILO', 'MG'),
(578, 'BERIZAL', 'MG'),
(579, 'BERNARDINO BATISTA', 'PB'),
(580, 'BERNARDINO DE CAMPOS', 'SP'),
(581, 'BERNARDO SAYÃO', 'TO'),
(582, 'BERTIOGA', 'SP'),
(583, 'BERTOLÍNIA', 'PI'),
(584, 'BERTÓPOLIS', 'MG'),
(585, 'BERURI', 'AM'),
(586, 'BETANIA', 'PE'),
(587, 'BETANIA DO PIAUÍ', 'PI'),
(588, 'BETIM', 'MG'),
(589, 'BEZERROS', 'PE'),
(590, 'BIAS FORTES', 'MG'),
(591, 'BICAS', 'MG'),
(592, 'BIGUAÇU', 'SC'),
(593, 'BILAC', 'SP'),
(594, 'BIQUINHAS', 'MG'),
(595, 'BIRIGUI', 'SP'),
(596, 'BIRITIBA-MIRIM', 'SP'),
(597, 'BIRITINGA', 'BA'),
(598, 'BITURUNA', 'PR'),
(599, 'BLUMENAU', 'SC'),
(600, 'BOA ESPERANÇA', 'ES'),
(601, 'BOA ESPERANÇA', 'PR'),
(602, 'BOA ESPERANÇA DO SUL', 'SP'),
(603, 'BOA ESPERANÇA DO IGUAÇU', 'PR'),
(604, 'BOA HORA', 'PI'),
(605, 'BOA NOVA', 'BA'),
(606, 'BOA SAÚDE', 'RN'),
(607, 'BOA VENTURA', 'PB'),
(608, 'BOA VENTURA DE SÃO ROQUE', 'PR'),
(609, 'BOA VIAGEM', 'CE'),
(610, 'BOA VISTA', 'PB'),
(611, 'BOA VISTA', 'RR'),
(612, 'BOA VISTA DA APARECIDA', 'PR'),
(613, 'BOA VISTA DAS MISSÕES', 'RS'),
(614, 'BOA VISTA DO BURICA', 'RS'),
(615, 'BOA VISTA DO GURUPI', 'MA'),
(616, 'BOA VISTA DO RAMOS', 'AM'),
(617, 'BOA VISTA DO SUL', 'RS'),
(618, 'BOA VISTA DO TUPIM', 'BA'),
(619, 'BOCA DA MATA', 'AL'),
(620, 'BOCA DO ACRE', 'AM'),
(621, 'BOCAINA', 'PI'),
(622, 'BOCAINA DE MINAS', 'MG'),
(623, 'BOCAINA DO SUL', 'SC'),
(624, 'BOCAIUVA', 'MG'),
(625, 'BOCAIUVA DO SUL', 'PR'),
(626, 'BODO', 'RN'),
(627, 'BODOCO', 'PE'),
(628, 'BODOQUENA', 'MS'),
(629, 'BOFETE', 'SP'),
(630, 'BOITUVA', 'SP'),
(631, 'BOM CAVATI', 'MG'),
(632, 'BOM CONSELHO', 'PE'),
(633, 'BOM DESPACHO', 'MG'),
(634, 'BOM JARDIM', 'MA'),
(635, 'BOM JARDIM DA SERRA', 'SC'),
(636, 'BOM JARDIM DE GOIÁS', 'GO'),
(637, 'BOM JARDIM DE MINAS', 'MG'),
(638, 'BOM JESUS', 'PB'),
(639, 'BOM JESUS', 'SC'),
(640, 'BOM JESUS DE ITABAPOANA', 'RJ'),
(641, 'BOM JESUS DA LAPA', 'BA'),
(642, 'BOM JESUS DA PENHA', 'MG'),
(643, 'BOM JESUS DA SERRA', 'BA'),
(644, 'BOM JESUS DAS SELVAS', 'MA'),
(645, 'BOM JESUS DE GOIÁS', 'GO'),
(646, 'BOM JESUS DO AMPARO', 'MG'),
(647, 'BOM JESUS DO GALHO', 'MG'),
(648, 'BOM JESUS DO NORTE', 'ES'),
(649, 'BOM JESUS DO OESTE', 'SC'),
(650, 'BOM JESUS DO TOCANTINS', 'PA'),
(651, 'BOM JESUS DOS PERDÕES', 'SP'),
(652, 'BOM LUGAR', 'MA'),
(653, 'BOM PRINCIPIO', 'RS'),
(654, 'BOM PRINCIPIO DO PIAUÍ', 'PI'),
(655, 'BOM PROGRESSO', 'RS'),
(656, 'BOM REPOUSO', 'MG'),
(657, 'BOM RETIRO', 'SC'),
(658, 'BOM RETIRO DO SUL', 'RS'),
(659, 'BOM SUCESSO', 'MG'),
(660, 'BOM SUCESSO', 'PB'),
(661, 'BOM SUCESSO DE ITARARÉ', 'SP'),
(662, 'BOM SUCESSO', 'PR'),
(663, 'BOMBINHAS', 'SC'),
(664, 'BONFIM', 'MG'),
(665, 'BONFIM', 'RR'),
(666, 'BONFIM DO PIAUÍ', 'PI'),
(667, 'BONFINÓPOLIS', 'GO'),
(668, 'BONFINÓPOLIS DE MINAS', 'MG'),
(669, 'BONINAL', 'BA'),
(670, 'BONITO', 'BA'),
(671, 'BONITO DE SANTA FÉ', 'PB'),
(672, 'BONÓPOLIS', 'GO'),
(673, 'BOQUEIRÃO', 'PB'),
(674, 'BOQUEIRÃO DO LEÃO', 'RS'),
(675, 'BOQUEIRÃO DO PIAUÍ', 'PI'),
(676, 'BOQUIM', 'SE'),
(677, 'BOQUIRA', 'BA'),
(678, 'BORA', 'SP'),
(679, 'BORACEIA', 'SP'),
(680, 'BORBA', 'AM'),
(681, 'BORBOREMA', 'PB'),
(682, 'BORDA DA MATA', 'MG'),
(683, 'BOREBI', 'SP'),
(684, 'BORRAZÓPOLIS', 'PR'),
(685, 'BOSSOROCA', 'RS'),
(686, 'BOTELHOS', 'MG'),
(687, 'BOTUCATU', 'SP'),
(688, 'BOTUMIRIM', 'MG'),
(689, 'BOTUPORA', 'BA'),
(690, 'BOTUVERA', 'SC'),
(691, 'BRACO DO NORTE', 'SC'),
(692, 'BRACO DO TROMBUDO', 'SC'),
(693, 'BRAGA', 'RS'),
(694, 'BRAGANÇA', 'PA'),
(695, 'BRAGANÇA PAULISTA', 'SP'),
(696, 'BRAGANEY', 'PR'),
(697, 'BRANQUINHA', 'AL'),
(698, 'BRAS PIRES', 'MG'),
(699, 'BRASIL NOVO', 'PA'),
(700, 'BRASILÂNDIA', 'MS'),
(701, 'BRASILÂNDIA DE MINAS', 'MG'),
(702, 'BRASILÂNDIA DO SUL', 'PR'),
(703, 'BRASILÂNDIA DO TOCANTINS', 'TO'),
(704, 'BRASILÉIA', 'AC'),
(705, 'BRASILEIRA', 'PI'),
(706, 'BRASILIA', 'DF'),
(707, 'BRASÍLIA DE MINAS', 'MG'),
(708, 'BRASNORTE', 'MT'),
(709, 'BRASÓPOLIS', 'MG'),
(710, 'BRAÚNA', 'SP'),
(711, 'BRAUNAS', 'MG'),
(712, 'BRAZABRANTES', 'GO'),
(713, 'BRAZILÂNDIA DO OESTE', 'RO'),
(714, 'BREJÃO', 'PE'),
(715, 'BREJETUBA', 'ES'),
(716, 'BREJINHO', 'PE'),
(717, 'BREJINHO DE NAZARÉ', 'TO'),
(718, 'BREJO', 'MA'),
(719, 'BREJO ALEGRE', 'SP'),
(720, 'BREJO DA MADRE DE DEUS', 'PE'),
(721, 'BREJO DE AREIA', 'MA'),
(722, 'BREJO DO CRUZ', 'PB'),
(723, 'BREJO DO PIAUÍ', 'PI'),
(724, 'BREJO DOS SANTOS', 'PB'),
(725, 'BREJO GRANDE', 'SE'),
(726, 'BREJO GRANDE DO ARÁGUAIA', 'PA'),
(727, 'BREJO SANTO', 'CE'),
(728, 'BREJOES', 'BA'),
(729, 'BREJOLÂNDIA', 'BA'),
(730, 'BREU BRANCO', 'PA'),
(731, 'BREVES', 'PA'),
(732, 'BRITANIA', 'GO'),
(733, 'BROCHIER', 'RS'),
(734, 'BRODOWSKI', 'SP'),
(735, 'BROTAS', 'SP'),
(736, 'BROTAS DE MACAÚBAS', 'BA'),
(737, 'BRUMADINHO', 'MG'),
(738, 'BRUMADO', 'BA'),
(739, 'BRUNÓPOLIS', 'SC'),
(740, 'BRUSQUE', 'SC'),
(741, 'BUENO BRANDÃO', 'MG'),
(742, 'BUENÓPOLIS', 'MG'),
(743, 'BUENOS AIRES', 'PE'),
(744, 'BUERAREMA', 'BA'),
(745, 'BUGRE', 'MG'),
(746, 'BUÍQUE', 'PE'),
(747, 'BUJARI', 'AC'),
(748, 'BUJARU', 'PA'),
(749, 'BURI', 'SP'),
(750, 'BURITAMA', 'SP'),
(751, 'BURITI', 'MA'),
(752, 'BURITI ALEGRE', 'GO'),
(753, 'BURITI BRAVO', 'MA'),
(754, 'BURITI DE GOIÁS', 'GO'),
(755, 'BURITI DO TOCANTINS', 'TO'),
(756, 'BURITI DOS LOPES', 'PI'),
(757, 'BURITI DOS MONTES', 'PI'),
(758, 'BURITICUPU', 'MA'),
(759, 'BURITINÓPOLIS', 'GO'),
(760, 'BURITIRAMA', 'BA'),
(761, 'BURITIRANA', 'MA'),
(762, 'BURITIS', 'MG'),
(763, 'BURITIS', 'RO'),
(764, 'BURITIZAL', 'SP'),
(765, 'BURITIZEIRO', 'MG'),
(766, 'BUTIA', 'RS'),
(767, 'CAAPIRANGA', 'AM'),
(768, 'CAAPORA', 'PB'),
(769, 'CAARAPO', 'MS'),
(770, 'CAATIBA', 'BA'),
(771, 'CABACEIRAS', 'PB'),
(772, 'CABACEIRAS DO PARAGUAÇU', 'BA'),
(773, 'CABECEIRA GRANDE', 'MG'),
(774, 'CABECEIRAS', 'GO'),
(775, 'CABECEIRAS DO PIAUÍ', 'PI'),
(776, 'CABEDELO', 'PB'),
(777, 'CABO DE SANTO AGOSTINHO', 'PE'),
(778, 'CABO FRIO', 'RJ'),
(779, 'CABO VERDE', 'MG'),
(780, 'CABRÁLIA PAULISTA', 'SP'),
(781, 'CABREUVA', 'SP'),
(782, 'CABROBO', 'PE'),
(783, 'CACADOR', 'SC'),
(784, 'CAÇAPAVA', 'SP'),
(785, 'CAÇAPAVA DO SUL', 'RS'),
(786, 'CACAULÂNDIA', 'RO'),
(787, 'CACEQUI', 'RS'),
(788, 'CÁCERES', 'MT'),
(789, 'CACHOEIRA', 'BA'),
(790, 'CACHOEIRA ALTA', 'GO'),
(791, 'CACHOEIRA DA PRATA', 'MG'),
(792, 'CACHOEIRA DE GOIÁS', 'GO'),
(793, 'CACHOEIRA DE MINAS', 'MG'),
(794, 'CACHOEIRA DO ARARI', 'PA'),
(795, 'CACHOEIRA DO PAJEU', 'MG'),
(796, 'CACHOEIRA DO PIRIA', 'PA'),
(797, 'CACHOEIRA DO SUL', 'RS'),
(798, 'CACHOEIRA DOS ÍNDIOS', 'PB'),
(799, 'CACHOEIRA DOURADA', 'GO'),
(800, 'CACHOEIRA GRANDE', 'MA'),
(801, 'CACHOEIRA PAULISTA', 'SP'),
(802, 'CACHOEIRAS DE MACAÇU', 'RJ'),
(803, 'CACHOEIRINHA', 'PE'),
(804, 'CACHOEIRINHA', 'RS'),
(805, 'CACHOEIRINHA DO TOCANTINS', 'TO'),
(806, 'CACHOEIRO DO ITAPEMIRIM', 'ES'),
(807, 'CACIMBA DE AREIA', 'PB'),
(808, 'CACIMBA DE DENTRO', 'PB'),
(809, 'CACIMBAS', 'PB'),
(810, 'CACIMBINHAS', 'AL'),
(811, 'CACIQUE DOBLE', 'RS'),
(812, 'CACOAL', 'RO'),
(813, 'CACONDE', 'SP'),
(814, 'CAÇU', 'GO'),
(815, 'CACULÉ', 'BA'),
(816, 'CAEM', 'BA'),
(817, 'CAETANÓPOLIS', 'MG'),
(818, 'CAETANOS', 'BA'),
(819, 'CAETÉ', 'MG'),
(820, 'CAETÉS', 'PE'),
(821, 'CAETITÉ', 'BA'),
(822, 'CAFARNAUM', 'BA'),
(823, 'CAFEARA', 'PR'),
(824, 'CAFELÂNDIA', 'PR'),
(825, 'CAFEZAL DO SUL', 'PR'),
(826, 'CAIABU', 'SP'),
(827, 'CAIANA', 'MG'),
(828, 'CAIAPÔNIA', 'GO'),
(829, 'CAIBATE', 'RS'),
(830, 'CAIBI', 'SC'),
(831, 'CAIÇARA', 'PB'),
(832, 'CAIÇARA', 'RS'),
(833, 'CAIÇARA DO NORTE', 'RN'),
(834, 'CAIÇARA DO RIO DO VENTO', 'RN'),
(835, 'CAICO', 'RN'),
(836, 'CAIEIRAS', 'SP'),
(837, 'CAIRU', 'BA'),
(838, 'CAIUÁ', 'SP'),
(839, 'CAJAMAR', 'SP'),
(840, 'CAJAPIO', 'MA'),
(841, 'CAJARI', 'MA'),
(842, 'CAJATI', 'SP'),
(843, 'CAJAZEIRAS', 'PB'),
(844, 'CAJAZEIRAS DO PIAUÍ', 'PI'),
(845, 'CAJAZEIRINHAS', 'PB'),
(846, 'CAJOBI', 'SP'),
(847, 'CAJUEIRO', 'AL'),
(848, 'CAJUEIRO DA PRAIA', 'PI'),
(849, 'CAJURI', 'MG'),
(850, 'CAJURU', 'SP'),
(851, 'CALCADO', 'PE'),
(852, 'CALÇADO', 'PE'),
(853, 'CALÇOENE', 'AP'),
(854, 'CALDAS', 'MG'),
(855, 'CALDAS BRANDÃO', 'PB'),
(856, 'CALDAS NOVAS', 'GO'),
(857, 'CALDAZINHA', 'GO'),
(858, 'CALDEIRÃO GRANDE', 'BA'),
(859, 'CALDEIRÃO GRANDE DO PIAUÍ', 'PI'),
(860, 'CALIFORNIA', 'PR'),
(861, 'CALMON', 'SC'),
(862, 'CALUMBI', 'PE'),
(863, 'CAMACA', 'BA'),
(864, 'CAMACAN', 'BA'),
(865, 'CAMACARI', 'BA'),
(866, 'CAMACHO', 'MG'),
(867, 'CAMALAU', 'PB'),
(868, 'CAMAMU', 'BA'),
(869, 'CAMANDUCAIA', 'MG'),
(870, 'CAMAPUÃ', 'MS'),
(871, 'CAMAQUÃ', 'RS'),
(872, 'CAMARAGIBE', 'PE'),
(873, 'CAMARAGIPE', 'PE'),
(874, 'CAMARGO', 'RS'),
(875, 'CAMBARÁ', 'PR'),
(876, 'CAMBARÁ DO SUL', 'RS'),
(877, 'CAMBÉ', 'PR'),
(878, 'CAMBIRA', 'PR'),
(879, 'CAMBORIU', 'SC'),
(880, 'CAMBUCI', 'RJ'),
(881, 'CAMBUI', 'MG'),
(882, 'CAMBUQUIRA', 'MG'),
(883, 'CAMETA', 'PA'),
(884, 'CAMOCIM', 'CE'),
(885, 'CAMOCIM DE SÃO FELIX', 'PE'),
(886, 'CAMPANÁRIO', 'MG'),
(887, 'CAMPANHA', 'MG'),
(888, 'CAMPESTRE', 'AL'),
(889, 'CAMPESTRE DA SERRA', 'RS'),
(890, 'CAMPESTRE DE GOIÁS', 'GO'),
(891, 'CAMPESTRE DO MARANHÃO', 'MA'),
(892, 'CAMPINA DA LAGOA', 'PR'),
(893, 'CAMPINA DAS MISSÕES', 'RS'),
(894, 'CAMPINA DO SIMÃO', 'PR'),
(895, 'CAMPINA GRANDE', 'PB'),
(896, 'CAMPINA GRANDE DO SUL', 'PR'),
(897, 'CAMPINA DO MONTE ALEGRE', 'SP'),
(898, 'CAMPINA VERDE', 'MG'),
(899, 'CAMPINAÇU', 'GO'),
(900, 'CAMPINÁPOLIS', 'MT'),
(901, 'CAMPINAS', 'SP'),
(902, 'CAMPINAS DO PIAUÍ', 'PI'),
(903, 'CAMPINAS DO SUL', 'RS'),
(904, 'CAMPINORTE', 'GO'),
(905, 'CAMPO ALEGRE', 'AL'),
(906, 'CAMPO ALEGRE DE GOIÁS', 'GO'),
(907, 'CAMPO ALEGRE DE LOURDES', 'BA'),
(908, 'CAMPO ALEGRE DO FIDALGO', 'PI'),
(909, 'CAMPO BELO', 'MG'),
(910, 'CAMPO BELO DO SUL', 'SC'),
(911, 'CAMPO BOM', 'RS'),
(912, 'CAMPO BONITO', 'PR'),
(913, 'CAMPO DO BRITO', 'SE'),
(914, 'CAMPO DO MEIO', 'MG'),
(915, 'CAMPO DO TENENTE', 'PR'),
(916, 'CAMPO FLORIDO', 'MG'),
(917, 'CAMPO FORMOSO', 'BA'),
(918, 'CAMPO GRANDE', 'AL'),
(919, 'CAMPO GRANDE', 'MS'),
(920, 'CAMPO GRANDE DO PIAUÍ', 'PI'),
(921, 'CAMPO LARGO', 'PR'),
(922, 'CAMPO LARGO DO PIAUÍ', 'PI'),
(923, 'CAMPO LIMPO DE GOIÁS', 'GO'),
(924, 'CAMPO LIMPO PAULISTA', 'SP'),
(925, 'CAMPO MAGRO', 'PR'),
(926, 'CAMPO MAIOR', 'PI'),
(927, 'CAMPO MOURÃO', 'PR'),
(928, 'CAMPO NOVO', 'RS'),
(929, 'CAMPO NOVO DE RONDONIA', 'RO'),
(930, 'CAMPO NOVO DO PARECIS', 'MT'),
(931, 'CAMPO REDONDO', 'RN'),
(932, 'CAMPO VERDE', 'MT'),
(933, 'CAMPO ERÊ', 'SC'),
(934, 'CAMPOS ALTOS', 'MG'),
(935, 'CAMPOS BELOS DE GOIÁS', 'GO'),
(936, 'CAMPOS BORGES', 'RS'),
(937, 'CAMPOS DE JÚLIO', 'MT'),
(938, 'CAMPOS DO JORDÃO', 'SP'),
(939, 'CAMPOS DOS GOYTACAZES', 'RJ'),
(940, 'CAMPOS GERAIS', 'MG'),
(941, 'CAMPOS LINDOS', 'TO'),
(942, 'CAMPOS NOVOS', 'SC'),
(943, 'CAMPOS NOVOS PAULISTA', 'SP'),
(944, 'CAMPOS SALES', 'CE'),
(945, 'CAMPOS VERDES', 'GO'),
(946, 'CAMUTANGA', 'PE'),
(947, 'CANA BRAVA DO NORTE', 'MT'),
(948, 'CANA VERDE', 'MG'),
(949, 'CANAÃ', 'MG'),
(950, 'CANAÃ DOS CARAJAS', 'PA'),
(951, 'CANANÉIA', 'SP'),
(952, 'CANAPI', 'AL'),
(953, 'CANÁPOLIS', 'BA'),
(954, 'CANARANA', 'BA'),
(955, 'CANAS', 'SP'),
(956, 'CANAVIEIRA', 'PI'),
(957, 'CANAVIEIRAS', 'BA'),
(958, 'CANDEIAS', 'BA'),
(959, 'CANDEIAS DO JAMARI', 'RO'),
(960, 'CANDELÁRIA', 'RS'),
(961, 'CÂNDIDO DE ABREU', 'PR'),
(962, 'CÂNDIDO GODOI', 'RS'),
(963, 'CÂNDIDO MENDES', 'MA'),
(964, 'CÂNDIDO MOTA', 'SP'),
(965, 'CÂNDIDO RODRIGUES', 'SP'),
(966, 'CÂNDIDO SALES', 'BA'),
(967, 'CANDIIBA', 'BA'),
(968, 'CANDIOTA', 'RS'),
(969, 'CANDOI', 'PR'),
(970, 'CANELA', 'RS'),
(971, 'CANELINHA', 'SC'),
(972, 'CANGUARETAMA', 'RN'),
(973, 'CANGUÇU', 'RS'),
(974, 'CANHOBA', 'SE'),
(975, 'CANHOTINHO', 'PE'),
(976, 'CANINDE', 'CE'),
(977, 'CANINDÉ DO SÃO FRANCISCO', 'SE'),
(978, 'CANINÓPOLIS', 'MG'),
(979, 'CANITAR', 'SP'),
(980, 'CANOAS', 'RS'),
(981, 'CANOINHAS', 'SC'),
(982, 'CANSANCÃO', 'BA'),
(983, 'CANTÁ', 'RR'),
(984, 'CANTAGALO', 'PR'),
(985, 'CANTAGALO', 'RJ'),
(986, 'CANTANHEDE', 'MA'),
(987, 'CANTO DO BURITI', 'PI'),
(988, 'CANUDOS', 'BA'),
(989, 'CANUDOS DO VALE', 'RS'),
(990, 'CANUTAMA', 'AM'),
(991, 'CAPANEMA', 'PA'),
(992, 'CAPÃO ALTO', 'SC'),
(993, 'CAPÃO BONITO', 'SP'),
(994, 'CAPÃO DA CANOA', 'RS'),
(995, 'CAPÃO DO LEÃO', 'RS'),
(996, 'CAPARÃO', 'MG'),
(997, 'CAPELA', 'AL'),
(998, 'CAPELA DE SANTANA', 'RS'),
(999, 'CAPELA DO ALTO', 'SP'),
(1000, 'CAPELA DO ALTO ALEGRE', 'BA'),
(1001, 'CAPELA NOVA', 'MG'),
(1002, 'CAPELINHA', 'MG'),
(1003, 'CAPETINGA', 'MG'),
(1004, 'CAPIM', 'PB'),
(1005, 'CAPIM BRANCO', 'MG'),
(1006, 'CAPIM GROSSO', 'BA'),
(1007, 'CAPINÓPOLIS', 'MG'),
(1008, 'CAPINZAL', 'SC'),
(1009, 'CAPITÃO', 'RS'),
(1010, 'CAPITÃO ANDRADE', 'MG'),
(1011, 'CAPITÃO DE CAMPOS', 'PI'),
(1012, 'CAPITÃO ENÉAS', 'MG'),
(1013, 'CAPITÃO GERVÁSIO DE OLIVEIRA', 'PI'),
(1014, 'CAPITÃO LEÔNIDAS MARQUES', 'PR'),
(1015, 'CAPITÃO POÇO', 'PA'),
(1016, 'CAPITÓLIO', 'MG'),
(1017, 'CAPIVARI', 'SP'),
(1018, 'CAPIVARI DE BAIXO', 'SC'),
(1019, 'CAPIVARI DO SUL', 'RS'),
(1020, 'CAPIXABA', 'AC'),
(1021, 'CAPOEIRAS', 'PE'),
(1022, 'CAPRISTANO', 'CE'),
(1023, 'CAPUTIRA', 'MG'),
(1024, 'CARAA', 'RS'),
(1025, 'CARACARAI', 'RR'),
(1026, 'CARACOL', 'MS'),
(1027, 'CARAGUATATUBA', 'SP'),
(1028, 'CARAÍ', 'MG'),
(1029, 'CARAIBAS', 'BA'),
(1030, 'CARAMBEI', 'PR'),
(1031, 'CARANAIBA', 'MG'),
(1032, 'CARANDAÍ', 'MG'),
(1033, 'CARANGOLA', 'MG'),
(1034, 'CARAPEBUS', 'RJ'),
(1035, 'CARAPICUIBA', 'SP'),
(1036, 'CARATINGA', 'MG'),
(1037, 'CARAUARI', 'AM'),
(1038, 'CARAUBAS', 'PB'),
(1039, 'CARAÚBAS DO PIAUÍ', 'PI'),
(1040, 'CARAVELAS', 'BA'),
(1041, 'CARAZINHO', 'RS'),
(1042, 'CARBONITA', 'MG'),
(1043, 'CARDEAL DA SILVA', 'BA'),
(1044, 'CARDOSO', 'SP'),
(1045, 'CARDOSO MOREIRA', 'RJ'),
(1046, 'CAREAÇU', 'MG'),
(1047, 'CAREIRO CASTANHO', 'AM'),
(1048, 'CAREIRO DA VARZEA', 'AM'),
(1049, 'CARIAÇICA', 'ES'),
(1050, 'CARIDADE', 'CE'),
(1051, 'CARIDADE DO PIAUÍ', 'PI'),
(1052, 'CARINHANHA', 'BA'),
(1053, 'CARIRA', 'SE'),
(1054, 'CARIRE', 'CE'),
(1055, 'CARIRI DO TOCANTINS', 'TO'),
(1056, 'CARIRIAÇU', 'CE'),
(1057, 'CARIUS', 'CE'),
(1058, 'CARLINDA', 'MT'),
(1059, 'CARLÓPOLIS', 'PR'),
(1060, 'CARLOS BARBOSA', 'RS'),
(1061, 'CARLOS CHAGAS', 'MG'),
(1062, 'CARLOS GOMES', 'RS'),
(1063, 'CARMESIA', 'MG'),
(1064, 'CARMO', 'RJ'),
(1065, 'CARMO DA CACHOEIRA', 'MG'),
(1066, 'CARMO DA MATA', 'MG'),
(1067, 'CARMO DE MINAS', 'MG'),
(1068, 'CARMO DO CAJURU', 'MG'),
(1069, 'CARMO DO PARANAÍBA', 'MG'),
(1070, 'CARMO DO RIO CLARO', 'MG'),
(1071, 'CARMO DO RIO VERDE', 'GO'),
(1072, 'CARMOLÂNDIA', 'TO'),
(1073, 'CARMÓPOLIS', 'SE'),
(1074, 'CARMÓPOLIS DE MINAS', 'MG'),
(1075, 'CARNAIBA', 'PE'),
(1076, 'CARNAÚBA DOS DANTAS', 'RN'),
(1077, 'CARNAUBAIS', 'RN'),
(1078, 'CARNAUBAL', 'CE'),
(1079, 'CARNAUBEIRA DA PENHA', 'PE'),
(1080, 'CARNEIRINHO', 'MG'),
(1081, 'CARNEIROS', 'AL'),
(1082, 'CAROEBE', 'RR'),
(1083, 'CAROLINA', 'MA'),
(1084, 'CARPINA', 'PE'),
(1085, 'CARRANCAS', 'MG'),
(1086, 'CARRAPATEIRA', 'PB'),
(1087, 'CARRASCO BONITO', 'TO'),
(1088, 'CARUARU', 'PE'),
(1089, 'CARUTAPERA', 'MA'),
(1090, 'CARVALHÓPOLIS', 'MG'),
(1091, 'CARVALHOS', 'MG'),
(1092, 'CASA BRANCA', 'SP'),
(1093, 'CASA GRANDE', 'MG'),
(1094, 'CASA NOVA', 'BA'),
(1095, 'CASCA', 'RS'),
(1096, 'CASCALHO RICO', 'MG'),
(1097, 'CASCAVEL', 'CE'),
(1098, 'CASCAVEL', 'PR'),
(1099, 'CASEARA', 'TO'),
(1100, 'CASEIROS', 'RS'),
(1101, 'CASIMIRO DE ABREU', 'RJ'),
(1102, 'CASINHAS', 'PE'),
(1103, 'CASSERENGUE', 'PB'),
(1104, 'CASSIA', 'MG'),
(1105, 'CÁSSIA DOS COQUEIROS', 'SP'),
(1106, 'CASSILÂNDIA', 'MS'),
(1107, 'CASTANHAL', 'PA'),
(1108, 'CASTANHEIRA', 'MT'),
(1109, 'CASTANHEIRAS', 'RO'),
(1110, 'CASTELÂNDIA', 'GO'),
(1111, 'CASTELO DO PIAUÍ', 'PI'),
(1112, 'CASTILHO', 'SP'),
(1113, 'CASTRO', 'PR'),
(1114, 'CASTRO ALVES', 'BA'),
(1115, 'CATÁGUASES', 'MG'),
(1116, 'CATALÃO', 'GO'),
(1117, 'CATANDUVA', 'SC'),
(1118, 'CATANDUVA', 'SP'),
(1119, 'CATANDUVAS', 'PR'),
(1120, 'CATARINA', 'CE'),
(1121, 'CATAS ALTAS ', 'MG'),
(1122, 'CATENDE', 'PE'),
(1123, 'CATIGUA', 'SP'),
(1124, 'CATINGUEIRA', 'PB'),
(1125, 'CATOLÂNDIA', 'BA'),
(1126, 'CATOLÉ DO ROCHA', 'PB'),
(1127, 'CATU', 'BA'),
(1128, 'CATUIPE', 'RS'),
(1129, 'CATUJI', 'MG'),
(1130, 'CATUNDA', 'CE'),
(1131, 'CATURAÍ', 'GO'),
(1132, 'CATURAMA', 'BA'),
(1133, 'CATURITE', 'PB'),
(1134, 'CATUTI', 'MG'),
(1135, 'CAUCAIA', 'CE'),
(1136, 'CAVALCANTE', 'GO'),
(1137, 'CAXAMBU', 'MG'),
(1138, 'CAXAMBU DO SUL', 'SC'),
(1139, 'CAXIAS', 'MA'),
(1140, 'CAXIAS DO SUL', 'RS'),
(1141, 'CAXINGO', 'PI'),
(1142, 'CEARÁ MIRIM', 'RN'),
(1143, 'CEDRAL', 'MA'),
(1144, 'CEDRO', 'CE'),
(1145, 'CEDRO DO ABAETÉ', 'MG'),
(1146, 'CEDRO SÃO JOÃO', 'SE'),
(1147, 'CELSO RAMOS', 'SC'),
(1148, 'CENTENARIO', 'RS'),
(1149, 'CENTENARIO DO SUL', 'PR'),
(1150, 'CENTRAL', 'BA'),
(1151, 'CENTRAL DE MINAS', 'MG'),
(1152, 'CENTRAL DO MARANHÃO', 'MA'),
(1153, 'CENTRALINA', 'MG'),
(1154, 'CENTRO DO GUILHERME', 'MA'),
(1155, 'CENTRO NOVO DO MARANHÃO', 'MA'),
(1156, 'CEREJEIRAS', 'RO'),
(1157, 'CERES', 'GO'),
(1158, 'CERQUEIRA CESAR', 'SP'),
(1159, 'CERQUILHO', 'SP'),
(1160, 'CERRITO', 'RS'),
(1161, 'CERRO BRANCO', 'RS'),
(1162, 'CERRO CORA', 'RN'),
(1163, 'CERRO GRANDE', 'RS'),
(1164, 'CERRO GRANDE DO SUL', 'RS'),
(1165, 'CERRO LARGO', 'RS'),
(1166, 'CERRO NEGRO', 'SC'),
(1167, 'CESÁRIO LANGE', 'SP'),
(1168, 'CEU AZUL', 'PR'),
(1169, 'CEZARINA', 'GO'),
(1170, 'CHÃ DE ALEGRIA', 'PE'),
(1171, 'CHA GRANDE', 'PE'),
(1172, 'CHÃ PRETA', 'AL'),
(1173, 'CHACARA', 'MG'),
(1174, 'CHALE', 'MG'),
(1175, 'CHAPADA', 'RS'),
(1176, 'CHAPADA DA AREIA', 'TO'),
(1177, 'CHAPADA DA NATIVIDADE', 'TO'),
(1178, 'CHAPADA DO NORTE', 'MG'),
(1179, 'CHAPADA DOS GUIMARÃES', 'MT'),
(1180, 'CHAPADA GAÚCHA', 'MG'),
(1181, 'CHAPADÃO DO CEU', 'GO'),
(1182, 'CHAPADÃO DO LAGEADO', 'SC'),
(1183, 'CHAPADÃO DO SUL', 'MS'),
(1184, 'CHAPADINHA', 'MA'),
(1185, 'CHAPECÓ', 'SC'),
(1186, 'CHARQUEADA', 'SP'),
(1187, 'CHARQUEADAS', 'RS'),
(1188, 'CHARRUA', 'RS'),
(1189, 'CHAVAL', 'CE'),
(1190, 'CHAVANTES', 'SP'),
(1191, 'CHAVES', 'PA'),
(1192, 'CHIADOR', 'MG'),
(1193, 'CHIAPETTA', 'RS'),
(1194, 'CHOPINZINHO', 'PR'),
(1195, 'CHORO', 'CE'),
(1196, 'CHOROZINHO', 'CE'),
(1197, 'CHORROCHO', 'BA'),
(1198, 'CHUI', 'RS'),
(1199, 'CHUPINGUAIA', 'RO'),
(1200, 'CHUVISCA', 'RS'),
(1201, 'CIANORTE', 'PR'),
(1202, 'CICERO DANTAS', 'BA'),
(1203, 'CIDADE DO RECIFE', 'PE'),
(1204, 'CIDADE GAÚCHA', 'PR'),
(1205, 'CIDADE OCIDENTAL', 'GO'),
(1206, 'CIDELÂNDIA', 'MA'),
(1207, 'CIDREIRA', 'RS'),
(1208, 'CIPO', 'BA'),
(1209, 'CIPOTÂNEA', 'MG'),
(1210, 'CIRIACO', 'RS'),
(1211, 'CLARAVAL', 'MG'),
(1212, 'CLARO DOS POÇÕES', 'MG'),
(1213, 'CLÁUDIA', 'MT'),
(1214, 'CLAUDIO', 'MG'),
(1215, 'CLEMENTINA', 'SP'),
(1216, 'CLEVELÂNDIA', 'PR'),
(1217, 'COARACI', 'BA'),
(1218, 'COARI', 'AM'),
(1219, 'COCAL', 'PI'),
(1220, 'COCAL DE TELHA', 'PI'),
(1221, 'COCAL DOS ALVES', 'PI'),
(1222, 'COCAL DO SUL', 'SC'),
(1223, 'COCALINHO', 'MT'),
(1224, 'COCALZINHO DE GOIÁS', 'GO'),
(1225, 'COCEICÃO DA APARECIDA', 'MG'),
(1226, 'COCOS', 'BA'),
(1227, 'CODAJAS', 'AM'),
(1228, 'CODO', 'MA'),
(1229, 'COELHO NETO', 'MA'),
(1230, 'COIMBRA', 'MG'),
(1231, 'COITE DO NOIA', 'AL'),
(1232, 'COIVARAS', 'PI'),
(1233, 'COLARES', 'PA'),
(1234, 'COLATINA', 'ES'),
(1235, 'COLIDER', 'MT'),
(1236, 'COLINA', 'SP'),
(1237, 'COLINAS', 'MA'),
(1238, 'COLINAS DO SUL', 'GO'),
(1239, 'COLINAS DO TOCANTINS', 'TO'),
(1240, 'COLMEIA', 'TO'),
(1241, 'COLOMBIA', 'SP'),
(1242, 'COLOMBO', 'PR'),
(1243, 'COLONIA DE LEOPOLDINA', 'AL'),
(1244, 'COLÔNIA DO GURGUÉIA', 'PI'),
(1245, 'COLONIA DO PIAUÍ', 'PI'),
(1246, 'COLORADO', 'PR'),
(1247, 'COLORADO D''OESTE', 'RO'),
(1248, 'COLUNA', 'MG'),
(1249, 'COMBINADO', 'TO'),
(1250, 'COMENDADOR GOMES', 'MG'),
(1251, 'COMENDADOR LEVY GASPARIAN', 'RJ'),
(1252, 'COMERCINHO', 'MG'),
(1253, 'COMODORO', 'MT'),
(1254, 'CONCEIÇÃO', 'PB'),
(1255, 'CONCEIÇÃO ARÁGUAIA', 'PA'),
(1256, 'CONCEIÇÃO BARRA DE MINAS', 'MG'),
(1257, 'CONCEIÇÃO DA BARRA', 'ES'),
(1258, 'CONCEIÇÃO DA FEIRA', 'BA'),
(1259, 'CONCEIÇÃO DAS ALAGOAS', 'MG'),
(1260, 'CONCEIÇÃO DAS PEDRAS', 'MG'),
(1261, 'CONCEIÇÃO DE IPANEMA', 'MG'),
(1262, 'CONCEIÇÃO DE MACABU', 'RJ'),
(1263, 'CONCEIÇÃO DO ALMEIDA', 'BA'),
(1264, 'CONCEIÇÃO DO CANINDE', 'PI'),
(1265, 'CONCEIÇÃO DO CASTELO', 'ES'),
(1266, 'CONCEIÇÃO DO COITÉ', 'BA'),
(1267, 'CONCEIÇÃO DO JAÇUIPE', 'BA'),
(1268, 'CONCEIÇÃO DO LAGO AÇU', 'MA'),
(1269, 'CONCEIÇÃO DO MATO DENTRO', 'MG'),
(1270, 'CONCEIÇÃO DO PARA', 'MG'),
(1271, 'CONCEIÇÃO DO RIO VERDE', 'MG'),
(1272, 'CONCEIÇÃO DO TOCANTINS', 'TO'),
(1273, 'CONCEIÇÃO DOS OUROS', 'MG'),
(1274, 'CONCHAL', 'SP'),
(1275, 'CONCHAS', 'SP'),
(1276, 'CONCORDIA', 'SC'),
(1277, 'CONCÓRDIA DO PARÁ', 'PA'),
(1278, 'CONDADO', 'PB'),
(1279, 'CONDE', 'BA'),
(1280, 'CONDEUBA', 'BA'),
(1281, 'CONDOR', 'RS'),
(1282, 'CONFINS', 'MG'),
(1283, 'CONFRESA', 'MT'),
(1284, 'CONGO', 'PB'),
(1285, 'CONGONHAL', 'MG'),
(1286, 'CONGONHAS DO CAMPO', 'MG'),
(1287, 'CONGONHAS DO NORTE', 'MG'),
(1288, 'CONGONHINHAS', 'PR'),
(1289, 'CONQUISTA', 'MG'),
(1290, 'CONSELHEIRO LAFAIETE', 'MG'),
(1291, 'CONSELHEIRO MAIRINCK', 'PR'),
(1292, 'CONSELHEIRO PENA', 'MG'),
(1293, 'CONSOLACÃO', 'MG'),
(1294, 'CONSTANTINA', 'RS'),
(1295, 'CONTAGEM', 'MG'),
(1296, 'CONTENDA', 'PR'),
(1297, 'CONTENDAS DO SINCORA', 'BA'),
(1298, 'COQUEIRAL', 'MG'),
(1299, 'COQUEIRO BAIXO', 'RS'),
(1300, 'COQUEIRO SECO', 'AL'),
(1301, 'COQUEIROS DO SUL', 'RS'),
(1302, 'CORAÇÃO DE JESUS', 'MG'),
(1303, 'CORAÇÃO DE MARIA', 'BA'),
(1304, 'CORBÉLIA', 'PR'),
(1305, 'CORDEIRO', 'RJ'),
(1306, 'CORDEIRÓPOLIS', 'SP'),
(1307, 'CORDEIROS', 'BA'),
(1308, 'CORDILHEIRA ALTA', 'SC'),
(1309, 'CORDISBURGO', 'MG'),
(1310, 'CORDISLÂNDIA', 'MG'),
(1311, 'COREAU', 'CE'),
(1312, 'COREMAS', 'PB'),
(1313, 'CORGUINHO', 'MS'),
(1314, 'CORIBE', 'BA'),
(1315, 'CORINTO', 'MG'),
(1316, 'CORNÉLIO PROCÓPIO', 'PR'),
(1317, 'COROACI', 'MG'),
(1318, 'COROADOS', 'SP'),
(1319, 'COROATA', 'MA'),
(1320, 'COROMANDEL', 'MG'),
(1321, 'CORONEL BARROS', 'RS'),
(1322, 'CORONEL BICACO', 'RS'),
(1323, 'CORONEL DOMINGOS SOARES', 'PR'),
(1324, 'CORONEL EZEQUIEL', 'RN'),
(1325, 'CORONEL FABRICIANO', 'MG'),
(1326, 'CORONEL FREITAS', 'SC'),
(1327, 'CORONEL JOÃO PESSOA', 'RN'),
(1328, 'CORONEL JOÃO SÁ', 'BA'),
(1329, 'CORONEL JOSÉ DIAS', 'PI'),
(1330, 'CORONEL MACEDO', 'SP'),
(1331, 'CORONEL MARTINS', 'SC'),
(1332, 'CORONEL MURTA', 'MG'),
(1333, 'CORONEL PACHECO', 'MG'),
(1334, 'CORONEL PILAR', 'RS'),
(1335, 'CORONEL SAPUCAIA', 'MS'),
(1336, 'CORONEL VIVIDA', 'PR'),
(1337, 'CORONEL XAVIER CHAVES', 'MG'),
(1338, 'CORREGO DANTA', 'MG'),
(1339, 'CORREGO DO BOM JESUS', 'MG'),
(1340, 'CORREGO DO OURO', 'GO'),
(1341, 'CÓRREGO NOVO', 'MG'),
(1342, 'CORREIA PINTO', 'SC'),
(1343, 'CORRENTE', 'PI'),
(1344, 'CORRENTES', 'PE'),
(1345, 'CORRENTINA', 'BA'),
(1346, 'CORTES', 'PE'),
(1347, 'CORUMBA', 'MS'),
(1348, 'CORUMBA DE GOIÁS', 'GO'),
(1349, 'CORUMBAIBA', 'GO'),
(1350, 'CORUMBATAI', 'SP'),
(1351, 'CORUMBATAI DO SUL', 'PR'),
(1352, 'CORUMBIARA', 'RO'),
(1353, 'CORUPA', 'SC'),
(1354, 'CORURIPE', 'AL'),
(1355, 'COSMÓPOLIS', 'SP'),
(1356, 'COSMORAMA', 'SP'),
(1357, 'COSTA MARQUES', 'RO'),
(1358, 'COSTA RICA', 'MS'),
(1359, 'COTEGIPE', 'BA'),
(1360, 'COTIA', 'SP'),
(1361, 'COTIPORA', 'RS'),
(1362, 'COTRIGUAÇU', 'MT'),
(1363, 'COUTO DE MAGALHAES MINAS', 'MG'),
(1364, 'COUTO MAGALHAES', 'TO'),
(1365, 'COXILHA', 'RS'),
(1366, 'COXIM', 'MS'),
(1367, 'COXIXOLA', 'PB'),
(1368, 'CRAIBAS', 'AL'),
(1369, 'CRATEUS', 'CE'),
(1370, 'CRATO', 'CE'),
(1371, 'CRAVINHOS', 'SP'),
(1372, 'CRAVOLÂNDIA', 'BA'),
(1373, 'CRICIUMA', 'SC'),
(1374, 'CRISÓLITA', 'MG'),
(1375, 'CRISÓPOLIS', 'BA'),
(1376, 'CRISSIUMAL', 'RS'),
(1377, 'CRISTAIS', 'MG'),
(1378, 'CRISTAIS PAULISTA', 'SP'),
(1379, 'CRISTAL', 'RS'),
(1380, 'CRISTAL DO SUL', 'RS'),
(1381, 'CRISTALÂNDIA DO PIAUÍ', 'PI'),
(1382, 'CRISTÁLIA', 'MG'),
(1383, 'CRISTALINA', 'GO'),
(1384, 'CRISTIANO OTONI', 'MG'),
(1385, 'CRISTIANÓPOLIS', 'GO'),
(1386, 'CRISTINA', 'MG'),
(1387, 'CRISTINÁPOLIS', 'SE'),
(1388, 'CRISTINO CASTRO', 'PI'),
(1389, 'CRISTÓPOLIS', 'BA'),
(1390, 'CRIXAS', 'GO'),
(1391, 'CRIXAS DO TOCANTINS', 'TO'),
(1392, 'CROATÁ', 'CE'),
(1393, 'CROMINIA', 'GO'),
(1394, 'CRUCILÂNDIA', 'MG'),
(1395, 'CRUZ', 'CE'),
(1396, 'CRUZ ALTA', 'RS'),
(1397, 'CRUZ DAS ALMAS', 'BA'),
(1398, 'CRUZ DO ESPÍRITO SANTO', 'PB'),
(1399, 'CRUZ MACHADO', 'PR'),
(1400, 'CRUZALIA', 'SP'),
(1401, 'CRUZEIRO', 'SP'),
(1402, 'CRUZEIRO DA FORTALEZA', 'MG'),
(1403, 'CRUZEIRO DO IGUAÇU', 'PR'),
(1404, 'CRUZEIRO DO OESTE', 'PR'),
(1405, 'CRUZEIRO DO SUL', 'AC'),
(1406, 'CRUZETA', 'RN'),
(1407, 'CRUZILIA', 'MG'),
(1408, 'CRUZMALTINA', 'PR'),
(1409, 'CUBATÃO', 'SP'),
(1410, 'CUBATI', 'PB'),
(1411, 'CUIABÁ', 'MT'),
(1412, 'CUITE', 'PB'),
(1413, 'CUITE DE MAMANGUAPE', 'PB'),
(1414, 'CUITEGI', 'PB'),
(1415, 'CUMARI', 'GO'),
(1416, 'CUMARU', 'PE'),
(1417, 'CUMARU DO NORTE', 'PA'),
(1418, 'CUMBE', 'SE'),
(1419, 'CUNHA', 'SP'),
(1420, 'CUNHAPORA', 'SC'),
(1421, 'CUNHATAI', 'SC'),
(1422, 'CUPARAQUE', 'MG'),
(1423, 'CUPIRA', 'PE'),
(1424, 'CURAÇÁ', 'BA'),
(1425, 'CURIMATA', 'PI'),
(1426, 'CURIONÓPOLIS', 'PA'),
(1427, 'CURITIBA', 'PR'),
(1428, 'CURITIBANOS', 'SC'),
(1429, 'CURIUVA', 'PR'),
(1430, 'CURRAIS', 'PI'),
(1431, 'CURRAIS NOVOS', 'RN'),
(1432, 'CURRAL DE CIMA', 'PB'),
(1433, 'CURRAL DE DENTRO', 'MG'),
(1434, 'CURRAL NOVO DO PIAUÍ', 'PI'),
(1435, 'CURRAL VELHO', 'PB'),
(1436, 'CURRALINHO', 'PA'),
(1437, 'CURRALINHOS', 'PI'),
(1438, 'CURUÁ', 'PA'),
(1439, 'CURUÇÁ', 'PA'),
(1440, 'CURURUPU', 'MA'),
(1441, 'CURVELO', 'MG'),
(1442, 'CUSTÓDIA', 'PE'),
(1443, 'CUTIAS DO ARAGUARI', 'AP'),
(1444, 'DAMIANÓPOLIS', 'GO'),
(1445, 'DAMIÃO', 'PB'),
(1446, 'DAMOLÂNDIA', 'GO'),
(1447, 'DARCINÓPOLIS', 'TO'),
(1448, 'DARIO MEIRA', 'BA'),
(1449, 'DATAS', 'MG'),
(1450, 'DAVID CANABARRO', 'RS'),
(1451, 'DAVINÓPOLIS', 'MA'),
(1452, 'DELFIM MOREIRA', 'MG'),
(1453, 'DELFINÓPOLIS', 'MG'),
(1454, 'DELMIRO GOLVEIA', 'AL'),
(1455, 'DELTA', 'MG'),
(1456, 'DEMERVAL LOBAO', 'PI'),
(1457, 'DENISE', 'MT'),
(1458, 'DEODÁPOLIS', 'MS'),
(1459, 'DEPUTADO IRAPUAN PINHEIRO', 'CE'),
(1460, 'DERESÓPOLIS', 'MG'),
(1461, 'DERRUBADAS', 'RS'),
(1462, 'DESCALVADO', 'SP'),
(1463, 'DESCANSO', 'SC'),
(1464, 'DESCOBERTO', 'MG'),
(1465, 'DESTERRO', 'PB'),
(1466, 'DESTERRO DE ENTRE RIOS', 'MG'),
(1467, 'DESTERRO DO MELO', 'MG'),
(1468, 'DEZESSEIS DE NOVEMBRO', 'RS'),
(1469, 'DIADEMA', 'SP'),
(1470, 'DIAMANTE', 'PB'),
(1471, 'DIAMANTE DO NORTE', 'PR'),
(1472, 'DIAMANTE DO SUL', 'PR'),
(1473, 'DIAMANTE D''OESTE', 'PR'),
(1474, 'DIAMANTINA', 'MG'),
(1475, 'DIAMANTINO', 'MT'),
(1476, 'DIANÓPOLIS', 'TO'),
(1477, 'DIAS D''AVILA', 'BA'),
(1478, 'DILERMANDO DE AGUIAR', 'RS'),
(1479, 'DIOGO DE VASCONCELOS', 'MG'),
(1480, 'DIONISIO', 'MG'),
(1481, 'DIONISIO CERQUEIRA', 'SC'),
(1482, 'DIORAMA', 'GO'),
(1483, 'DIRCE REIS', 'SP'),
(1484, 'DIRCEU ARCOVERDE', 'PI'),
(1485, 'DIVIANÓPOLIS DE GOIÁS', 'GO'),
(1486, 'DIVINA PASTORA', 'SE'),
(1487, 'DIVINÉSIA', 'MG'),
(1488, 'DIVINO', 'MG'),
(1489, 'DIVINO DAS LARANJEIRAS', 'MG'),
(1490, 'DIVINO SÃO LOURENCO', 'ES'),
(1491, 'DIVINOLÂNDIA', 'SP'),
(1492, 'DIVINOLÂNDIA DE MINAS', 'MG'),
(1493, 'DIVINÓPOLIS', 'MG'),
(1494, 'DIVINÓPOLIS DO TOCANTINS', 'TO'),
(1495, 'DIVISA ALEGRE', 'MG'),
(1496, 'DIVISA NOVA', 'MG'),
(1497, 'DIVISÓPOLIS', 'MG'),
(1498, 'DOBRADA', 'SP'),
(1499, 'DOIS CORREGOS', 'SP'),
(1500, 'DOIS IRMAOS', 'RS'),
(1501, 'DOIS IRMÃOS DAS MISSÕES', 'RS'),
(1502, 'DOIS IRMÃOS DO BURITI', 'MS'),
(1503, 'DOIS IRMAOS DO TOCANTINS', 'TO'),
(1504, 'DOIS LAJEADOS', 'RS'),
(1505, 'DOIS RIACHOS', 'AL'),
(1506, 'DOIS VIZINHOS', 'PR'),
(1507, 'DOLCINÓPOLIS', 'SP'),
(1508, 'DOM AQUINO', 'MT'),
(1509, 'DOM BASILIO', 'BA'),
(1510, 'DOM BOSCO', 'MG'),
(1511, 'DOM CAVATI', 'MG'),
(1512, 'DOM ELISEU', 'PA'),
(1513, 'DOM EXPEDITO LOPES', 'PI'),
(1514, 'DOM FELICIANO', 'RS'),
(1515, 'DOM INOCÊNCIO', 'PI'),
(1516, 'DOM JOAQUIM', 'MG'),
(1517, 'DOM MACEDO COSTA', 'BA'),
(1518, 'DOM PEDRITO', 'RS'),
(1519, 'DOM PEDRO', 'MA'),
(1520, 'DOM PEDRO DE ALCANTARA', 'RS'),
(1521, 'DOM SILVERIO', 'MG'),
(1522, 'DOM VICOSO', 'MG'),
(1523, 'DOMINGOS MARTINS', 'ES'),
(1524, 'DOMINGOS MOURÃO', 'PI'),
(1525, 'DONA EMMA', 'SC'),
(1526, 'DONA EUZÉBIA', 'MG'),
(1527, 'DONA FRANCISCA', 'RS'),
(1528, 'DONA INES', 'PB'),
(1529, 'DORES DE CAMPOS', 'MG'),
(1530, 'DORES DE GUANHÃES', 'MG'),
(1531, 'DORES DO INDAIÁ', 'MG'),
(1532, 'DORES DO RIO PRETO', 'ES'),
(1533, 'DORES DO TURVO', 'MG'),
(1534, 'DORESÓPOLIS', 'MG'),
(1535, 'DORMENTES', 'PE'),
(1536, 'DOURADINA', 'PR'),
(1537, 'DOURADO', 'SP'),
(1538, 'DOURADOQUARA', 'MG'),
(1539, 'DOURADOS', 'MS'),
(1540, 'DOUTOR MAURICIO CARDOSO', 'RS'),
(1541, 'DOUTOR PEDRINHO', 'SC'),
(1542, 'DOUTOR RICARDO', 'RS'),
(1543, 'DOUTOR SEVERIANO', 'RN'),
(1544, 'DOUTOR ULYSSES', 'PR'),
(1545, 'DOVERLÂNDIA', 'GO'),
(1546, 'DRACENA', 'SP'),
(1547, 'DUARTINA', 'SP'),
(1548, 'DUAS BARRAS', 'RJ'),
(1549, 'DUAS ESTRADAS', 'PB'),
(1550, 'DUERE', 'TO'),
(1551, 'DUMONT', 'SP'),
(1552, 'DUQUE BACELAR', 'MA'),
(1553, 'DUQUE DE CAXIAS', 'RJ'),
(1554, 'DURANDE', 'MG'),
(1555, 'ECHAPORÃ', 'SP'),
(1556, 'ECOPORANGA', 'ES'),
(1557, 'EDEALINA', 'GO'),
(1558, 'EDEIA', 'GO'),
(1559, 'EIRUNEPE', 'AM'),
(1560, 'ELDORADO', 'MS'),
(1561, 'ELDORADO DO SUL', 'RS'),
(1562, 'ELDORADO DOS CARAJÁS', 'PA'),
(1563, 'ELESBAO VELOSO', 'PI'),
(1564, 'ELIAS FAUSTO', 'SP'),
(1565, 'ELISIARIO', 'SP'),
(1566, 'ELISIO MEBRADO', 'BA'),
(1567, 'ELISEU MARTINS', 'PI'),
(1568, 'ELOI MENDES', 'MG'),
(1569, 'EMAS', 'PB'),
(1570, 'EMBAUBA', 'SP'),
(1571, 'EMBU', 'SP'),
(1572, 'EMBU-GUAÇU', 'SP'),
(1573, 'EMILIANÓPOLIS', 'SP'),
(1574, 'ENCANTADO', 'RS'),
(1575, 'ENCANTO', 'RN'),
(1576, 'ENCRUZILHADA', 'BA'),
(1577, 'ENCRUZILHADA DO SUL', 'RS'),
(1578, 'ENÉAS MARQUES', 'PR'),
(1579, 'ENGENHEIRO PAULO FRONTIN', 'RJ'),
(1580, 'ENGENHEIRO BELTRÃO', 'PR'),
(1581, 'ENGENHEIRO CALDAS', 'MG'),
(1582, 'ENGENHEIRO COELHO', 'SP'),
(1583, 'ENGENHEIRO NAVARRO', 'MG'),
(1584, 'ENGENHO VELHO', 'RS'),
(1585, 'ENTRE FOLHAS', 'MG'),
(1586, 'ENTRE IJUIS', 'RS'),
(1587, 'ENTRE RIOS DE MINAS', 'MG'),
(1588, 'ENTRE RIOS DO OESTE', 'PR'),
(1589, 'ENTRE RIOS DO SUL', 'RS'),
(1590, 'ENTRE RIOS SUL', 'RS'),
(1591, 'ENVIRA', 'AM'),
(1592, 'EPITACIOLÂNDIA', 'AC'),
(1593, 'EQUADOR', 'RN'),
(1594, 'EREBANGO', 'RS'),
(1595, 'ERECHIM', 'RS'),
(1596, 'ERERE', 'CE'),
(1597, 'ERICO CARDOSO', 'BA'),
(1598, 'ERMO', 'SC'),
(1599, 'ERNESTINA', 'RS'),
(1600, 'ERVAL GRANDE', 'RS'),
(1601, 'ERVAL SECO', 'RS'),
(1602, 'ERVAL VELHO', 'SC'),
(1603, 'ERVALIA', 'MG'),
(1604, 'ESCADA', 'PE'),
(1605, 'ESMERALDA', 'RS'),
(1606, 'ESMERALDAS', 'MG'),
(1607, 'ESPERA FELIZ', 'MG'),
(1608, 'ESPERANÇA', 'PB'),
(1609, 'ESPERANÇA DO SUL', 'RS'),
(1610, 'ESPERANÇA NOVA', 'PR'),
(1611, 'ESPERANTINA', 'PI'),
(1612, 'ESPERANTINÓPOLIS', 'MA'),
(1613, 'ESPIGAO ALTO DO IGUAÇU', 'PR'),
(1614, 'ESPIGÃO DO OESTE', 'RO'),
(1615, 'ESPINOSA', 'MG'),
(1616, 'ESPIRITO SANTO', 'RN'),
(1617, 'ESPIRITO SANTO DO DOURADO', 'MG'),
(1618, 'ESPIRITO SANTO DO OESTE', 'RN'),
(1619, 'ESPIRITO SANTO DO PINHAL', 'SP'),
(1620, 'ESPIRITO SANTO DO TURVO', 'SP'),
(1621, 'ESPLANADA', 'BA'),
(1622, 'ESPUMOSO', 'RS'),
(1623, 'ESTACÃO', 'RS'),
(1624, 'ESTÂNCIA', 'SE'),
(1625, 'ESTÂNCIA CLIMÁTICA DE MORUNGABA', 'SP'),
(1626, 'ESTÂNCIA VELHA', 'RS'),
(1627, 'ESTEIO', 'RS'),
(1628, 'ESTIVA', 'MG'),
(1629, 'ESTIVA GERBI', 'SP'),
(1630, 'ESTREITO', 'MA'),
(1631, 'ESTRELA', 'RS'),
(1632, 'ESTRELA DALVA', 'MG'),
(1633, 'ESTRELA DE ALAGOAS', 'AL'),
(1634, 'ESTRELA DO INDAIÁ', 'MG'),
(1635, 'ESTRELA DO NORTE', 'GO'),
(1636, 'ESTRELA DO SUL', 'MG'),
(1637, 'ESTRELA DO OESTE', 'SP'),
(1638, 'ESTRELA VELHA', 'RS'),
(1639, 'EUCLIDES DA CUNHA PAULISTA', 'SP'),
(1640, 'EUCLIDES DA CUNHA', 'BA'),
(1641, 'EUGENIO DE CASTRO', 'RS'),
(1642, 'EUGENÓPOLIS', 'MG'),
(1643, 'EUNÁPOLIS', 'BA'),
(1644, 'EUSEBIO', 'CE'),
(1645, 'EWBANK DA CAMARA', 'MG'),
(1646, 'EXTREMA', 'MG'),
(1647, 'EXTREMOZ', 'RN'),
(1648, 'EXU', 'PE'),
(1649, 'FAGUNDES', 'PB'),
(1650, 'FAGUNDES VARELA', 'RS'),
(1651, 'FAINA', 'GO'),
(1652, 'FAMA', 'MG'),
(1653, 'FARIA LEMOS', 'MG'),
(1654, 'FARIAS BRITO', 'CE'),
(1655, 'FARO', 'PA'),
(1656, 'FAROL', 'PR'),
(1657, 'FARROUPILHA', 'RS'),
(1658, 'FARTURA', 'SP'),
(1659, 'FARTURA DO PIAUÍ', 'PI'),
(1660, 'FÁTIMA', 'BA'),
(1661, 'FÁTIMA DO SUL', 'MS'),
(1662, 'FAXINAL', 'PR'),
(1663, 'FAXINAL DO SOTURNO', 'RS'),
(1664, 'FAXINAL DOS GUEDES', 'SC'),
(1665, 'FAXINALZINHO', 'RS'),
(1666, 'FAZENDA NOVA', 'GO'),
(1667, 'FAZENDA RIO GRANDE', 'PR'),
(1668, 'FAZENDA VILA NOVA', 'RS'),
(1669, 'FEIJÓ', 'AC'),
(1670, 'FEIRA DA MATA', 'BA'),
(1671, 'FEIRA DE SANTANA', 'BA'),
(1672, 'FEIRA GRANDE', 'AL'),
(1673, 'FEIRA NOVA', 'SE'),
(1674, 'FEIRA NOVA DO MARANHÃO', 'MA'),
(1675, 'FELICIO DOS SANTOS', 'MG'),
(1676, 'FELIPE GUERRA', 'RN'),
(1677, 'FELIZBURGO', 'MG'),
(1678, 'FELIXLÂNDIA', 'MG'),
(1679, 'FELIZ', 'RS'),
(1680, 'FELIZ DESERTO', 'AL'),
(1681, 'FELIZ NATAL', 'MT'),
(1682, 'FÊNIX', 'PR'),
(1683, 'FERNANDES TOURINHO', 'MG'),
(1684, 'FERNANDO FALCÃO', 'MA'),
(1685, 'FERNANDO PEDROZA', 'RN'),
(1686, 'FERNANDO PRESTES', 'SP'),
(1687, 'FERNANDÓPOLIS', 'SP'),
(1688, 'FERNÃO', 'SP'),
(1689, 'FERRAZ DE VASCONCELOS', 'SP'),
(1690, 'FERREIRA GOMES', 'AP'),
(1691, 'FERREIROS', 'PE'),
(1692, 'FERROS', 'MG'),
(1693, 'FERVEDOURO', 'MG'),
(1694, 'FIGUEIRA', 'PR'),
(1695, 'FIGUEIRÓPOLIS', 'TO'),
(1696, 'FIGUEIRÓPOLIS DO OESTE', 'MT'),
(1697, 'FILADELFIA', 'BA'),
(1698, 'FIRMINO ALVES', 'BA'),
(1699, 'FIRMINÓPOLIS', 'GO'),
(1700, 'FLEXEIRAS', 'AL'),
(1701, 'FLOR DA SERRA DO SUL', 'PR'),
(1702, 'FLOR DO SERTÃO', 'SC'),
(1703, 'FLORA RICA', 'SP'),
(1704, 'FLORAÍ', 'PR'),
(1705, 'FLORANIA', 'RN'),
(1706, 'FLOREAL', 'SP'),
(1707, 'FLORES', 'PE'),
(1708, 'FLORES DA CUNHA', 'RS'),
(1709, 'FLORES DE GOIÁS', 'GO'),
(1710, 'FLORES DO PIAUÍ', 'PI'),
(1711, 'FLORESTA', 'PE'),
(1712, 'FLORESTA', 'PR'),
(1713, 'FLORESTA AZUL', 'BA'),
(1714, 'FLORESTA DO ARAGUAIA', 'PA'),
(1715, 'FLORESTA DO PIAUÍ', 'PI'),
(1716, 'FLORESTAL', 'MG'),
(1717, 'FLORESTÓPOLIS', 'PR'),
(1718, 'FLORIANO', 'PI'),
(1719, 'FLORIANO PEIXOTO', 'RS'),
(1720, 'FLORIANÓPOLIS', 'SC'),
(1721, 'FLORIDA', 'PR'),
(1722, 'FLORIDA PAULISTA', 'SP'),
(1723, 'FLORÍNEA', 'SP'),
(1724, 'FONTE BOA', 'AM'),
(1725, 'FONTOURA XAVIER', 'RS'),
(1726, 'FORMIGA', 'MG'),
(1727, 'FORMIGUEIRO', 'RS'),
(1728, 'FORMOSA', 'GO'),
(1729, 'FORMOSA DA SERRA NEGRA', 'MA'),
(1730, 'FORMOSA DO OESTE', 'PR'),
(1731, 'FORMOSA DO RIO PRETO', 'BA'),
(1732, 'FORMOSA DO SUL', 'SC'),
(1733, 'FORMOSO', 'GO'),
(1734, 'FORMOSO DO ARAGUAIA', 'TO'),
(1735, 'FORQUILHA', 'CE'),
(1736, 'FORQUILHINHA', 'SC'),
(1737, 'FORTALEZA', 'CE'),
(1738, 'FORTALEZA DE MINAS', 'MG'),
(1739, 'FORTALEZA DO TABOCÃO', 'TO'),
(1740, 'FORTALEZA DOS NOGUEIRAS', 'MA'),
(1741, 'FORTALEZA DOS VALOS', 'RS'),
(1742, 'FORTIM', 'CE'),
(1743, 'FORTUNA', 'MA'),
(1744, 'FORTUNA DE MINAS', 'MG'),
(1745, 'FOZ DO IGUAÇU', 'PR'),
(1746, 'FOZ DO JORDÃO', 'PR'),
(1747, 'FRAIBURGO', 'SC'),
(1748, 'FRANCA', 'SP'),
(1749, 'FRANCINÓPOLIS', 'PI'),
(1750, 'FRANCISCO ALVES', 'PR'),
(1751, 'FRANCISCO AYRES', 'PI'),
(1752, 'FRANCISCO BADARÓ', 'MG'),
(1753, 'FRANCISCO BELTRÃO', 'PR'),
(1754, 'FRANCISCO DANTAS', 'RN'),
(1755, 'FRANCISCO DUMONT', 'MG'),
(1756, 'FRANCISCO MACEDO', 'PI'),
(1757, 'FRANCISCO MORATO', 'SP'),
(1758, 'FRANCISCO SÁ', 'MG'),
(1759, 'FRANCISCO SANTOS', 'PI'),
(1760, 'FRANCISCÓPOLIS', 'MG'),
(1761, 'FRANCO DA ROCHA', 'SP'),
(1762, 'FRECHEIRINHA', 'CE'),
(1763, 'FREDERICO WESTPHALEN', 'RS'),
(1764, 'FREI GASPAR', 'MG'),
(1765, 'FREI INOCENCIO', 'MG'),
(1766, 'FREI LAGO NEGRO', 'MG'),
(1767, 'FREI MARTINHO', 'PB'),
(1768, 'FREI MIGUELINHO', 'PE'),
(1769, 'FREI PAULO', 'SE'),
(1770, 'FRONTEIRA', 'MG'),
(1771, 'FRONTEIRA DOS VALES', 'MG'),
(1772, 'FRONTEIRAS', 'PI'),
(1773, 'FRUTAL', 'MG'),
(1774, 'FRUTUOSO GOMES', 'RN'),
(1775, 'FUNDÃO', 'ES'),
(1776, 'FUNILÂNDIA', 'MG'),
(1777, 'GABRIEL MONTEIRO', 'SP'),
(1778, 'GADO BRAVO', 'PB'),
(1779, 'GALIA', 'SP'),
(1780, 'GALILEIA', 'MG'),
(1781, 'GALINHOS', 'RN'),
(1782, 'GALVÃO', 'SC'),
(1783, 'GAMELEIRA', 'PE'),
(1784, 'GAMELEIRAS', 'MG'),
(1785, 'GANBU', 'BA'),
(1786, 'GARANHUNS', 'PE'),
(1787, 'GARARU', 'SE'),
(1788, 'GARÇA', 'SP'),
(1789, 'GARIBALDI', 'RS'),
(1790, 'GAROPABA', 'SC'),
(1791, 'GARRAFÃO DO NORTE', 'PA'),
(1792, 'GARRUCHOS', 'RS'),
(1793, 'GARUVA', 'SC'),
(1794, 'GASPAR', 'SC'),
(1795, 'GASTÃO VIDIGAL', 'SP'),
(1796, 'GAÚCHA DO NORTE', 'MT'),
(1797, 'GAURAMA', 'RS'),
(1798, 'GAVIÃO', 'BA'),
(1799, 'GAVIÃO PEIXOTO', 'SP'),
(1800, 'GEMINIANO', 'PI'),
(1801, 'GENERAL CAMARA', 'RS'),
(1802, 'GENERAL CARNEIRO', 'MT'),
(1803, 'GENERAL MAYNARD', 'SE'),
(1804, 'GENERAL SALGADO', 'SP'),
(1805, 'GENERAL SAMPAIO', 'CE'),
(1806, 'GENTIL', 'RS'),
(1807, 'GENTIO DO OURO', 'BA'),
(1808, 'GETULINA', 'SP'),
(1809, 'GETULIO VARGAS', 'RS'),
(1810, 'GILBUES', 'PI'),
(1811, 'GIRAU DO PONCIANO', 'AL'),
(1812, 'GIRUA', 'RS'),
(1813, 'GLAUCILÂNDIA', 'MG'),
(1814, 'GLICERIO', 'SP'),
(1815, 'GLÓRIA', 'BA'),
(1816, 'GLÓRIA DE DOURADOS', 'MS'),
(1817, 'GLÓRIA DO GOITA', 'PE'),
(1818, 'GLÓRIA D''OESTE', 'MT'),
(1819, 'GLORINHA', 'RS'),
(1820, 'GODOFREDO VIANA', 'MA'),
(1821, 'GODOY MOREIRA', 'PR'),
(1822, 'GOIANÁ', 'MG'),
(1823, 'GOIANÁPOLIS', 'GO'),
(1824, 'GOIANDIRA', 'GO'),
(1825, 'GOIANESIA', 'GO'),
(1826, 'GOIANÉSIA DO PARÁ', 'PA'),
(1827, 'GOIANIA', 'GO'),
(1828, 'GOIANINHA', 'RN'),
(1829, 'GOIANIRA', 'GO'),
(1830, 'GOIANORTE', 'TO'),
(1831, 'GOIÁS', 'GO'),
(1832, 'GOIATINS', 'TO'),
(1833, 'GOIATUBA', 'GO'),
(1834, 'GOIOERE', 'PR'),
(1835, 'GOIOXIM', 'PR'),
(1836, 'GONÇALVES', 'MG'),
(1837, 'GONÇALVES DIAS', 'MA'),
(1838, 'GONGOGI', 'BA'),
(1839, 'GONZAGA', 'MG'),
(1840, 'GOUVEIA', 'MG'),
(1841, 'GOUVELÂNDIA', 'GO'),
(1842, 'GOVERNADOR DIX-SEPT ROSADO', 'RN'),
(1843, 'GOVERNADOR ARCHER', 'MA');
INSERT INTO `municipios` (`codigo`, `nome`, `uf`) VALUES
(1844, 'GOVERNADOR CELSO RAMOS', 'SC'),
(1845, 'GOVERNADOR EDSON LOBAO', 'MA'),
(1846, 'GOVERNADOR EUGENIO BARROS', 'MA'),
(1847, 'GOVERNADOR LOMANTO JUNIOR', 'BA'),
(1848, 'GOVERNADOR LUIS ROCHA', 'MA'),
(1849, 'GOVERNADOR MANGABEIRA', 'BA'),
(1850, 'GOVERNADOR NEWTON BELO', 'MA'),
(1851, 'GOVERNADOR NUNES FREIRE', 'MA'),
(1852, 'GOVERNADOR VALADARES', 'MG'),
(1853, 'GRAÇA', 'CE'),
(1854, 'GRAÇA ARANHA', 'MA'),
(1855, 'GRACCHO CARDOSO', 'SE'),
(1856, 'GRAJAU', 'MA'),
(1857, 'GRAMADO', 'RS'),
(1858, 'GRAMADO DOS LOUREIROS', 'RS'),
(1859, 'GRAMADO XAVIER', 'RS'),
(1860, 'GRANDES RIOS', 'PR'),
(1861, 'GRANITO', 'PE'),
(1862, 'GRANJA', 'CE'),
(1863, 'GRANJEIRO', 'CE'),
(1864, 'GRÃO MOGOL', 'MG'),
(1865, 'GRÃO-PARA', 'SC'),
(1866, 'GRAVATA', 'PE'),
(1867, 'GRAVATAÍ', 'RS'),
(1868, 'GRAVATAL', 'SC'),
(1869, 'GROAIRAS', 'CE'),
(1870, 'GROSSOS', 'RN'),
(1871, 'GRUPIARA', 'MG'),
(1872, 'GUABIJU', 'RS'),
(1873, 'GUABIRUBA', 'SC'),
(1874, 'GUAÇUÍ', 'ES'),
(1875, 'GUADALUPE', 'PI'),
(1876, 'GUAIBA', 'RS'),
(1877, 'GUAIÇARA', 'SP'),
(1878, 'GUAIMBÉ', 'SP'),
(1879, 'GUAIRA', 'PR'),
(1880, 'GUAIRAÇÁ', 'PR'),
(1881, 'GUAIUBA', 'CE'),
(1882, 'GUAJARA', 'AC'),
(1883, 'GUAJARA-MIRIM', 'RO'),
(1884, 'GUAJERU', 'BA'),
(1885, 'GUAMARE', 'RN'),
(1886, 'GUAMIRANGA', 'PR'),
(1887, 'GUANANBI', 'BA'),
(1888, 'GUANHÃES', 'MG'),
(1889, 'GUAPE', 'MG'),
(1890, 'GUAPIAÇU', 'SP'),
(1891, 'GUAPIARA', 'SP'),
(1892, 'GUAPIMIRIM', 'RJ'),
(1893, 'GUAPIRAMA', 'PR'),
(1894, 'GUAPÓ', 'GO'),
(1895, 'GUAPORE', 'RS'),
(1896, 'GUAPOREMA', 'PR'),
(1897, 'GUARA', 'SP'),
(1898, 'GUARABIRA', 'PB'),
(1899, 'GUARAÇAÍ', 'SP'),
(1900, 'GUARACI', 'PR'),
(1901, 'GUARACIABA', 'MG'),
(1902, 'GUARACIABA DO NORTE', 'CE'),
(1903, 'GUARACIAMA', 'MG'),
(1904, 'GUARAI', 'TO'),
(1905, 'GUARAÍTA', 'GO'),
(1906, 'GUARAMIRANGA', 'CE'),
(1907, 'GUARAMIRIM', 'SC'),
(1908, 'GUARANESIA', 'MG'),
(1909, 'GUARANI', 'MG'),
(1910, 'GUARANI DAS MISSÕES', 'RS'),
(1911, 'GUARANI DE GOIÁS', 'GO'),
(1912, 'GUARANI DO OESTE', 'SP'),
(1913, 'GUARANIAÇU', 'PR'),
(1914, 'GUARANTÃ', 'SP'),
(1915, 'GUARANTA DO NORTE', 'MT'),
(1916, 'GUARAPARI', 'ES'),
(1917, 'GUARAPUAVA', 'PR'),
(1918, 'GUARAQUECABA', 'PR'),
(1919, 'GUARARA', 'MG'),
(1920, 'GUARARAPES', 'SP'),
(1921, 'GUARAREMA', 'SP'),
(1922, 'GUARATINGA', 'BA'),
(1923, 'GUARATINGUETA', 'SP'),
(1924, 'GUARATUBA', 'PR'),
(1925, 'GUARDA-MOR', 'MG'),
(1926, 'GUAREI', 'SP'),
(1927, 'GUARIBA', 'SP'),
(1928, 'GUARIBAS', 'PI'),
(1929, 'GUARINOS', 'GO'),
(1930, 'GUARUJA', 'SP'),
(1931, 'GUARUJA DO SUL', 'SC'),
(1932, 'GUARULHOS', 'SP'),
(1933, 'GUATAMBU', 'SC'),
(1934, 'GUATAPARÁ', 'SP'),
(1935, 'GUAXUPE', 'MG'),
(1936, 'GUIA LOPES DA LAGUNA', 'MS'),
(1937, 'GUIDOVAL', 'MG'),
(1938, 'GUIMARAES', 'MA'),
(1939, 'GUIMARANIA', 'MG'),
(1940, 'GUIRATINGA', 'MT'),
(1941, 'GUIRICEMA', 'MG'),
(1942, 'GURINHATA', 'MG'),
(1943, 'GURINHÉM', 'PB'),
(1944, 'GURJÃO', 'PB'),
(1945, 'GURUPA', 'PA'),
(1946, 'GURUPI', 'TO'),
(1947, 'GUZOLÂNDIA', 'SP'),
(1948, 'HARMONIA', 'RS'),
(1949, 'HEITORAI', 'GO'),
(1950, 'HELIODORA', 'MG'),
(1951, 'HELIÓPOLIS', 'BA'),
(1952, 'HERCULÂNDIA', 'SP'),
(1953, 'HERVAL', 'RS'),
(1954, 'HERVAL D''OESTE', 'SC'),
(1955, 'HERVEIRAS', 'RS'),
(1956, 'HIDROLÂNDIA', 'CE'),
(1957, 'HIDROLINA', 'GO'),
(1958, 'HOLAMBRA', 'SP'),
(1959, 'HONORIO SERPA', 'PR'),
(1960, 'HORIZONTE', 'CE'),
(1961, 'HORIZONTINA', 'RS'),
(1962, 'HORTOLÂNDIA', 'SP'),
(1963, 'HUGO NAPOLEÃO', 'PI'),
(1964, 'HULHA NEGRA', 'RS'),
(1965, 'HUMAITA', 'AM'),
(1966, 'HUMBERTO DE CAMPOS', 'MA'),
(1967, 'IACANGA', 'SP'),
(1968, 'IACIARA', 'GO'),
(1969, 'IACRI', 'SP'),
(1970, 'IAÇU', 'BA'),
(1971, 'IAPU', 'MG'),
(1972, 'IARAS', 'SP'),
(1973, 'IATI', 'PE'),
(1974, 'IBAITI', 'PR'),
(1975, 'IBARAMA', 'RS'),
(1976, 'IBARETAMA', 'CE'),
(1977, 'IBATE', 'SP'),
(1978, 'IBATEGUARA', 'AL'),
(1979, 'IBATIBA', 'ES'),
(1980, 'IBEMA', 'PR'),
(1981, 'IBERTIOGA', 'MG'),
(1982, 'IBIA', 'MG'),
(1983, 'IBIACA', 'RS'),
(1984, 'IBIAI', 'MG'),
(1985, 'IBIAM', 'SC'),
(1986, 'IBIAPINA', 'CE'),
(1987, 'IBIARA', 'PB'),
(1988, 'IBIASSUCE', 'BA'),
(1989, 'IBICARAI', 'BA'),
(1990, 'IBICARE', 'SC'),
(1991, 'IBICOARA', 'BA'),
(1992, 'IBICUI', 'BA'),
(1993, 'IBICUTINGA', 'CE'),
(1994, 'IBIMIRIM', 'PE'),
(1995, 'IBIPEBA', 'BA'),
(1996, 'IBIPITANGA', 'BA'),
(1997, 'IBIPORÃ', 'PR'),
(1998, 'IBIQUERA', 'BA'),
(1999, 'IBIRA', 'SP'),
(2000, 'IBIRACATU', 'MG'),
(2001, 'IBIRACI', 'MG'),
(2002, 'IBIRAÇU', 'ES'),
(2003, 'IBIRAIARAS', 'RS'),
(2004, 'IBIRAJUBA', 'PE'),
(2005, 'IBIRAMA', 'SC'),
(2006, 'IBIRAPITANGA', 'BA'),
(2007, 'IBIRAPUÃ', 'BA'),
(2008, 'IBIRAPUITA', 'RS'),
(2009, 'IBIRAREMA', 'SP'),
(2010, 'IBIRATAIA', 'BA'),
(2011, 'IBIRITE', 'MG'),
(2012, 'IBIRUBA', 'RS'),
(2013, 'IBITIARA', 'BA'),
(2014, 'IBITIBA', 'BA'),
(2015, 'IBITINGA', 'SP'),
(2016, 'IBITIRAMA', 'ES'),
(2017, 'IBITIURA', 'MG'),
(2018, 'IBITIÚRA DE MINAS', 'MG'),
(2019, 'IBITURUNA', 'MG'),
(2020, 'IBIUNA', 'SP'),
(2021, 'ICAPUI', 'CE'),
(2022, 'ICARA', 'PA'),
(2023, 'ICARAI DE MINAS', 'MG'),
(2024, 'ICARAIMA', 'PR'),
(2025, 'ICATU', 'MA'),
(2026, 'ICÉM', 'SP'),
(2027, 'ICHU', 'BA'),
(2028, 'ICO', 'CE'),
(2029, 'ICONHA', 'ES'),
(2030, 'IELMO MARINHO', 'RN'),
(2031, 'IEPE', 'SP'),
(2032, 'IGAPORA', 'BA'),
(2033, 'IGARAÇU DO TIETE', 'SP'),
(2034, 'IGARACY', 'PB'),
(2035, 'IGARAPAVA', 'SP'),
(2036, 'IGARAPÉ DO MEIO', 'MA'),
(2037, 'IGARAPE GRANDE', 'MA'),
(2038, 'IGARAPÉ-AÇU', 'PA'),
(2039, 'IGARAPE-MIRIM', 'PA'),
(2040, 'IGARASSU', 'PE'),
(2041, 'IGARATÁ', 'SP'),
(2042, 'IGARATINGA', 'MG'),
(2043, 'IGREJA NOVA', 'AL'),
(2044, 'IGREJINHA', 'RS'),
(2045, 'IGUABA GRANDE', 'RJ'),
(2046, 'IGUACI', 'AL'),
(2047, 'IGUAI', 'BA'),
(2048, 'IGUAPE', 'SP'),
(2049, 'IGUARACI', 'PE'),
(2050, 'IGUARAÇU', 'PR'),
(2051, 'IGUATAMA', 'MG'),
(2052, 'IGUATEMI', 'MS'),
(2053, 'IGUATU', 'CE'),
(2054, 'IJACI', 'MG'),
(2055, 'IJUI', 'RS'),
(2056, 'ILHA COMPRIDA', 'SP'),
(2057, 'ILHA DE ITAMARACÁ', 'PE'),
(2058, 'ILHA GRANDE', 'PI'),
(2059, 'ILHA SOLTEIRA', 'SP'),
(2060, 'ILHABELA', 'SP'),
(2061, 'ILHA DAS FLORES', 'SE'),
(2062, 'ILHEUS', 'BA'),
(2063, 'ILHOTA', 'SC'),
(2064, 'ILICINEA', 'MG'),
(2065, 'ILÓPOLIS', 'RS'),
(2066, 'IMACULADA', 'PB'),
(2067, 'IMARUI', 'SC'),
(2068, 'IMBAU', 'PR'),
(2069, 'IMBÉ', 'RS'),
(2070, 'IMBITUBA', 'SC'),
(2071, 'IMBITUVA', 'PR'),
(2072, 'IMBUIA', 'SC'),
(2073, 'IMIGRANTE', 'RS'),
(2074, 'IMPERATRIZ', 'MA'),
(2075, 'INACIO MARTINS', 'PR'),
(2076, 'INACIOLÂNDIA', 'GO'),
(2077, 'INAJÁ', 'PR'),
(2078, 'INAJÁ', 'PE'),
(2079, 'INCONFIDENTES', 'MG'),
(2080, 'INDAIAL', 'SC'),
(2081, 'INDAIATUBA', 'SP'),
(2082, 'INDAJAVIRA', 'MG'),
(2083, 'INDEPENDÊNCIA', 'CE'),
(2084, 'INDIANA', 'SP'),
(2085, 'INDIANÓPOLIS', 'PR'),
(2086, 'INDIAPORA', 'SP'),
(2087, 'INDIARA', 'GO'),
(2088, 'INDIAROBA', 'SE'),
(2089, 'INDIAVAI', 'MT'),
(2090, 'INGA', 'PB'),
(2091, 'INGAI', 'MG'),
(2092, 'INGAZEIRA', 'PE'),
(2093, 'INHACORA', 'RS'),
(2094, 'INHAMBUPE', 'BA'),
(2095, 'INHANGAPI', 'PA'),
(2096, 'INHAPI', 'AL'),
(2097, 'INHAPIM', 'MG'),
(2098, 'INHAUMA', 'MG'),
(2099, 'INHUMA', 'PI'),
(2100, 'INHUMAS', 'GO'),
(2101, 'INIMUTABA', 'MG'),
(2102, 'INOCÊNCIA', 'MS'),
(2103, 'INUBIA PAULISTA', 'SP'),
(2104, 'IOMERE', 'SC'),
(2105, 'IPABA', 'MG'),
(2106, 'IPAMERI', 'GO'),
(2107, 'IPANEMA', 'MG'),
(2108, 'IPANGUAÇU', 'RN'),
(2109, 'IPAPORANGA', 'CE'),
(2110, 'IPATINGA', 'MG'),
(2111, 'IPAUMIRIM', 'CE'),
(2112, 'IPAUSSU', 'SP'),
(2113, 'IPÊ', 'RS'),
(2114, 'IPECAETA', 'BA'),
(2115, 'IPERÓ', 'SP'),
(2116, 'IPEUNA', 'SP'),
(2117, 'IPIAÇU', 'MG'),
(2118, 'IPIAU', 'BA'),
(2119, 'IPIGUA', 'SP'),
(2120, 'IPIRÁ', 'BA'),
(2121, 'IPIRANGA', 'PR'),
(2122, 'IPIRANGA DO PIAUÍ', 'PI'),
(2123, 'IPIRANGA DO SUL', 'RS'),
(2124, 'IPIXUNA', 'AM'),
(2125, 'IPIXUNA DO PARÁ', 'PA'),
(2126, 'IPOJUCA', 'PE'),
(2127, 'IPORA', 'GO'),
(2128, 'IPORA DO OESTE', 'SC'),
(2129, 'IPORANGA', 'SP'),
(2130, 'IPU', 'CE'),
(2131, 'IPUA', 'SP'),
(2132, 'IPUAÇU', 'SC'),
(2133, 'IPUBI', 'PE'),
(2134, 'IPUEIRA', 'RN'),
(2135, 'IPUEIRAS', 'CE'),
(2136, 'IPUIUNA', 'MG'),
(2137, 'IPUMIRIM', 'SC'),
(2138, 'IPUPIARA', 'BA'),
(2139, 'IRACEMA', 'CE'),
(2140, 'IRACEMA DO OESTE', 'PR'),
(2141, 'IRACEMAPOLIS', 'SP'),
(2142, 'IRACEMINHA', 'SC'),
(2143, 'IRAÍ', 'RS'),
(2144, 'IRAÍ DE MINAS', 'MG'),
(2145, 'IRAJUBA', 'BA'),
(2146, 'IRAMAIA', 'BA'),
(2147, 'IRAMUTA', 'RR'),
(2148, 'IRANDUBA', 'AM'),
(2149, 'IRANI', 'SC'),
(2150, 'IRAPUA', 'SP'),
(2151, 'IRAPURU', 'SP'),
(2152, 'IRARA', 'BA'),
(2153, 'IRATI', 'PR'),
(2154, 'IRAUCUBA', 'CE'),
(2155, 'IRECE', 'BA'),
(2156, 'IRETAMA', 'PR'),
(2157, 'IRINEÓPOLIS', 'SC'),
(2158, 'IRITUIA', 'PA'),
(2159, 'IRUPI', 'ES'),
(2160, 'ISAIAS COELHO', 'PI'),
(2161, 'ISRAELÂNDIA', 'GO'),
(2162, 'ITA', 'SC'),
(2163, 'ITAARA', 'RS'),
(2164, 'ITABAIANA', 'PB'),
(2165, 'ITABAIANA', 'SE'),
(2166, 'ITABAIANINHA', 'SE'),
(2167, 'ITABELA', 'BA'),
(2168, 'ITABERA', 'SP'),
(2169, 'ITABERABA', 'BA'),
(2170, 'ITABERAI', 'GO'),
(2171, 'ITABI', 'SE'),
(2172, 'ITABIRA', 'MG'),
(2173, 'ITABIRINHA DE MANTENA', 'MG'),
(2174, 'ITABIRITO', 'MG'),
(2175, 'ITABORAÍ', 'RJ'),
(2176, 'ITABUNA', 'BA'),
(2177, 'ITACAJA', 'TO'),
(2178, 'ITACAMBIRA', 'MG'),
(2179, 'ITACARAMBI', 'MG'),
(2180, 'ITACARÉ', 'BA'),
(2181, 'ITACOATIARA', 'AM'),
(2182, 'ITACURUBA', 'PE'),
(2183, 'ITAÇURUBI', 'RS'),
(2184, 'ITAETE', 'BA'),
(2185, 'ITAGI', 'BA'),
(2186, 'ITAGIBA', 'BA'),
(2187, 'ITAGIMIRIM', 'BA'),
(2188, 'ITAGUAÇU', 'ES'),
(2189, 'ITÁGUAÇU DA BAHIA', 'BA'),
(2190, 'ITAGUAÍ', 'RJ'),
(2191, 'ITAGUAJÉ', 'PR'),
(2192, 'ITÁGUARA', 'MG'),
(2193, 'ITAGUARI', 'GO'),
(2194, 'ITÁGUARU', 'GO'),
(2195, 'ITÁGUATINS', 'TO'),
(2196, 'ITAI', 'SP'),
(2197, 'ITAÍBA', 'PE'),
(2198, 'ITAICABA', 'CE'),
(2199, 'ITAINÓPOLIS', 'PI'),
(2200, 'ITAIÓPOLIS', 'SC'),
(2201, 'ITAIPAVA DO GRAJAU', 'MA'),
(2202, 'ITAIPÉ', 'MG'),
(2203, 'ITAIPULÂNDIA', 'PR'),
(2204, 'ITAITINGA', 'CE'),
(2205, 'ITAITUBA', 'PA'),
(2206, 'ITAJA', 'GO'),
(2207, 'ITAJAI', 'SC'),
(2208, 'ITAJOBI', 'SP'),
(2209, 'ITAJU', 'SP'),
(2210, 'ITAJU DO COLONIA', 'BA'),
(2211, 'ITAJUBA', 'MG'),
(2212, 'ITAJUIPE', 'BA'),
(2213, 'ITALVA', 'RJ'),
(2214, 'ITAMAGNA', 'BA'),
(2215, 'ITAMARAÇA', 'PE'),
(2216, 'ITAMARAJU', 'BA'),
(2217, 'ITAMARANDIBA', 'MG'),
(2218, 'ITAMARATI', 'AM'),
(2219, 'ITAMARATI DE MINAS', 'MG'),
(2220, 'ITAMARI', 'BA'),
(2221, 'ITAMBAÇURI', 'MG'),
(2222, 'ITAMBARAÇA', 'PR'),
(2223, 'ITAMBÉ', 'BA'),
(2224, 'ITAMBE DO MATO DENTRO', 'MG'),
(2225, 'ITAMOGI', 'MG'),
(2226, 'ITAMONTE', 'MG'),
(2227, 'ITANAGRA', 'BA'),
(2228, 'ITANHAEM', 'SP'),
(2229, 'ITANHANDU', 'MG'),
(2230, 'ITANHÉM', 'BA'),
(2231, 'ITANHOMI', 'MG'),
(2232, 'ITAOBIM', 'MG'),
(2233, 'ITAÓCA', 'SP'),
(2234, 'ITAPACI', 'GO'),
(2235, 'ITAPAGIPE', 'MG'),
(2236, 'ITAPAJE', 'CE'),
(2237, 'ITAPARICA', 'BA'),
(2238, 'ITAPÉ', 'BA'),
(2239, 'ITAPEBI', 'BA'),
(2240, 'ITAPECERICA', 'MG'),
(2241, 'ITAPECERICA DA SERRA', 'SP'),
(2242, 'ITAPECURU-MIRIM', 'MA'),
(2243, 'ITAPEJARA DO OESTE', 'PR'),
(2244, 'ITAPEMA', 'SC'),
(2245, 'ITAPEMIRIM', 'ES'),
(2246, 'ITAPERUCU', 'PR'),
(2247, 'ITAPERUNA', 'RJ'),
(2248, 'ITAPETIM', 'PE'),
(2249, 'ITAPETINGA', 'BA'),
(2250, 'ITAPETININGA', 'SP'),
(2251, 'ITAPEVA', 'MG'),
(2252, 'ITAPEVA', 'SP'),
(2253, 'ITAPEVI', 'SP'),
(2254, 'ITAPICURU', 'BA'),
(2255, 'ITAPIPOCA', 'CE'),
(2256, 'ITAPIRA', 'SP'),
(2257, 'ITAPIRANGA', 'AM'),
(2258, 'ITAPIRAPUA', 'GO'),
(2259, 'ITAPIRAPUÃ PAULISTA', 'SP'),
(2260, 'ITAPIRATINS', 'TO'),
(2261, 'ITAPISSUMA', 'PE'),
(2262, 'ITAPITANGA', 'BA'),
(2263, 'ITAPIUNA', 'CE'),
(2264, 'ITAPOÃ', 'SC'),
(2265, 'ITAPOA DO OESTE', 'RO'),
(2266, 'ITÁPOLIS', 'SP'),
(2267, 'ITAPORÃ', 'MS'),
(2268, 'ITAPORA DO TOCANTINS', 'TO'),
(2269, 'ITAPORANGA', 'PB'),
(2270, 'ITAPORANGA', 'SP'),
(2271, 'ITAPORANGA D''AJUDA', 'SE'),
(2272, 'ITAPOROROCA', 'PB'),
(2273, 'ITAPUCA', 'RS'),
(2274, 'ITAPUI', 'SP'),
(2275, 'ITAPURA', 'SP'),
(2276, 'ITAPURANGA', 'GO'),
(2277, 'ITAQUAQUECETUBA', 'SP'),
(2278, 'ITAQUARA', 'BA'),
(2279, 'ITAQUI', 'RS'),
(2280, 'ITAQUIRAÍ', 'MS'),
(2281, 'ITAQUITINGA', 'PE'),
(2282, 'ITARANA', 'ES'),
(2283, 'ITARANTIM', 'BA'),
(2284, 'ITARARE', 'SP'),
(2285, 'ITAREMA', 'CE'),
(2286, 'ITARIRI', 'SP'),
(2287, 'ITARUMA', 'GO'),
(2288, 'ITATI', 'PR'),
(2289, 'ITATIAIA', 'RJ'),
(2290, 'ITATIAIUCU', 'MG'),
(2291, 'ITATIBA', 'SP'),
(2292, 'ITATIBA DO SUL', 'RS'),
(2293, 'ITATIM', 'BA'),
(2294, 'ITATINGA', 'SP'),
(2295, 'ITATIRA', 'CE'),
(2296, 'ITATUBA', 'PB'),
(2297, 'ITAU', 'RN'),
(2298, 'ITAU DE MINAS', 'MG'),
(2299, 'ITAÚBA', 'MT'),
(2300, 'ITAUBAL', 'AP'),
(2301, 'ITAUCU', 'GO'),
(2302, 'ITAUEIRA', 'PI'),
(2303, 'ITAUNA', 'MG'),
(2304, 'ITAÚNA DO SUL', 'PR'),
(2305, 'ITAVERAVA', 'MG'),
(2306, 'ITINGA', 'MG'),
(2307, 'ITINGA DO MARANHÃO', 'MA'),
(2308, 'ITIQUIRA', 'MT'),
(2309, 'ITIRAPINA', 'SP'),
(2310, 'ITIRAPUA', 'SP'),
(2311, 'ITIRUÇU', 'BA'),
(2312, 'ITIÚBA', 'BA'),
(2313, 'ITOBI', 'SP'),
(2314, 'ITORORO', 'BA'),
(2315, 'ITU', 'SP'),
(2316, 'ITUAÇU', 'BA'),
(2317, 'ITUBERA', 'BA'),
(2318, 'ITUETA', 'MG'),
(2319, 'ITUIUTABA', 'MG'),
(2320, 'ITUMBIARA', 'GO'),
(2321, 'ITUMIRIM', 'MG'),
(2322, 'ITUPEVA', 'SP'),
(2323, 'ITUPIRANGA', 'PA'),
(2324, 'ITUPORANGA', 'SC'),
(2325, 'ITURAMA', 'MG'),
(2326, 'ITUTINGA', 'MG'),
(2327, 'ITUVERAVA', 'SP'),
(2328, 'IUIU', 'BA'),
(2329, 'IUNA', 'ES'),
(2330, 'IVAÍ', 'PR'),
(2331, 'IVAIPORÃ', 'PR'),
(2332, 'IVATE', 'PR'),
(2333, 'IVATUBA', 'PR'),
(2334, 'IVINHEMA', 'MS'),
(2335, 'IVOLÂNDIA', 'GO'),
(2336, 'IVORA', 'RS'),
(2337, 'IVOTI', 'RS'),
(2338, 'JABOATÃO DOS GUARARAPES', 'PE'),
(2339, 'JABORA', 'SC'),
(2340, 'JABORANDI', 'BA'),
(2341, 'JABOTI', 'PR'),
(2342, 'JABOTICABA', 'RS'),
(2343, 'JABOTICABAL', 'SP'),
(2344, 'JABOTICATUBAS', 'MG'),
(2345, 'JACANA', 'RN'),
(2346, 'JACARACI', 'BA'),
(2347, 'JACARAU', 'PB'),
(2348, 'JACARÉ DOS HOMENS', 'AL'),
(2349, 'JACARÉACANGA', 'PA'),
(2350, 'JACARÉÍ', 'SP'),
(2351, 'JACARÉZINHO', 'PR'),
(2352, 'JACI', 'SP'),
(2353, 'JACIARA', 'MT'),
(2354, 'JACINTO', 'MG'),
(2355, 'JACINTO MACHADO', 'SC'),
(2356, 'JACOBINA', 'BA'),
(2357, 'JACOBINA DO PIAUÍ', 'PI'),
(2358, 'JAÇUI', 'MG'),
(2359, 'JAÇUNDA', 'PA'),
(2360, 'JACUPIRANGA', 'SP'),
(2361, 'JACUTINGA', 'RS'),
(2362, 'JAÇUTINGA', 'MG'),
(2363, 'JÁGUAPITA', 'PR'),
(2364, 'JÁGUAQUARA', 'BA'),
(2365, 'JÁGUARAÇU', 'MG'),
(2366, 'JAGUARÃO', 'RS'),
(2367, 'JAGUARARI', 'BA'),
(2368, 'JÁGUARE', 'ES'),
(2369, 'JÁGUARETAMA', 'CE'),
(2370, 'JÁGUARI', 'RS'),
(2371, 'JAGUARIAÍVA', 'PR'),
(2372, 'JÁGUARIBARA', 'CE'),
(2373, 'JÁGUARIBE', 'CE'),
(2374, 'JÁGUARIUNA', 'SP'),
(2375, 'JÁGUARUANA', 'CE'),
(2376, 'JÁGUARUNA', 'SC'),
(2377, 'JAÍBA', 'MG'),
(2378, 'JAICOS', 'PI'),
(2379, 'JALES', 'SP'),
(2380, 'JAMARI', 'RO'),
(2381, 'JAMBEIRO', 'SP'),
(2382, 'JAMPRUCA', 'MG'),
(2383, 'JANAÚBA', 'MG'),
(2384, 'JANDAIA', 'GO'),
(2385, 'JANDAIA DO SUL', 'PR'),
(2386, 'JANDAÍRA', 'BA'),
(2387, 'JANDIRA', 'SP'),
(2388, 'JANDUIS', 'RN'),
(2389, 'JANGADA', 'MT'),
(2390, 'JANIÓPOLIS', 'PR'),
(2391, 'JANUARIA', 'MG'),
(2392, 'JAPARAÍBA', 'MG'),
(2393, 'JAPARATINGA', 'AL'),
(2394, 'JAPARATUBA', 'SE'),
(2395, 'JAPERI', 'RJ'),
(2396, 'JAPI', 'RN'),
(2397, 'JAPIRA', 'PR'),
(2398, 'JAPOATÃ', 'SE'),
(2399, 'JAPORÃ', 'MS'),
(2400, 'JAPURA', 'AM'),
(2401, 'JAQUARIPE', 'BA'),
(2402, 'JAQUEIRA', 'PE'),
(2403, 'JAQUIRANA', 'RS'),
(2404, 'JARÁGUA', 'GO'),
(2405, 'JARÁGUA DO SUL', 'SC'),
(2406, 'JARAGUARI', 'MS'),
(2407, 'JARAMATAIA', 'AL'),
(2408, 'JARDIM', 'CE'),
(2409, 'JARDIM ALEGRE', 'PR'),
(2410, 'JARDIM DE ANGICOS', 'RN'),
(2411, 'JARDIM DE MULATO', 'PI'),
(2412, 'JARDIM DE PIRANHAS', 'RN'),
(2413, 'JARDIM OLINDA', 'PR'),
(2414, 'JARDIM SERIDO', 'RN'),
(2415, 'JARDINÓPOLIS', 'SC'),
(2416, 'JARI', 'RS'),
(2417, 'JARINU', 'SP'),
(2418, 'JARU', 'RO'),
(2419, 'JATAI', 'GO'),
(2420, 'JATAIZINHO', 'PR'),
(2421, 'JATAUBA', 'PE'),
(2422, 'JATEI', 'MS'),
(2423, 'JATI', 'CE'),
(2424, 'JATOBÁ', 'MA'),
(2425, 'JATOBA DO PIAUÍ', 'PI'),
(2426, 'JAÚ DO TOCANTINS', 'TO'),
(2427, 'JAU DO TOCANTINS', 'TO'),
(2428, 'JAUPACI', 'GO'),
(2429, 'JAURU', 'MT'),
(2430, 'JECEABA', 'MG'),
(2431, 'JENIPAPO DE MINAS', 'MG'),
(2432, 'JENIPAPO DOS VIEIRAS', 'MA'),
(2433, 'JEQUERI', 'MG'),
(2434, 'JEQUIE', 'BA'),
(2435, 'JEQUITAÍ', 'MG'),
(2436, 'JEQUITIBÁ', 'MG'),
(2437, 'JEQUITINHONHA', 'MG'),
(2438, 'JEREMOABO', 'BA'),
(2439, 'JERICÓ', 'PB'),
(2440, 'JERIQUARA', 'SP'),
(2441, 'JERONIMO MONTEIRO', 'ES'),
(2442, 'JERUMENHA', 'PI'),
(2443, 'JESUANIA', 'MG'),
(2444, 'JESUITAS', 'PR'),
(2445, 'JESUPOLIS', 'GO'),
(2446, 'JIJOCA', 'CE'),
(2447, 'JI-PARANÁ', 'RO'),
(2448, 'JIQUIRICA', 'BA'),
(2449, 'JITAUMA', 'BA'),
(2450, 'JOAÇABA', 'SC'),
(2451, 'JOAIMA', 'MG'),
(2452, 'JOANÉSIA', 'MG'),
(2453, 'JOANÓPOLIS', 'SP'),
(2454, 'JOÃO ALFREDO', 'PE'),
(2455, 'JOÃO CÂMARA', 'RN'),
(2456, 'JOÃO COSTA', 'PI'),
(2457, 'JOÃO DIAS', 'RN'),
(2458, 'JOÃO DOURADO', 'BA'),
(2459, 'JOÃO LISBOA', 'MA'),
(2460, 'JOÃO MONLEVADE', 'MG'),
(2461, 'JOÃO NEIVA', 'ES'),
(2462, 'JOÃO PESSOA', 'PB'),
(2463, 'JOÃO PINHEIRO', 'MG'),
(2464, 'JOÃO RAMALHO', 'SP'),
(2465, 'JOAQUIM FELICIO', 'MG'),
(2466, 'JOAQUIM GOMES', 'AL'),
(2467, 'JOAQUIM NABUCO', 'PE'),
(2468, 'JOAQUIM PIRES', 'PI'),
(2469, 'JOAQUIM TAVORA', 'PR'),
(2470, 'JOCA MARQUES', 'PI'),
(2471, 'JOIA', 'RS'),
(2472, 'JOINVILLE', 'SC'),
(2473, 'JORDANIA', 'MG'),
(2474, 'JORDÃO', 'AC'),
(2475, 'JOSÉ BOITEUX', 'SC'),
(2476, 'JOSÉ BONIFACIO', 'SP'),
(2477, 'JOSÉ DA PENHA', 'RN'),
(2478, 'JOSÉ DE FREITAS', 'PI'),
(2479, 'JOSÉ GONÇALVES DE MINAS', 'MG'),
(2480, 'JOSÉ RAYDAN', 'MG'),
(2481, 'JOSÉLÂNDIA', 'MA'),
(2482, 'JOSENÓPOLIS', 'MG'),
(2483, 'JOVIÂNIA', 'GO'),
(2484, 'JUARA', 'MT'),
(2485, 'JUAREZ TAVORA', 'PB'),
(2486, 'JUARINA', 'TO'),
(2487, 'JUATUBA', 'MG'),
(2488, 'JUAZEIRINHO', 'PB'),
(2489, 'JUAZEIRO', 'BA'),
(2490, 'JUAZEIRO DO NORTE', 'CE'),
(2491, 'JUAZEIRO DO PIAUÍ', 'PI'),
(2492, 'JUCAS', 'CE'),
(2493, 'JUCATI', 'PE'),
(2494, 'JUCURUÇU', 'BA'),
(2495, 'JUCURUTU', 'RN'),
(2496, 'JUÍNA', 'MT'),
(2497, 'JUIZ DE FORA', 'MG'),
(2498, 'JULIO BORGES', 'PI'),
(2499, 'JÚLIO DE CASTILHOS', 'RS'),
(2500, 'JÚLIO MESQUITA', 'SP'),
(2501, 'JUMIRIM', 'SP'),
(2502, 'JUNCO DO MARANHÃO', 'MA'),
(2503, 'JUNCO DO SERIDO', 'PB'),
(2504, 'JUNDIA', 'AL'),
(2505, 'JUNDIAI', 'SP'),
(2506, 'JUNDIAI DO SUL', 'PR'),
(2507, 'JUNQUEIRO', 'AL'),
(2508, 'JUNQUEIRÓPOLIS', 'SP'),
(2509, 'JUPI', 'PE'),
(2510, 'JUPIÁ', 'SC'),
(2511, 'JUQUIÁ', 'SP'),
(2512, 'JUQUITIBA', 'SP'),
(2513, 'JURAMENTO', 'MG'),
(2514, 'JURANDA', 'PR'),
(2515, 'JUREMA', 'PE'),
(2516, 'JURIPIRANGA', 'PB'),
(2517, 'JURU', 'PB'),
(2518, 'JURUA', 'AM'),
(2519, 'JURUAIA', 'MG'),
(2520, 'JURUENA', 'MT'),
(2521, 'JURUTI', 'PA'),
(2522, 'JUSCIMEIRA', 'MT'),
(2523, 'JUSSARA', 'GO'),
(2524, 'JUSSARA', 'PR'),
(2525, 'JUSSARI', 'BA'),
(2526, 'JUSSIAPE', 'BA'),
(2527, 'JUTAÍ', 'AM'),
(2528, 'JUTI', 'MS'),
(2529, 'KALORÉ', 'PR'),
(2530, 'LABREA', 'AM'),
(2531, 'LACERDÓPOLIS', 'SC'),
(2532, 'LADAINHA', 'MG'),
(2533, 'LADÁRIO', 'MS'),
(2534, 'LAFAIETE COUTINHO', 'BA'),
(2535, 'LAGAMAR', 'MG'),
(2536, 'LAGARTO', 'SE'),
(2537, 'LAGE', 'BA'),
(2538, 'LAGEADO', 'TO'),
(2539, 'LAGES', 'RN'),
(2540, 'LAGO DA PEDRA', 'MA'),
(2541, 'LAGO DO JUNCO', 'MA'),
(2542, 'LAGO DO MATO', 'MA'),
(2543, 'LAGO DOS RODRIGUES', 'MA'),
(2544, 'LAGO VERDE', 'MA'),
(2545, 'LAGOA', 'PB'),
(2546, 'LAGOA ALEGRE', 'PI'),
(2547, 'LAGOA DA CANOA', 'AL'),
(2548, 'LAGOA DA CONFUSÃO', 'TO'),
(2549, 'LAGOA DA PRATA', 'MG'),
(2550, 'LAGOA D''ANTA', 'RN'),
(2551, 'LAGOA DE DENTRO', 'PB'),
(2552, 'LAGOA DE ITAENGA', 'PE'),
(2553, 'LAGOA DE PEDRAS', 'RN'),
(2554, 'LAGOA DE SÃO FRANCISCO', 'PI'),
(2555, 'LAGOA DE VELHOS', 'RN'),
(2556, 'LAGOA DO  PIAUÍ', 'PI'),
(2557, 'LAGOA DO BARRO DO PIAUÍ', 'PI'),
(2558, 'LAGOA DO CARRO', 'PE'),
(2559, 'LAGOA DO OURO', 'PE'),
(2560, 'LAGOA DO SITIO', 'PI'),
(2561, 'LAGOA DO TOCANTINS', 'TO'),
(2562, 'LAGOA DOS GATOS', 'PE'),
(2563, 'LAGOA DOS PATOS', 'MG'),
(2564, 'IRACEMA', 'RR'),
(2565, 'LAGOA DOS TRÊS CANTOS', 'RS'),
(2566, 'LAGOA DOURADA', 'MG'),
(2567, 'LAGOA FORMOSA', 'MG'),
(2568, 'LAGOA GRANDE', 'MG'),
(2569, 'LAGOA GRANDE DO MARANHÃO', 'MA'),
(2570, 'LAGOA NOVA', 'RN'),
(2571, 'LAGOA REAL', 'BA'),
(2572, 'LAGOA SALGADA', 'RN'),
(2573, 'LAGOA SANTA', 'MG'),
(2574, 'LAGOA SECA', 'PB'),
(2575, 'LAGOA VERMELHA', 'RS'),
(2576, 'LAGOAO', 'RS'),
(2577, 'LAGOINHA', 'SP'),
(2578, 'LAGOINHA DO PIAUÍ', 'PI'),
(2579, 'LAGUNA', 'SC'),
(2580, 'LAGUNA CARAPÃ', 'MS'),
(2581, 'LAJE DO MURIAÉ', 'RJ'),
(2582, 'LAJEADO', 'RS'),
(2583, 'LAJEADO DO BUGRE', 'RS'),
(2584, 'LAJEADO GRANDE', 'SC'),
(2585, 'LAJEADO NOVO', 'MA'),
(2586, 'LAJEDÃO', 'BA'),
(2587, 'LAJEDINHO', 'BA'),
(2588, 'LAJEDO', 'PE'),
(2589, 'LAJEDO DO TABOCAL', 'BA'),
(2590, 'LAJES PINTADAS', 'RN'),
(2591, 'LAJINHA', 'MG'),
(2592, 'LAMARÃO', 'BA'),
(2593, 'LAMBARI', 'MG'),
(2594, 'LAMBARI DO OESTE', 'MT'),
(2595, 'LAMIM', 'MG'),
(2596, 'LANDRI SALES', 'PI'),
(2597, 'LAPA', 'PR'),
(2598, 'LAPÃO', 'BA'),
(2599, 'LARANJA DA TERRA', 'ES'),
(2600, 'LARANJAL', 'MG'),
(2601, 'LARANJAL DO JARI', 'AP'),
(2602, 'LARANJAL PAULISTA', 'SP'),
(2603, 'LARANJEIRAS', 'SE'),
(2604, 'LASSANCE', 'MG'),
(2605, 'LASTRO', 'PB'),
(2606, 'LAURENTINO', 'SC'),
(2607, 'LAURO DE FREITAS', 'BA'),
(2608, 'LAURO MULLER', 'SC'),
(2609, 'LAVANDEIRA', 'TO'),
(2610, 'LAVINIA', 'SP'),
(2611, 'LAVRAS', 'MG'),
(2612, 'LAVRAS DA MANGABEIRA', 'CE'),
(2613, 'LAVRAS DO SUL', 'RS'),
(2614, 'LAVRINHAS', 'SP'),
(2615, 'LEANDRO FERREIRA', 'MG'),
(2616, 'LEBON REGIS', 'SC'),
(2617, 'LEME', 'SP'),
(2618, 'LEME DO PRADO', 'MG'),
(2619, 'LENÇÓIS', 'BA'),
(2620, 'LENÇÓIS PAULISTA', 'SP'),
(2621, 'LEOBERTO LEAL', 'SC'),
(2622, 'LEOPOLDINA', 'MG'),
(2623, 'LEOPOLDO DE BULHOES', 'GO'),
(2624, 'LEÓPOLIS', 'PR'),
(2625, 'LIBERATO SALZANO', 'RS'),
(2626, 'LIBERDADE', 'MG'),
(2627, 'LICINIO DE ALMEIDA', 'BA'),
(2628, 'LIDIANÓPOLIS', 'PR'),
(2629, 'LIMA CAMPOS', 'MA'),
(2630, 'LIMA DUARTE', 'MG'),
(2631, 'LIMEIRA', 'SP'),
(2632, 'LIMEIRA DO OESTE', 'MG'),
(2633, 'LIMOEIRO', 'PE'),
(2634, 'LIMOEIRO DE ANADIA', 'AL'),
(2635, 'LIMOEIRO DO AJURU', 'PA'),
(2636, 'LIMOEIRO DO NORTE', 'CE'),
(2637, 'LINDOESTE', 'PR'),
(2638, 'LINDÓIA', 'SP'),
(2639, 'LINDOIA DO SUL', 'SC'),
(2640, 'LINDOLFO COLLOR', 'RS'),
(2641, 'LINHA NOVA', 'RS'),
(2642, 'LINHARES', 'ES'),
(2643, 'LINS', 'SP'),
(2644, 'LIV DE NOSSA SENHORA', 'BA'),
(2645, 'LIVRAMENTO', 'PB'),
(2646, 'LIZARDA', 'TO'),
(2647, 'LOANDA', 'PR'),
(2648, 'LOBATO', 'PR'),
(2649, 'LOGRADOURO', 'PB'),
(2650, 'LONDRINA', 'PR'),
(2651, 'LONTRA', 'MG'),
(2652, 'LONTRAS', 'SC'),
(2653, 'LORENA', 'SP'),
(2654, 'LORETO', 'MA'),
(2655, 'LOURDES', 'SP'),
(2656, 'LOUVEIRA', 'SP'),
(2657, 'LUCAS DO RIO VERDE', 'MT'),
(2658, 'LUCELIA', 'SP'),
(2659, 'LUCENA', 'PB'),
(2660, 'LUCIANÓPOLIS', 'SP'),
(2661, 'LUCIARA', 'MT'),
(2662, 'LUCRECIA', 'RN'),
(2663, 'LUGAR CEDRO', 'MA'),
(2664, 'LUIS ALVES', 'SC'),
(2665, 'LUIS DOMINGUES', 'MA'),
(2666, 'LUIS GOMES', 'RN'),
(2667, 'LUIZIANA', 'PR'),
(2668, 'LUISIANIA', 'SP'),
(2669, 'LUIZ ANTÔNIO', 'SP'),
(2670, 'LUIZ CORREIA', 'PI'),
(2671, 'LUMINÁRIAS', 'MG'),
(2672, 'LUNARDELLI', 'PR'),
(2673, 'LUPERCIO', 'SP'),
(2674, 'LUPIONÓPOLIS', 'PR'),
(2675, 'LUTÉCIA', 'SP'),
(2676, 'LUZ', 'MG'),
(2677, 'LUZERNA', 'SC'),
(2678, 'LUZIANIA', 'GO'),
(2679, 'LUZILÂNDIA', 'PI'),
(2680, 'LUZINÓPOLIS', 'TO'),
(2681, 'MACAÉ', 'RJ'),
(2682, 'MACAIBA', 'RN'),
(2683, 'MACAJUBA', 'BA'),
(2684, 'MACAMBARA', 'RS'),
(2685, 'MACAMBIRA', 'SE'),
(2686, 'MACAPÁ', 'AP'),
(2687, 'MACAPARANÁ', 'PE'),
(2688, 'MACARANI', 'BA'),
(2689, 'MACATUBA', 'SP'),
(2690, 'MACAU', 'RN'),
(2691, 'MACAUBAL', 'SP'),
(2692, 'MACAUBAS', 'BA'),
(2693, 'MACÊDO', 'PI'),
(2694, 'MACEDÔNIA', 'SP'),
(2695, 'MACEIÓ', 'AL'),
(2696, 'MACHACALIS', 'MG'),
(2697, 'MACHADINHO', 'RS'),
(2698, 'MACHADINHO DO OESTE', 'RO'),
(2699, 'MACHADO', 'MG'),
(2700, 'MACHADOS', 'PE'),
(2701, 'MACHALIS', 'MG'),
(2702, 'MACIEIRA', 'SC'),
(2703, 'MACUCO', 'RJ'),
(2704, 'MAÇURURE', 'BA'),
(2705, 'MADALENA', 'CE'),
(2706, 'MADEIRO', 'PI'),
(2707, 'MADRE DE DEUS', 'BA'),
(2708, 'MADRE DE DEUS DE MINAS', 'MG'),
(2709, 'MAE DA ÁGUA', 'PB'),
(2710, 'MAE DO RIO', 'PA'),
(2711, 'MAETINGA', 'BA'),
(2712, 'MAFRA', 'SC'),
(2713, 'MAGALHÃES BARATA', 'PA'),
(2714, 'MAGALHAES DE ALMEIDA', 'MA'),
(2715, 'MAGDA', 'SP'),
(2716, 'MAGÉ', 'RJ'),
(2717, 'MAIQUINIQUE', 'BA'),
(2718, 'MAIRI', 'BA'),
(2719, 'MAIRINQUE', 'SP'),
(2720, 'MAIRIPORA', 'SP'),
(2721, 'MAIRIPOTABA', 'GO'),
(2722, 'MAJOR GERCINO', 'SC'),
(2723, 'MAJOR ISIDORO', 'AL'),
(2724, 'MAJOR SALES', 'RN'),
(2725, 'MAJOR VIEIRA', 'SC'),
(2726, 'MALACACHETA', 'MG'),
(2727, 'MALHADA', 'BA'),
(2728, 'MALHADA DE PEDRAS', 'BA'),
(2729, 'MALHADA DOS BOIS', 'SE'),
(2730, 'MALHADOR', 'SE'),
(2731, 'MALLET', 'PR'),
(2732, 'MALTA', 'PB'),
(2733, 'MAMANGUAPE', 'PB'),
(2734, 'MAMBAÍ', 'GO'),
(2735, 'MAMBORÊ', 'PR'),
(2736, 'MAMONAS', 'MG'),
(2737, 'MAMPITUBA', 'RS'),
(2738, 'MANACAPURU', 'AM'),
(2739, 'MANAÍRA', 'PB'),
(2740, 'MANAQUIRI', 'AM'),
(2741, 'MANARI', 'PE'),
(2742, 'MANAUS', 'AM'),
(2743, 'MÂNCIO LIMA', 'AC'),
(2744, 'MANDÁGUAÇU', 'PR'),
(2745, 'MANDAGUARI', 'PR'),
(2746, 'MANDIRITUBA', 'PR'),
(2747, 'MANDURI', 'SP'),
(2748, 'MANFRINÓPOLIS', 'PR'),
(2749, 'MANGA', 'MG'),
(2750, 'MANGARATIBA', 'RJ'),
(2751, 'MANGUEIRINHA', 'PR'),
(2752, 'MANHUAÇU', 'MG'),
(2753, 'MANHUMIRIM', 'MG'),
(2754, 'MANICORE', 'AM'),
(2755, 'MANOEL EMÍDIO', 'PI'),
(2756, 'MANOEL RIBAS', 'PR'),
(2757, 'MANOEL URBANO', 'AC'),
(2758, 'MANOEL VIANA', 'RS'),
(2759, 'MANOEL VITORINO', 'BA'),
(2760, 'MANSIDÃO', 'BA'),
(2761, 'MANTENA', 'MG'),
(2762, 'MANTENÓPOLIS', 'ES'),
(2763, 'MAQUINÉ', 'RS'),
(2764, 'MAR DE ESPANHA', 'MG'),
(2765, 'MARA ROSA', 'GO'),
(2766, 'MARAA', 'AM'),
(2767, 'MARABA', 'PA'),
(2768, 'MARABA PAULISTA', 'SP'),
(2769, 'MARAÇAÇUME', 'MA'),
(2770, 'MARAÇAI', 'SP'),
(2771, 'MARAÇAJA', 'SC'),
(2772, 'MARACAJU', 'MS'),
(2773, 'MARACANÃ', 'PA'),
(2774, 'MARAÇANAU', 'CE'),
(2775, 'MARAÇAS', 'BA'),
(2776, 'MARAGOGI', 'AL'),
(2777, 'MARAGOJIPE', 'BA'),
(2778, 'MARAIAL', 'PE'),
(2779, 'MARAJA DO SENA', 'MA'),
(2780, 'MARANDIBA', 'PE'),
(2781, 'MARANGUAPE', 'CE'),
(2782, 'MARANHÃOZINHO', 'MA'),
(2783, 'MARAPANIM', 'PA'),
(2784, 'MARAPOAMA', 'SP'),
(2785, 'MARATÁ', 'RS'),
(2786, 'MARATAIZES', 'ES'),
(2787, 'MARAÚ', 'BA'),
(2788, 'MARAVILHA', 'AL'),
(2789, 'MARAVILHAS', 'MG'),
(2790, 'MARCAÇÃO', 'PB'),
(2791, 'MARCELÂNDIA', 'MT'),
(2792, 'MARCELINO RAMOS', 'RS'),
(2793, 'MARCELINO VIEIRA', 'RN'),
(2794, 'MARCIONILIO SOUZA', 'BA'),
(2795, 'MARCO', 'CE'),
(2796, 'MARCOLÂNDIA', 'PI'),
(2797, 'MARCOS PARENTE', 'PI'),
(2798, 'MARECHAL CÂNDIDO RONDON', 'PR'),
(2799, 'MARECHAL DEODORO', 'AL'),
(2800, 'MARECHAL FLORIANO', 'ES'),
(2801, 'MARECHAL TRAUMATURGO', 'AC'),
(2802, 'MAREMA', 'SC'),
(2803, 'MARI', 'PB'),
(2804, 'MARIA DA FE', 'MG'),
(2805, 'MARIA HELENA', 'PR'),
(2806, 'MARIALVA', 'PR'),
(2807, 'MARIANA', 'MG'),
(2808, 'MARIANA PIMENTEL', 'RS'),
(2809, 'MARIANO MORO', 'RS'),
(2810, 'MARIANÓPOLIS DO TOCANTINS', 'TO'),
(2811, 'MARIAPOLIS', 'SP'),
(2812, 'MARIBONDO', 'AL'),
(2813, 'MARICÁ', 'RJ'),
(2814, 'MARILAC', 'MG'),
(2815, 'MARILÂNDIA', 'ES'),
(2816, 'MARILÂNDIA DO SUL', 'PR'),
(2817, 'MARILENA', 'PR'),
(2818, 'MARILIA', 'SP'),
(2819, 'MARILUZ', 'PR'),
(2820, 'MARINGÁ', 'PR'),
(2821, 'MARINÓPOLIS', 'SP'),
(2822, 'MARIO CAMPOS', 'MG'),
(2823, 'MARIÓPOLIS', 'PR'),
(2824, 'MARIPA', 'PR'),
(2825, 'MARIPA DE MINAS', 'MG'),
(2826, 'MARITUBA', 'PA'),
(2827, 'MARIZÓPOLIS', 'PB'),
(2828, 'MARLIERIA', 'MG'),
(2829, 'MARMELEIRO', 'PR'),
(2830, 'MARMELÓPOLIS', 'MG'),
(2831, 'MARQUES DE SOUZA', 'RS'),
(2832, 'MARQUINHO', 'PR'),
(2833, 'MARTINHO CAMPOS', 'MG'),
(2834, 'MARTINOPOLE', 'CE'),
(2835, 'MARTINÓPOLIS', 'SP'),
(2836, 'MARTINS', 'RN'),
(2837, 'MARTINS SOARES', 'MG'),
(2838, 'MARUIM', 'SE'),
(2839, 'MARUMBI', 'PR'),
(2840, 'MARZAGAO', 'GO'),
(2841, 'MASCOTE', 'BA'),
(2842, 'MASSAPE', 'CE'),
(2843, 'MASSAPE DO PIAUÍ', 'PI'),
(2844, 'MASSARANDUBA', 'PB'),
(2845, 'MASSARANDUBA', 'SC'),
(2846, 'MATA', 'RS'),
(2847, 'MATA DE SÃO JOÃO', 'BA'),
(2848, 'MATA GRANDE', 'AL'),
(2849, 'MATA ROMA', 'MA'),
(2850, 'MATA VERDE', 'MG'),
(2851, 'MATÃO', 'SP'),
(2852, 'MATARAÇA', 'PB'),
(2853, 'MATEIROS', 'TO'),
(2854, 'MATELÂNDIA', 'PR'),
(2855, 'MATERLÂNDIA', 'MG'),
(2856, 'MATEUS LEME', 'MG'),
(2857, 'MATHIAS LOBATO', 'MG'),
(2858, 'MATIAS BARBOSA', 'MG'),
(2859, 'MATIAS CARDOSO', 'MG'),
(2860, 'MATIAS OLIMPIO', 'PI'),
(2861, 'MATINA', 'BA'),
(2862, 'MATINHA', 'MA'),
(2863, 'MATINHAS', 'PB'),
(2864, 'MATINHOS', 'PR'),
(2865, 'MATIPO', 'MG'),
(2866, 'MATO CASTELHANO', 'RS'),
(2867, 'MATO GROSSO', 'PB'),
(2868, 'MATO LEITÃO', 'RS'),
(2869, 'MATO RICO', 'PR'),
(2870, 'MATO VERDE', 'MG'),
(2871, 'MATÕES', 'MA'),
(2872, 'MATOES DO NORTE', 'MA'),
(2873, 'MATOS COSTA', 'SC'),
(2874, 'MATOZINHOS', 'MG'),
(2875, 'MATRINCHA', 'GO'),
(2876, 'MATRIZ DE CAMARAGIBE', 'AL'),
(2877, 'MATUPA', 'MT'),
(2878, 'MATUREIA', 'PB'),
(2879, 'MATUTINA', 'MG'),
(2880, 'MAUA', 'SP'),
(2881, 'MAUA DA SERRA', 'PR'),
(2882, 'MAUES', 'AM'),
(2883, 'MAURILÂNDIA', 'GO'),
(2884, 'MAURILÂNDIA DO TOCANTINS', 'TO'),
(2885, 'MAURITI', 'CE'),
(2886, 'MAXARANGUAPE', 'RN'),
(2887, 'MAXIMILIANO DE ALMEIDA', 'RS'),
(2888, 'MAZAGÃO', 'AP'),
(2889, 'MEDEIROS', 'MG'),
(2890, 'MEDEIROS NETO', 'BA'),
(2891, 'MEDIANEIRA', 'PR'),
(2892, 'MEDICILÂNDIA', 'PA'),
(2893, 'MEDINA', 'MG'),
(2894, 'MELEIRO', 'SC'),
(2895, 'MELGACO', 'PA'),
(2896, 'MENDES', 'RJ'),
(2897, 'MENDES PIMENTEL', 'MG'),
(2898, 'MENDONCA', 'SP'),
(2899, 'MERCEDES', 'PR'),
(2900, 'MERCES', 'MG'),
(2901, 'MERIDIANO', 'SP'),
(2902, 'MERUOCA', 'CE'),
(2903, 'MESÓPOLIS', 'SP'),
(2904, 'MESQUITA', 'MG'),
(2905, 'MESSIAS', 'AL'),
(2906, 'MESSIAS TARGINO', 'RN'),
(2907, 'MIGUEL ALVES', 'PI'),
(2908, 'MIGUEL CALMON', 'BA'),
(2909, 'MIGUEL LEÃO', 'PI'),
(2910, 'MIGUEL PEREIRA', 'RJ'),
(2911, 'MIGUELÓPOLIS', 'SP'),
(2912, 'MILAGRES', 'BA'),
(2913, 'MILAGRES DO MARANHÃO', 'MA'),
(2914, 'MILHA', 'CE'),
(2915, 'MILTON BRANDÃO', 'PI'),
(2916, 'MIMOSO DE GOIÁS', 'GO'),
(2917, 'MIMOSO DO SUL', 'ES'),
(2918, 'MINAÇU', 'GO'),
(2919, 'MINADOR DO NEGRÃO', 'AL'),
(2920, 'MINAS DO LEÃO', 'RS'),
(2921, 'MINAS NOVAS', 'MG'),
(2922, 'MINDURI', 'MG'),
(2923, 'MINEIROS', 'GO'),
(2924, 'MINEIROS DO TIETE', 'SP'),
(2925, 'MINISTRO ANDRÉAZA', 'RO'),
(2926, 'MINISTRO ANDRÉAZZA', 'RO'),
(2927, 'MIRA ESTRELA', 'SP'),
(2928, 'MIRABELA', 'MG'),
(2929, 'MIRACATU', 'SP'),
(2930, 'MIRACEMA', 'RJ'),
(2931, 'MIRACEMA DO TOCANTINS', 'TO'),
(2932, 'MIRADOR', 'MA'),
(2933, 'MIRADOR', 'PR'),
(2934, 'MIRADOURO', 'MG'),
(2935, 'MIRÁGUAI', 'RS'),
(2936, 'MIRAÍ', 'MG'),
(2937, 'MIRAÍMA', 'CE'),
(2938, 'MIRANDA', 'MS'),
(2939, 'MIRANDA DO NORTE', 'MA'),
(2940, 'MIRANDIBA', 'PE'),
(2941, 'MIRANDÓPOLIS', 'SP'),
(2942, 'MIRANGABA', 'BA'),
(2943, 'MIRANORTE', 'TO'),
(2944, 'MIRANTE', 'BA'),
(2945, 'MIRANTE DA SERRA', 'RO'),
(2946, 'MIRANTE DO PARANÁPANEMA', 'SP'),
(2947, 'MIRASELVA', 'PR'),
(2948, 'MIRASSOL', 'SP'),
(2949, 'MIRASSOL D''OESTE', 'MT'),
(2950, 'MIRASSOLÂNDIA', 'SP'),
(2951, 'MIRAVÂNIA', 'MG'),
(2952, 'MIRIM DOCE', 'SC'),
(2953, 'MIRINZAL', 'MA'),
(2954, 'MISSAL', 'PR'),
(2955, 'MISSÃO VELHA', 'CE'),
(2956, 'MOCAJUBA', 'PA'),
(2957, 'MOCOCA', 'SP'),
(2958, 'MODELO', 'SC'),
(2959, 'MOEDA', 'MG'),
(2960, 'MOEMA', 'MG'),
(2961, 'MOGEIRO', 'PB'),
(2962, 'MOGI DAS CRUZES', 'SP'),
(2963, 'MOGI GUAÇU', 'SP'),
(2964, 'MOGI MIRIM', 'SP'),
(2965, 'MOIPORA', 'GO'),
(2966, 'MOITA BONITA', 'SE'),
(2967, 'MOJU', 'PA'),
(2968, 'MOMBAÇA', 'CE'),
(2969, 'MOMBUCA', 'SP'),
(2970, 'MONCÃO', 'MA'),
(2971, 'MONÇÕES', 'SP'),
(2972, 'MONDAI', 'SC'),
(2973, 'MONGAGUÁ', 'SP'),
(2974, 'MONJOLOS', 'MG'),
(2975, 'MONSENHOR GIL', 'PI'),
(2976, 'MONSENHOR HIPÓLITO', 'PI'),
(2977, 'MONSENHOR PAULO', 'MG'),
(2978, 'MONSENHOR TABOSA', 'CE'),
(2979, 'MONTADAS', 'PB'),
(2980, 'MONTALVÂNIA', 'MG'),
(2981, 'MONTANHA', 'ES'),
(2982, 'MONTANHAS', 'RN'),
(2983, 'MONTAURI', 'RS'),
(2984, 'MONTE ALEGRE', 'PA'),
(2985, 'MONTE ALEGRE', 'SE'),
(2986, 'MONTE ALEGRE DE GOIÁS', 'GO'),
(2987, 'MONTE ALEGRE DE MINAS', 'MG'),
(2988, 'MONTE ALEGRE DO PIAUÍ', 'PI'),
(2989, 'MONTE ALEGRE DO SUL', 'SP'),
(2990, 'MONTE ALEGRE DOS CAMPOS', 'RS'),
(2991, 'MONTE ALTO', 'SP'),
(2992, 'MONTE APRAZIVEL', 'SP'),
(2993, 'MONTE AZUL', 'MG'),
(2994, 'MONTE AZUL PAULISTA', 'SP'),
(2995, 'MONTE BELO', 'MG'),
(2996, 'MONTE BELO DO SUL', 'RS'),
(2997, 'MONTE CARLO', 'SC'),
(2998, 'MONTE CARMELO', 'MG'),
(2999, 'MONTE CASTELO', 'SC'),
(3000, 'MONTE DAS GAMELEIRAS', 'RN'),
(3001, 'MONTE DO CARMO', 'TO'),
(3002, 'MONTE FORMOSO', 'MG'),
(3003, 'MONTE HOREBE', 'PB'),
(3004, 'MONTE MOR', 'SP'),
(3005, 'MONTE SANTO', 'BA'),
(3006, 'MONTE SANTO DE MINAS', 'MG'),
(3007, 'MONTE SIÃO', 'MG'),
(3008, 'MONTEIRO', 'PB'),
(3009, 'MONTEIRO LOBATO', 'SP'),
(3010, 'MONTENEGRO', 'RS'),
(3011, 'MONTES ALTOS', 'MA'),
(3012, 'MONTES CLAROS', 'MG'),
(3013, 'MONTES CLAROS DE GOIÁS', 'GO'),
(3014, 'MONTEZUMA', 'MG'),
(3015, 'MONTIVIDIU', 'GO'),
(3016, 'MONTIVIDIU DO NORTE', 'GO'),
(3017, 'MORADA NOVA', 'CE'),
(3018, 'MORADA NOVA DE MINAS', 'MG'),
(3019, 'MORAUJO', 'CE'),
(3020, 'MOREIRA SALES', 'PR'),
(3021, 'MORENO', 'PE'),
(3022, 'MORMAÇO', 'RS'),
(3023, 'MORPARÁ', 'BA'),
(3024, 'MORRETES', 'PR'),
(3025, 'MORRINHAS DO SUL', 'RS'),
(3026, 'MORRINHOS', 'CE'),
(3027, 'MORRINHOS DO SUL', 'RS'),
(3028, 'MORRO AGUDO', 'SP'),
(3029, 'MORRO AGUDO DE GOIÁS', 'GO'),
(3030, 'MORRO CABEÇA NO TEMPO', 'PI'),
(3031, 'MORRO DA FUMAÇA', 'SC'),
(3032, 'MORRO DA GARÇA', 'MG'),
(3033, 'MORRO DO CHAPÉU', 'BA'),
(3034, 'MORRO DO CHAPÉU DO PIAUÍ', 'PI'),
(3035, 'MORRO DO PILAR', 'MG'),
(3036, 'MORRO GRANDE', 'SC'),
(3037, 'MORRO REDONDO', 'RS'),
(3038, 'MORRO REUTER', 'RS'),
(3039, 'MORROS', 'MA'),
(3040, 'MORTUGABA', 'BA'),
(3041, 'MORUNGABA', 'SP'),
(3042, 'MOSSAMEDES', 'GO'),
(3043, 'MOSSORÓ', 'RN'),
(3044, 'MOSTARDAS', 'RS'),
(3045, 'MOTUCA', 'SP'),
(3046, 'MOZARLÂNDIA', 'GO'),
(3047, 'MUANA', 'PA'),
(3048, 'MUCAJAÍ', 'RR'),
(3049, 'MUCAMBO', 'CE'),
(3050, 'MUCUGE', 'BA'),
(3051, 'MUÇUM', 'RS'),
(3052, 'MUCURI', 'BA'),
(3053, 'MUCURICI', 'ES'),
(3054, 'MUITOS CAPOES', 'RS'),
(3055, 'MULITERNO', 'RS'),
(3056, 'MULUNGU', 'CE'),
(3057, 'MULUNGU DO MORRO', 'BA'),
(3058, 'MUNDO NOVO', 'BA'),
(3059, 'MUNHOZ', 'MG'),
(3060, 'MUNHOZ DE MELO', 'PR'),
(3061, 'MUNIZ FERREIRA', 'BA'),
(3062, 'MUNIZ FREIRE', 'ES'),
(3063, 'MUQUEM DO SÃO FRANCISCO', 'BA'),
(3064, 'MUQUI', 'ES'),
(3065, 'MURIAÉ', 'MG'),
(3066, 'MURIBECA', 'SE'),
(3067, 'MURICI', 'AL'),
(3068, 'MURICI DOS PORTELA', 'PI'),
(3069, 'MURICILÂNDIA', 'TO'),
(3070, 'MURITIBA', 'BA'),
(3071, 'MURUTINGA DO SUL', 'SP'),
(3072, 'MUTUIPE', 'BA'),
(3073, 'MUTUM', 'MG'),
(3074, 'MUTUNÓPOLIS', 'GO'),
(3075, 'MUZAMBINHO', 'MG'),
(3076, 'NACIF RAYDAN', 'MG'),
(3077, 'NANTES', 'SP'),
(3078, 'NANUQUE', 'MG'),
(3079, 'NÃO ME TOQUE', 'RS'),
(3080, 'NAQUE', 'MG'),
(3081, 'NARANDIBA', 'SP'),
(3082, 'NATAL', 'RN'),
(3083, 'NATALÂNDIA', 'MG'),
(3084, 'NATÉRCIA', 'MG'),
(3085, 'NATIVIDADE', 'RJ'),
(3086, 'NATIVIDADE DA SERRA', 'SP'),
(3087, 'NATUBA', 'PB'),
(3088, 'NAVEGANTES', 'SC'),
(3089, 'NAVIRAÍ', 'MS'),
(3090, 'NAZARÉ', 'BA'),
(3091, 'NAZARÉ DA MATA', 'PE'),
(3092, 'NAZARÉ DO PIAUÍ', 'PI'),
(3093, 'NAZARÉ PAULISTA', 'SP'),
(3094, 'NAZARENO', 'MG'),
(3095, 'NAZARÉZINHO', 'PB'),
(3096, 'NAZARIO', 'GO'),
(3097, 'NEÓPOLIS', 'SE'),
(3098, 'NEPOMUCENO', 'MG'),
(3099, 'NERÓPOLIS', 'GO'),
(3100, 'NEVES PAULISTA', 'SP'),
(3101, 'NHAMUNDA', 'AM'),
(3102, 'NHANDEARA', 'SP'),
(3103, 'NICOLAU VERGUEIRO', 'RS'),
(3104, 'NILO PEÇANHA', 'BA'),
(3105, 'NILÓPOLIS', 'RJ'),
(3106, 'NINA RODRIGUES', 'MA'),
(3107, 'NINHEIRA', 'MG'),
(3108, 'NIOAQUE', 'MS'),
(3109, 'NIPOÃ', 'SP'),
(3110, 'NIQUELÂNDIA', 'GO'),
(3111, 'NISIA FLORESTA', 'RN'),
(3112, 'NITERÓI', 'RJ'),
(3113, 'NOBRES', 'MT'),
(3114, 'NONOAI', 'RS'),
(3115, 'NORDESTINA', 'BA'),
(3116, 'NORMANDIA', 'RR'),
(3117, 'NORTELÂNDIA', 'MT'),
(3118, 'NOSSA SENHORA APARECIDA', 'SE'),
(3119, 'NOSSA SENHORA DA GLÓRIA', 'SE'),
(3120, 'NOSSA SENHORA DAS DORES', 'SE'),
(3121, 'NOSSA SENHORA DAS GRAÇAS', 'PR'),
(3122, 'NOSSA SENHORA DE NAZARÉ', 'PI'),
(3123, 'NOSSA SENHORA DO LIVRAMENTO', 'MT'),
(3124, 'NOSSA SENHORA DO SOCORRO', 'SE'),
(3125, 'NOSSA SENHORA DOS REMÉDIOS', 'PI'),
(3126, 'NOSSA SENHORA DE LOURDES', 'SE'),
(3127, 'NOVA ALIANÇA', 'SP'),
(3128, 'NOVA ALIANÇA DO IVAÍ', 'PR'),
(3129, 'NOVA ALVORADA', 'RS'),
(3130, 'NOVA ALVORADA DO SUL', 'MS'),
(3131, 'NOVA AMÉRICA', 'GO'),
(3132, 'NOVA AMÉRICA DA COLINA', 'PR'),
(3133, 'NOVA ANDRADINA', 'MS'),
(3134, 'NOVA ARAÇA', 'RS'),
(3135, 'NOVA AURORA', 'GO'),
(3136, 'NOVA BANDEIRANTE', 'MT'),
(3137, 'NOVA BASSANO', 'RS'),
(3138, 'NOVA BELÉM', 'MG'),
(3139, 'NOVA BOA BISTA', 'RS'),
(3140, 'NOVA BOA VISTA', 'RS'),
(3141, 'NOVA BRASILÂNDIA', 'MT'),
(3142, 'NOVA BRAZILÂNDIA DO OESTE', 'RO'),
(3143, 'NOVA BRÉSCIA', 'RS'),
(3144, 'NOVA CAMPINA', 'SP'),
(3145, 'NOVA CANAÃ', 'BA'),
(3146, 'NOVA CANAÃ DO NORTE', 'MT'),
(3147, 'NOVA CANAÃ PAULISTA', 'SP'),
(3148, 'NOVA CANDELÁRIA', 'RS'),
(3149, 'NOVA CANTU', 'PR'),
(3150, 'NOVA CASTILHO', 'SP'),
(3151, 'NOVA COLINAS', 'MA'),
(3152, 'NOVA CRIXAS', 'GO'),
(3153, 'NOVA CRUZ', 'RN'),
(3154, 'NOVA ERA', 'MG'),
(3155, 'NOVA ERECHIM', 'SC'),
(3156, 'NOVA ESPERANÇA', 'PR'),
(3157, 'NOVA ESPERANÇA DO PIRIÁ', 'PA'),
(3158, 'NOVA ESPERANÇA DO SUL', 'RS'),
(3159, 'NOVA ESPERANÇA DO SUDOESTE', 'PR'),
(3160, 'NOVA EUROPA', 'SP'),
(3161, 'NOVA FÁTIMA', 'BA'),
(3162, 'NOVA FLORESTA', 'PB'),
(3163, 'NOVA FRIBURGO', 'RJ'),
(3164, 'NOVA GLÓRIA', 'GO'),
(3165, 'NOVA GRANADA', 'SP'),
(3166, 'NOVA GUARITA', 'MT'),
(3167, 'NOVA GUATAPORANGA', 'SP'),
(3168, 'NOVA HARTZ', 'RS'),
(3169, 'NOVA IBIA', 'BA'),
(3170, 'NOVA IGUAÇU', 'GO'),
(3171, 'NOVA IGUAÇU DE GOIÁS', 'GO'),
(3172, 'NOVA INDEPENDÊNCIA', 'SP'),
(3173, 'NOVA IORQUE', 'MA'),
(3174, 'NOVA IPIXUNA', 'PA'),
(3175, 'NOVA ITABERABA', 'SC'),
(3176, 'NOVA ITARANA', 'BA'),
(3177, 'NOVA LACERDA', 'MT'),
(3178, 'NOVA LARANJEIRAS', 'PR'),
(3179, 'NOVA LIMA', 'MG'),
(3180, 'NOVA LONDRINA', 'PR'),
(3181, 'NOVA LUZITANIA', 'SP'),
(3182, 'NOVA MAMORÉ', 'RO'),
(3183, 'NOVA MARILÂNDIA', 'MT'),
(3184, 'NOVA MARINGÁ', 'MT'),
(3185, 'NOVA MODICA', 'MG'),
(3186, 'NOVA MONTE VERDE', 'MT'),
(3187, 'NOVA MUTUM', 'MT'),
(3188, 'NOVA ODESSA', 'SP'),
(3189, 'NOVA OLÍMPIA', 'MT'),
(3190, 'NOVA OLÍMPIA', 'PR'),
(3191, 'NOVA OLINDA', 'CE'),
(3192, 'NOVA OLINDA DO MARANHÃO', 'MA'),
(3193, 'NOVA OLINDA DO NORTE', 'AM'),
(3194, 'NOVA PÁDUA', 'RS'),
(3195, 'NOVA PALMA', 'RS'),
(3196, 'NOVA PALMEIRA', 'PB'),
(3197, 'NOVA PETRÓPOLIS', 'RS'),
(3198, 'NOVA PONTE', 'MG'),
(3199, 'NOVA PORTEIRINHA', 'MG'),
(3200, 'NOVA PRATA', 'RS'),
(3201, 'NOVA PRATA DO IGUAÇU', 'PR'),
(3202, 'NOVA RAMADA', 'RS'),
(3203, 'NOVA REDENCÃO', 'BA'),
(3204, 'NOVA RESENDE', 'MG'),
(3205, 'NOVA ROMA', 'GO'),
(3206, 'NOVA ROMA DO SUL', 'RS'),
(3207, 'NOVA ROSALÂNDIA', 'TO'),
(3208, 'NOVA RUSSAS', 'CE'),
(3209, 'NOVA SANTA BÁRBARA', 'PR'),
(3210, 'NOVA SANTA RITA', 'PI'),
(3211, 'NOVA SANTA ROSA', 'PR'),
(3212, 'NOVA SERRANA', 'MG'),
(3213, 'NOVA SOURE', 'BA'),
(3214, 'NOVA TEBAS', 'PR'),
(3215, 'NOVA TIMBOTEUA', 'PA'),
(3216, 'NOVA TRENTO', 'SC'),
(3217, 'NOVA UBIRATAN', 'MT'),
(3218, 'NOVA UNIÃO', 'MG'),
(3219, 'NOVA VENÉCIA', 'ES'),
(3220, 'NOVA VENEZA', 'GO'),
(3221, 'NOVA VIÇOSA', 'BA'),
(3222, 'NOVA XAVANTINA', 'MT'),
(3223, 'NOVAIS', 'SP'),
(3224, 'NOVO ACORDO', 'TO'),
(3225, 'NOVO AIRÃO', 'AM'),
(3226, 'NOVO ALEGRE', 'TO'),
(3227, 'NOVO ARIPUANÃ', 'AM'),
(3228, 'NOVO BARREIRO', 'RS'),
(3229, 'NOVO BRASIL', 'GO'),
(3230, 'NOVO CABRAIS', 'RS'),
(3231, 'NOVO CRUZEIRO', 'MG'),
(3232, 'NOVO GAMA', 'GO'),
(3233, 'NOVO HAMBURGO', 'RS'),
(3234, 'NOVO HORIZONTE', 'BA'),
(3235, 'NOVO HORIZONTE', 'SP'),
(3236, 'NOVO HORIZONTE DO NORTE', 'MT'),
(3237, 'NOVO HORIZONTE DO OESTE', 'RO'),
(3238, 'NOVO HORIZONTE DO SUL', 'MS'),
(3239, 'NOVO ITACOLOMI', 'PR'),
(3240, 'NOVO JARDIM', 'TO'),
(3241, 'NOVO LINO', 'AL'),
(3242, 'NOVO MACHADO', 'RS'),
(3243, 'NOVO MUNDO', 'MT'),
(3244, 'NOVO ORIENTE', 'CE'),
(3245, 'NOVO ORIENTE DE MINAS', 'MG'),
(3246, 'NOVO ORIENTE DO PIAUÍ', 'PI'),
(3247, 'NOVO PLANALTO', 'GO'),
(3248, 'NOVO PROGRESSO', 'PA'),
(3249, 'NOVO REPARTIMENTO', 'PA'),
(3250, 'NOVO SANTO ANTÔNIO', 'PI'),
(3251, 'NOVO SÃO JOAQUIM', 'MT'),
(3252, 'NOVO TIRADENTES', 'RS'),
(3253, 'NOVO TRIUNFO', 'BA'),
(3254, 'NOVO XINGU', 'RS'),
(3255, 'NOVORIZANTE', 'MG'),
(3256, 'NUPORANGA', 'SP'),
(3257, 'ÓBIDOS', 'PA'),
(3258, 'OCARA', 'CE'),
(3259, 'OCAUÇU', 'SP'),
(3260, 'OEIRAS', 'PI'),
(3261, 'OEIRAS DO PARA', 'PA'),
(3262, 'OIAPOQUE', 'AP'),
(3263, 'OLARIA', 'MG'),
(3264, 'ÓLEO', 'SP'),
(3265, 'OLHO D''ÁGUA', 'PB'),
(3266, 'OLHO D''ÁGUA DAS CUNHAS', 'MA'),
(3267, 'OLHO D''ÁGUA DAS FLORES', 'AL'),
(3268, 'OLHO D''ÁGUA DO CASADO', 'AL'),
(3269, 'OLHO D''ÁGUA DO PIAUÍ', 'PI'),
(3270, 'OLHO D''ÁGUA DOS BORGES', 'RN'),
(3271, 'OLIMPIA', 'SP'),
(3272, 'OLIMPIO NORONHA', 'MG'),
(3273, 'OLINDA', 'PE'),
(3274, 'OLINDA NOVA DO MARANHÃO', 'MA'),
(3275, 'OLINDINA', 'BA'),
(3276, 'OLIVEDOS', 'PB'),
(3277, 'OLIVEIRA', 'MG'),
(3278, 'OLIVEIRA DE FÁTIMA', 'TO'),
(3279, 'OLIVEIRA DOS BREJINHOS', 'BA'),
(3280, 'OLIVEIRA FORTES', 'MG'),
(3281, 'OLIVENÇA', 'AL'),
(3282, 'ONÇA DO PITANGUI', 'MG'),
(3283, 'ONDA VERDE', 'SP'),
(3284, 'ORATÓRIOS', 'MG'),
(3285, 'ORIENTE', 'SP'),
(3286, 'ORINDIUVA', 'SP'),
(3287, 'ORIXIMINA DO NORTE', 'PA'),
(3288, 'ORIZONA', 'GO'),
(3289, 'ORLÂNDIA', 'SP'),
(3290, 'ORLEANS', 'SC'),
(3291, 'OROBÓ', 'PE'),
(3292, 'OROCÓ', 'PE'),
(3293, 'OROS', 'CE'),
(3294, 'ORTIGUEIRA', 'PR'),
(3295, 'OSASCO', 'SP'),
(3296, 'OSCAR BRESSANE', 'SP'),
(3297, 'OSÓRIO', 'RS'),
(3298, 'OSVALDO CRUZ', 'SP'),
(3299, 'OTACÍLIO COSTA', 'SC'),
(3300, 'OUREM', 'PA'),
(3301, 'OURICANGAS', 'BA'),
(3302, 'OURICURI', 'PE'),
(3303, 'OURILÂNDIA DO NORTE', 'PA'),
(3304, 'OURINHOS', 'SP'),
(3305, 'OURIZONA', 'PR'),
(3306, 'OURO', 'SC'),
(3307, 'OURO BRANCO', 'AL'),
(3308, 'OURO BRANCO', 'RN'),
(3309, 'OURO FINO', 'MG'),
(3310, 'OURO PRETO', 'MG'),
(3311, 'OURO PRETO DO OESTE', 'RO'),
(3312, 'OURO VELHO', 'PB'),
(3313, 'OURO VERDE', 'SP'),
(3314, 'OURO VERDE DE GOIÁS', 'GO'),
(3315, 'OURO VERDE DE MINAS', 'MG'),
(3316, 'OURO VERDE DO OESTE', 'PR'),
(3317, 'OUROESTE', 'SP'),
(3318, 'OUROLÂNDIA', 'BA'),
(3319, 'OUVIDOR', 'GO'),
(3320, 'PACAEMBU', 'SP'),
(3321, 'PACAJA', 'PA'),
(3322, 'PACAJUS', 'CE'),
(3323, 'PACARAIMA', 'RR'),
(3324, 'PACATUBA', 'CE'),
(3325, 'PACO DO LUMIAR', 'MA'),
(3326, 'PACOTI', 'CE'),
(3327, 'PAÇUJA', 'CE'),
(3328, 'PADRE BERNARDO', 'GO'),
(3329, 'PADRE CARVALHO', 'MG'),
(3330, 'PADRE MARCOS', 'PI'),
(3331, 'PADRE PARAISO', 'MG'),
(3332, 'PAES LANDIM', 'PI'),
(3333, 'PAI PEDRO', 'MG'),
(3334, 'PAIAL', 'SC'),
(3335, 'PAIÇANDU', 'PR'),
(3336, 'PAIM FILHO', 'RS'),
(3337, 'PAINEIRAS', 'MG'),
(3338, 'PAINEL', 'SC'),
(3339, 'PAINS', 'MG'),
(3340, 'PAIVA', 'MG'),
(3341, 'PAJEU DO PIAUÍ', 'PI'),
(3342, 'PALESTINA', 'AL'),
(3343, 'PALESTINA', 'SP'),
(3344, 'PALESTINA DE GOIÁS', 'GO'),
(3345, 'PALESTINA DO PARÁ', 'PA'),
(3346, 'PALHANO', 'CE'),
(3347, 'PALHOÇA', 'SC'),
(3348, 'PALMA', 'MG'),
(3349, 'PALMA DO TOCANTINS', 'TO'),
(3350, 'PALMA SOLA', 'SC'),
(3351, 'PALMACIA', 'CE'),
(3352, 'PALMARES', 'PE'),
(3353, 'PALMARES DO SUL', 'RS'),
(3354, 'PALMARES PAULISTA', 'SP'),
(3355, 'PALMAS', 'PR'),
(3356, 'PALMAS', 'TO'),
(3357, 'PALMAS DE MONTE ALTO', 'BA'),
(3358, 'PALMEIRA', 'PR'),
(3359, 'PALMEIRA DAS MISSÕES', 'RS'),
(3360, 'PALMEIRA DO OESTE', 'SP'),
(3361, 'PALMEIRA DO PIAUÍ', 'PI'),
(3362, 'PALMEIRA DOS ÍNDIOS', 'AL'),
(3363, 'PALMEIRAÍS', 'PI'),
(3364, 'PALMEIRÂNDIA', 'MA'),
(3365, 'PALMEIRANTE', 'TO'),
(3366, 'PALMEIRAS', 'BA'),
(3367, 'PALMEIRAS DE GOIÁS', 'GO'),
(3368, 'PALMEIRAS DO TOCANTINS', 'TO'),
(3369, 'PALMERINA', 'PE'),
(3370, 'PALMEIRÓPOLIS', 'TO'),
(3371, 'PALMELO', 'GO'),
(3372, 'PALMINÓPOLIS', 'GO'),
(3373, 'PALMITAL', 'PR'),
(3374, 'PALMITINHO', 'RS'),
(3375, 'PALMITOS', 'SC'),
(3376, 'PALMÓPOLIS', 'MG'),
(3377, 'PALOTINA', 'PR'),
(3378, 'PANAMÁ', 'GO'),
(3379, 'PANAMBI', 'RS'),
(3380, 'PANCAS', 'ES'),
(3381, 'PANELAS', 'PE'),
(3382, 'PANORAMA', 'SP'),
(3383, 'PANTANA GRANDE', 'RS'),
(3384, 'PÂNTANO GRANDE', 'RS'),
(3385, 'PÃO DE AÇUCAR', 'AL'),
(3386, 'PAPAGAIOS', 'MG'),
(3387, 'PAPANDUVA', 'SC'),
(3388, 'PAQUETA', 'PI'),
(3389, 'PARÁ DE MINAS', 'MG'),
(3390, 'PARACAMBI', 'RJ'),
(3391, 'PARACATU', 'MG'),
(3392, 'PARAÇURU', 'CE'),
(3393, 'PARAGOMINAS', 'PA'),
(3394, 'PARÁGUAÇU', 'MG'),
(3395, 'PARÁGUAÇU PAULISTA', 'SP'),
(3396, 'PARAÍ', 'RS'),
(3397, 'PARAÍBA DO SUL', 'RJ'),
(3398, 'PARAÍBANO', 'MA'),
(3399, 'PARAIBUNA', 'SP'),
(3400, 'PARAIPABA', 'CE'),
(3401, 'PARAISO', 'SC'),
(3402, 'PARAÍSO', 'SP'),
(3403, 'PARAISO DO NORTE', 'PR'),
(3404, 'PARAISO DO SUL', 'RS'),
(3405, 'PARAISO DO TOCANTINS', 'TO'),
(3406, 'PARAISÓPOLIS', 'MG'),
(3407, 'PARAMBU', 'CE'),
(3408, 'PARAMIRIM', 'BA'),
(3409, 'PARAMOTI', 'CE'),
(3410, 'PARANÁ', 'RN'),
(3411, 'PARANÁCITY', 'PR'),
(3412, 'PARANAGUÁ', 'PR'),
(3413, 'PARANÁIBA', 'MS'),
(3414, 'PARANÁIGUARA', 'GO'),
(3415, 'PARANAÍTA', 'MT'),
(3416, 'PARANÁPANEMA', 'SP'),
(3417, 'PARANAPOEMA', 'PR'),
(3418, 'PARANAPUA', 'SP'),
(3419, 'PARANÁTAMA', 'PE'),
(3420, 'PARANÁTINGA', 'MT'),
(3421, 'PARANAVAÍ', 'PR'),
(3422, 'PARANHOS', 'MS'),
(3423, 'PARAOPEBA', 'MG'),
(3424, 'PARAPUA', 'SP'),
(3425, 'PARARI', 'PB'),
(3426, 'PARATINGA', 'BA'),
(3427, 'PARATY', 'RJ'),
(3428, 'PARAU', 'RN'),
(3429, 'PARAUAPEBAS', 'PA'),
(3430, 'PARAUNA', 'GO'),
(3431, 'PARAZINHO', 'RN'),
(3432, 'PARDINHO', 'SP'),
(3433, 'PARECI NOVO', 'RS'),
(3434, 'PARECIS', 'RO'),
(3435, 'PARELHAS', 'RN'),
(3436, 'PARICONHA', 'AL'),
(3437, 'PARINTINS', 'AM'),
(3438, 'PARIPIRANGA', 'BA'),
(3439, 'PARIPUEIRA', 'AL'),
(3440, 'PARIQUERA-AÇU', 'SP'),
(3441, 'PARISI', 'SP'),
(3442, 'PARNÁGUA', 'PI'),
(3443, 'PARNAIBA', 'PI'),
(3444, 'PARNAMIRIM', 'PE'),
(3445, 'PARNARAMA', 'MA'),
(3446, 'PAROBÉ', 'RS'),
(3447, 'PASSA E FICA', 'RN'),
(3448, 'PASSA QUATRO', 'MG'),
(3449, 'PASSA SETE', 'RS'),
(3450, 'PASSA TEMPO', 'MG'),
(3451, 'PASSA VINTE', 'MG'),
(3452, 'PASSABÉM', 'MG'),
(3453, 'PASSAGEM', 'PB'),
(3454, 'PASSAGEM', 'RN'),
(3455, 'PASSAGEM FRANCA', 'MA'),
(3456, 'PASSAGEM FRANCA DO PIAUÍ', 'PI'),
(3457, 'PASSIRA', 'PE'),
(3458, 'PASSO DE CAMARAGIBE', 'AL'),
(3459, 'PASSO DE TORRES', 'SC'),
(3460, 'PASSO DO SOBRADO', 'RS'),
(3461, 'PASSO FUNDO', 'RS'),
(3462, 'PASSOS', 'MG'),
(3463, 'PASSOS MAIA', 'SC'),
(3464, 'PASTOS BONS', 'MA'),
(3465, 'PATO BRAGADO', 'PR'),
(3466, 'PATO BRANCO', 'PR'),
(3467, 'PATOS', 'PB'),
(3468, 'PATOS DE MINAS', 'MG'),
(3469, 'PATOS DO PIAUÍ', 'PI'),
(3470, 'PATROCINIO', 'MG'),
(3471, 'PATROCÍNIO DE MURIAÉ', 'MG'),
(3472, 'PATROCINIO PAULISTA', 'SP'),
(3473, 'PATU', 'RN'),
(3474, 'PATY DO ALFERES', 'RJ'),
(3475, 'PAU BRASIL', 'BA'),
(3476, 'PAU D''ARCO', 'PA'),
(3477, 'PAU DOS FERROS', 'RN'),
(3478, 'PAUDALHO', 'PE'),
(3479, 'PAUINI', 'AM'),
(3480, 'PAULA CÂNDIDO', 'MG'),
(3481, 'PAULA FREITAS', 'PR'),
(3482, 'PAULICÉIA', 'SP'),
(3483, 'PAULINIA', 'SP'),
(3484, 'PAULINO NEVES', 'MA'),
(3485, 'PAULISTA', 'PB'),
(3486, 'PAULISTANA', 'PI'),
(3487, 'PAULISTANIA', 'SP'),
(3488, 'PAULISTAS', 'MG'),
(3489, 'PAULO AFONSO', 'BA'),
(3490, 'PAULO DE FARIA', 'SP'),
(3491, 'PAULO FRONTIN', 'PR'),
(3492, 'PAULO JACINTO', 'AL'),
(3493, 'PAULO LOPES', 'SC'),
(3494, 'PAULO RAMOS', 'MA'),
(3495, 'PAVÃO', 'MG'),
(3496, 'PAVERAMA', 'RS'),
(3497, 'PAVUSSU', 'PI'),
(3498, 'PÉ DE SERRA', 'BA'),
(3499, 'PEABIRU', 'PR'),
(3500, 'PEÇANHA', 'MG'),
(3501, 'PEDERNEIRAS', 'SP'),
(3502, 'PEDRA', 'PE'),
(3503, 'PEDRA AZUL', 'MG'),
(3504, 'PEDRA BELA', 'SP'),
(3505, 'PEDRA BRANCA', 'CE'),
(3506, 'PEDRA BRANCA DO AMAPARI', 'AP'),
(3507, 'PEDRA DO ANTA', 'MG'),
(3508, 'PEDRA DO INDAIA', 'MG'),
(3509, 'PEDRA DOURADA', 'MG'),
(3510, 'PEDRA GRANDE', 'RN'),
(3511, 'PEDRA LAVRADA', 'PB'),
(3512, 'PEDRA MOLE', 'SE'),
(3513, 'PEDRA PRETA', 'MT'),
(3514, 'PEDRALVA', 'MG'),
(3515, 'PEDRANÓPOLIS', 'SP'),
(3516, 'PEDRÃO', 'BA'),
(3517, 'PEDRAS DE FOGO', 'PB'),
(3518, 'PEDRAS DE MARIA DA CRUZ', 'MG'),
(3519, 'PEDRAS GRANDES', 'SC'),
(3520, 'PEDREGULHO', 'SP'),
(3521, 'PEDREIRA', 'SP'),
(3522, 'PEDREIRAS', 'MA'),
(3523, 'PEDRINHAS', 'SE'),
(3524, 'PEDRINHAS PAULISTA', 'SP'),
(3525, 'PEDRINÓPOLIS', 'MG'),
(3526, 'PEDRO AFONSO', 'TO'),
(3527, 'PEDRO ALEXANDRÉ', 'BA'),
(3528, 'PEDRO AVELINO', 'RN'),
(3529, 'PEDRO CANÁRIO', 'ES'),
(3530, 'PEDRO DE TOLEDO', 'SP'),
(3531, 'PEDRO DO ROSÁRIO', 'MA'),
(3532, 'PEDRO GOMES', 'MS'),
(3533, 'PEDRO II', 'PI'),
(3534, 'PEDRO LAURENTINO', 'PI'),
(3535, 'PEDRO LEOPOLDO', 'MG'),
(3536, 'PEDRO OSÓRIO', 'RS'),
(3537, 'PEDRO REGIS', 'PB'),
(3538, 'PEDRO TEIXEIRA', 'MG'),
(3539, 'PEDRO VELHO', 'RN'),
(3540, 'PEIXE', 'TO'),
(3541, 'PEIXE-BOI', 'PA'),
(3542, 'PEIXOTO DE AZEVEDO', 'MT'),
(3543, 'PEJUCARA', 'RS'),
(3544, 'PELOTAS', 'RS'),
(3545, 'PENAFORTE', 'CE'),
(3546, 'PENALVA', 'MA'),
(3547, 'PENÁPOLIS', 'SP'),
(3548, 'PENEDO', 'AL'),
(3549, 'PENHA', 'SC'),
(3550, 'PENTECOSTE', 'CE'),
(3551, 'PEQUERI', 'MG'),
(3552, 'PEQUI', 'MG'),
(3553, 'PEQUIZEIRO', 'TO'),
(3554, 'PERDIGAO', 'MG'),
(3555, 'PERDIZES', 'MG'),
(3556, 'PERDÕES', 'MG'),
(3557, 'PEREIRA BARRETO', 'SP'),
(3558, 'PEREIRAS', 'SP'),
(3559, 'PEREIRO', 'CE'),
(3560, 'PERI-MIRIM', 'MA'),
(3561, 'PERIQUITO', 'MG'),
(3562, 'PERITIBA', 'SC'),
(3563, 'PERITORO', 'MA'),
(3564, 'PEROBAL', 'PR'),
(3565, 'PEROLA', 'PR'),
(3566, 'PEROLA  D`OESTE', 'PR'),
(3567, 'PEROLÂNDIA', 'GO'),
(3568, 'PERUIBE', 'SP'),
(3569, 'PESCADOR', 'MG'),
(3570, 'PESQUEIRA', 'PE'),
(3571, 'PETROLÂNDIA', 'PE'),
(3572, 'PETROLINA', 'PE'),
(3573, 'PETROLINA DE GOIÁS', 'GO'),
(3574, 'PETRÓPOLIS', 'RJ'),
(3575, 'PIACABUCU', 'AL'),
(3576, 'PIACATU', 'SP'),
(3577, 'PIANCÓ', 'PB'),
(3578, 'PIATÃ', 'BA'),
(3579, 'PIAU', 'MG'),
(3580, 'PICADA CAFE', 'RS'),
(3581, 'PICARRA', 'PA'),
(3582, 'PIÇARRAS', 'SC'),
(3583, 'PICOS', 'PI'),
(3584, 'PICUI', 'PB'),
(3585, 'PIEDADE', 'SP'),
(3586, 'PIEDADE DE CARATINGA', 'MG'),
(3587, 'PIEDADE DE PONTE NOVA', 'MG'),
(3588, 'PIEDADE DO RIO GRANDE', 'MG'),
(3589, 'PIEDADE DOS GERAIS', 'MG'),
(3590, 'PIEN', 'PR'),
(3591, 'PILÃO ARCADO', 'BA'),
(3592, 'PILAR', 'AL'),
(3593, 'PILAR', 'PB'),
(3594, 'PILAR DE GOIÁS', 'GO'),
(3595, 'PILAR DO SUL', 'SP'),
(3596, 'PILOES', 'PB'),
(3597, 'PILÕES', 'RN'),
(3598, 'PILÕEZINHOS', 'PB'),
(3599, 'PIMENTA', 'MG'),
(3600, 'PIMENTA BUENO', 'RO'),
(3601, 'PIMENTEIRAS', 'PI'),
(3602, 'PIMENTEIRAS DO OESTE', 'RO'),
(3603, 'PINDAI (OURO BRANCO)', 'BA'),
(3604, 'PINDAMONHANGABA', 'SP'),
(3605, 'PINDARÉ MIRIM', 'MA'),
(3606, 'PINDOBAÇU', 'BA'),
(3607, 'PINDORAMA', 'SP'),
(3608, 'PINDORAMA DO TOCANTINS', 'TO'),
(3609, 'PINDORETAMA', 'CE'),
(3610, 'PINHAIS', 'PR'),
(3611, 'PINHAL', 'RS'),
(3612, 'PINHAL DA SERRA', 'RS'),
(3613, 'PINHAL DE SÃO BENTO', 'PR'),
(3614, 'PINHAL GRANDE', 'RS'),
(3615, 'PINHALÃO', 'PR'),
(3616, 'PINHALZINHO', 'SP'),
(3617, 'PINHÃO', 'PR'),
(3618, 'PINHÃO', 'SE'),
(3619, 'PINHEIRAL', 'RJ'),
(3620, 'PINHEIRINHO DO VALE', 'RS'),
(3621, 'PINHEIRO', 'MA'),
(3622, 'PINHEIRO MACHADO', 'RS'),
(3623, 'PINHEIRO PRETO', 'SC'),
(3624, 'PINTADAS', 'BA'),
(3625, 'PIO IX', 'PI'),
(3626, 'PIO XII', 'MA'),
(3627, 'PIQUEROBI', 'SP'),
(3628, 'PIQUET CARNEIRO', 'CE'),
(3629, 'PIQUETE', 'SP'),
(3630, 'PIRACAIA', 'SP'),
(3631, 'PIRACANJUBA', 'GO'),
(3632, 'PIRACEMA', 'MG'),
(3633, 'PIRACICABA', 'SP'),
(3634, 'PIRAÇURUCA', 'PI'),
(3635, 'PIRAÍ', 'RJ'),
(3636, 'PIRAÍ DO NORTE', 'BA'),
(3637, 'PIRAÍ DO SUL', 'PR'),
(3638, 'PIRAJU', 'SP'),
(3639, 'PIRAJUBA', 'MG'),
(3640, 'PIRAJUI', 'SP'),
(3641, 'PIRAMBU', 'SE'),
(3642, 'PIRANGA', 'MG'),
(3643, 'PIRANGI', 'SP'),
(3644, 'PIRANGUCU', 'MG'),
(3645, 'PIRANGUINHO', 'MG'),
(3646, 'PIRANHAS', 'AL'),
(3647, 'PIRAPEMAS', 'MA'),
(3648, 'PIRAPETINGA', 'MG'),
(3649, 'PIRAPO', 'RS'),
(3650, 'PIRAPORA', 'MG'),
(3651, 'PIRAPORA DO BOM JESUS', 'SP'),
(3652, 'PIRAPOZINHO', 'SP'),
(3653, 'PIRAQUARA', 'PR'),
(3654, 'PIRAQUE', 'TO'),
(3655, 'PIRASSUNUNGA', 'SP'),
(3656, 'PIRATINI', 'RS'),
(3657, 'PIRATININGA', 'SP'),
(3658, 'PIRATUBA', 'SC'),
(3659, 'PIRAÚBA', 'MG'),
(3660, 'PIRENÓPOLIS', 'GO'),
(3661, 'PIRES DO RIO', 'GO'),
(3662, 'PIRES FERREIRA', 'CE'),
(3663, 'PIRIPA', 'BA'),
(3664, 'PIRIPIRI', 'PI'),
(3665, 'PIRPIRITUBA', 'PB'),
(3666, 'PITANGA', 'PR'),
(3667, 'PITANGUEIRAS', 'PR'),
(3668, 'PITANGUI', 'MG'),
(3669, 'PITIMBU', 'PB'),
(3670, 'PIUM', 'TO'),
(3671, 'PIUMA', 'ES'),
(3672, 'PIUM-I', 'MG'),
(3673, 'PLACAS', 'PA'),
(3674, 'PLÁCIDO DE CASTRO', 'AC'),
(3675, 'PLANALTINA', 'GO'),
(3676, 'PLANALTINA DO PARANÁ', 'PR'),
(3677, 'PLANALTINO', 'BA'),
(3678, 'PLANALTO', 'BA'),
(3679, 'PLANALTO ALEGRE', 'SC'),
(3680, 'PLANALTO DA SERRA', 'MT'),
(3681, 'PLANURA', 'MG'),
(3682, 'PLATINA', 'SP'),
(3683, 'POA', 'SP'),
(3684, 'POÇÃO', 'PE'),
(3685, 'POÇÃO DE PEDRAS', 'MA'),
(3686, 'POCINHOS', 'PB'),
(3687, 'POÇO BRANCO', 'RN'),
(3688, 'POÇO DANTAS', 'PB'),
(3689, 'POÇO DAS ANTAS', 'RS'),
(3690, 'POÇO DAS TRINCHEIRAS', 'AL'),
(3691, 'POÇO DE JOSÉ DE MOURA', 'PB'),
(3692, 'POÇO FUNDO', 'MG'),
(3693, 'POÇO REDONDO', 'SE'),
(3694, 'POÇO VERDE', 'SE'),
(3695, 'POÇÕES', 'BA'),
(3696, 'POCONÉ', 'MT'),
(3697, 'POÇOS DE CALDAS', 'MG'),
(3698, 'POCRANE', 'MG'),
(3699, 'POJUCA', 'BA'),
(3700, 'POLONI', 'SP'),
(3701, 'POMBAL', 'PB'),
(3702, 'POMBOS', 'PE'),
(3703, 'POMERODE', 'SC'),
(3704, 'POMPÉIA', 'SP'),
(3705, 'POMPÉU', 'MG'),
(3706, 'PONGAÍ', 'SP'),
(3707, 'PONTA DE PEDRAS', 'PA'),
(3708, 'PONTA GROSSA', 'PR'),
(3709, 'PONTALINDA', 'SP'),
(3710, 'PONTA PORÃ', 'MS'),
(3711, 'PONTAL', 'SP'),
(3712, 'PONTAL DO ARAGUAIA', 'MT'),
(3713, 'PONTAL DO PARANÁ', 'PR'),
(3714, 'PONTALINA', 'GO'),
(3715, 'PONTÃO', 'RS'),
(3716, 'PONTE', 'MG'),
(3717, 'PONTE ALTA', 'SC'),
(3718, 'PONTE ALTA DO BOM JESUS', 'TO'),
(3719, 'PONTE ALTA DO NORTE', 'SC'),
(3720, 'PONTE ALTA DO TOCANTINS', 'TO'),
(3721, 'PONTE BRANCA', 'MT'),
(3722, 'PONTE NOVA', 'MG'),
(3723, 'PONTE PRETA', 'RS');
INSERT INTO `municipios` (`codigo`, `nome`, `uf`) VALUES
(3724, 'PONTE SERRADA', 'SC'),
(3725, 'PONTES E LACERDA', 'MT'),
(3726, 'PONTES GESTAL', 'SP'),
(3727, 'PONTO BELO', 'ES'),
(3728, 'PONTO CHIQUE', 'MG'),
(3729, 'PONTO DOS VOLANTES', 'MG'),
(3730, 'PONTO NOVO', 'BA'),
(3731, 'POPULINA', 'SP'),
(3732, 'PORANGA', 'CE'),
(3733, 'PORANGABA', 'CE'),
(3734, 'PORANGABA', 'SP'),
(3735, 'PORANGATU', 'GO'),
(3736, 'PORCIÚNCULA', 'RJ'),
(3737, 'PORECATU', 'PR'),
(3738, 'PORTALEGRE', 'RN'),
(3739, 'PORTÃO', 'RS'),
(3740, 'PORTEIRÃO', 'GO'),
(3741, 'PORTEIRAS', 'CE'),
(3742, 'PORTEIRINHA', 'MG'),
(3743, 'PORTEL', 'PA'),
(3744, 'PORTELÂNDIA', 'GO'),
(3745, 'PORTO', 'PI'),
(3746, 'PORTO ACRE', 'AC'),
(3747, 'PORTO ALEGRE', 'RS'),
(3748, 'PORTO ALEGRE DO NORTE', 'MT'),
(3749, 'PORTO ALEGRE DO PIAUÍ', 'PI'),
(3750, 'PORTO ALEGRE DO TOCANTINS', 'TO'),
(3751, 'PORTO AMAZONAS', 'PR'),
(3752, 'PORTO BARREIRO', 'PR'),
(3753, 'PORTO BELO', 'SC'),
(3754, 'PORTO CALVO', 'AL'),
(3755, 'PORTO DA FOLHA', 'SE'),
(3756, 'PORTO DE MOZ', 'PA'),
(3757, 'PORTO DE PEDRAS', 'AL'),
(3758, 'PORTO DO MANGUE', 'RN'),
(3759, 'PORTO DOS GAÚCHOS', 'MT'),
(3760, 'PORTO ESPERIDIÃO', 'MT'),
(3761, 'PORTO ESTRELA', 'MT'),
(3762, 'PORTO FELIZ', 'SP'),
(3763, 'PORTO FERREIRA', 'SP'),
(3764, 'PORTO FIRME', 'MG'),
(3765, 'PORTO FRANCO', 'MA'),
(3766, 'PORTO GRANDE', 'AP'),
(3767, 'PORTO LUCENA', 'RS'),
(3768, 'PORTO MAUA', 'RS'),
(3769, 'PORTO MURTINHO', 'MS'),
(3770, 'PORTO NACIONAL', 'TO'),
(3771, 'PORTO REAL', 'RJ'),
(3772, 'PORTO REAL DO COLÉGIO', 'AL'),
(3773, 'PORTO RICO', 'PR'),
(3774, 'PORTO RICO DO MARANHÃO', 'MA'),
(3775, 'PORTO SEGURO', 'BA'),
(3776, 'PORTO UNIÃO', 'SC'),
(3777, 'PORTO VELHO', 'RO'),
(3778, 'PORTO VERA CRUZ', 'RS'),
(3779, 'PORTO VITÓRIA', 'PR'),
(3780, 'PORTO WALTER', 'AC'),
(3781, 'PORTO XAVIER', 'RS'),
(3782, 'POSSE', 'GO'),
(3783, 'POTÉ', 'MG'),
(3784, 'POTENGI', 'CE'),
(3785, 'POTIM', 'SP'),
(3786, 'POTIRÁGUA', 'BA'),
(3787, 'POTIRENDABA', 'SP'),
(3788, 'POTIRETAMA', 'CE'),
(3789, 'POUSO ALEGRE', 'MG'),
(3790, 'POUSO ALTO', 'MG'),
(3791, 'POUSO NOVO', 'RS'),
(3792, 'POUSO REDONDO', 'SC'),
(3793, 'POXOREO', 'MT'),
(3794, 'PRACINHA', 'SP'),
(3795, 'PRACUÚBA', 'AP'),
(3796, 'PRADO', 'BA'),
(3797, 'PRADO FERREIRA', 'PR'),
(3798, 'PRADÓPOLIS', 'SP'),
(3799, 'PRADOS', 'MG'),
(3800, 'PRAIA GRANDE', 'SC'),
(3801, 'PRAIA NORTE', 'TO'),
(3802, 'PRAINHA', 'PA'),
(3803, 'PRANCHITA', 'PR'),
(3804, 'PRATA', 'MG'),
(3805, 'PRATA DO PIAUÍ', 'PI'),
(3806, 'PRATÂNIA', 'SP'),
(3807, 'PRATAPOLIS', 'MG'),
(3808, 'PRATINHA', 'MG'),
(3809, 'PREFEITO DE PINHÃO', 'SE'),
(3810, 'PRESIDENTE ALVES', 'SP'),
(3811, 'PRESIDENTE BERNARDES', 'MG'),
(3812, 'PRESIDENTE BERNARDES', 'SP'),
(3813, 'PRESIDENTE CASTELO BRANCO', 'PR'),
(3814, 'PRESIDENTE DUTRA', 'BA'),
(3815, 'PRESIDENTE EPITACIO', 'SP'),
(3816, 'PRESIDENTE FIGUEIREDO', 'AM'),
(3817, 'PRESIDENTE GETULIO', 'SC'),
(3818, 'PRESIDENTE JANIO QUADROS', 'BA'),
(3819, 'PRESIDENTE JUSCELINO', 'MA'),
(3820, 'PRESIDENTE KENNEDY', 'ES'),
(3821, 'PRESIDENTE KENNEDY', 'TO'),
(3822, 'PRESIDENTE KUBITSCHEK', 'MG'),
(3823, 'PRESIDENTE LUCENA', 'RS'),
(3824, 'PRESIDENTE MEDICI', 'MA'),
(3825, 'PRESIDENTE NEREU', 'SC'),
(3826, 'PRESIDENTE OLEGARIO', 'MG'),
(3827, 'PRESIDENTE PRUDENTE', 'SP'),
(3828, 'PRESIDENTE SARNEY', 'MA'),
(3829, 'PRESIDENTE TANCREDO NEVES', 'BA'),
(3830, 'PRESIDENTE VARGAS', 'MA'),
(3831, 'PRESIDENTE VENCESLAU', 'SP'),
(3832, 'PRIMAVERA', 'PA'),
(3833, 'PRIMAVERA', 'PE'),
(3834, 'PRIMAVERA DE RONDONIA', 'RO'),
(3835, 'PRIMAVERA DO LESTE', 'MT'),
(3836, 'PRIMEIRA CRUZ', 'MA'),
(3837, 'PRIMEIRO DE MAIO', 'PR'),
(3838, 'PRINCESA', 'SC'),
(3839, 'PRINCESA ISABEL', 'PB'),
(3840, 'PROFESSOR JAMIL', 'GO'),
(3841, 'PROGRESSO', 'RS'),
(3842, 'PROMISSÃO', 'SP'),
(3843, 'PROPRIA', 'SE'),
(3844, 'PROTÁSIO ALVES', 'RS'),
(3845, 'PRUDENTE DE MORAIS', 'MG'),
(3846, 'PRUDENTÓPOLIS', 'PR'),
(3847, 'PUGMIL', 'TO'),
(3848, 'PUREZA', 'RN'),
(3849, 'PUTINGA', 'RS'),
(3850, 'PUXINANA', 'PB'),
(3851, 'QUADRA', 'SP'),
(3852, 'QUARAI', 'RS'),
(3853, 'QUARTEL GERAL', 'MG'),
(3854, 'QUARTO CENTENARIO', 'PR'),
(3855, 'QUATA', 'SP'),
(3856, 'QUATIGUA', 'PR'),
(3857, 'QUATIPURU', 'PA'),
(3858, 'QUATIS', 'RJ'),
(3859, 'QUATRO BARRAS', 'PR'),
(3860, 'QUATRO PONTES', 'PR'),
(3861, 'QUEBRANGULO', 'AL'),
(3862, 'QUEDAS DO IGUAÇU', 'PR'),
(3863, 'QUEIMADA NOVA', 'PI'),
(3864, 'QUEIMADAS', 'BA'),
(3865, 'QUEIMADOS', 'RJ'),
(3866, 'QUEIROZ', 'SP'),
(3867, 'QUELUZ', 'SP'),
(3868, 'QUELUZITO', 'MG'),
(3869, 'QUERÊNCIA', 'MT'),
(3870, 'QUERÊNCIA DO NORTE', 'PR'),
(3871, 'QUEVEDOS', 'RS'),
(3872, 'QUIJINGUE', 'BA'),
(3873, 'QUILOMBO', 'SC'),
(3874, 'QUINTA DO SOL', 'PR'),
(3875, 'QUINTANA', 'SP'),
(3876, 'QUINZE DE NOVEMBRO', 'RS'),
(3877, 'QUIPAPA', 'PE'),
(3878, 'QUIRINÓPOLIS', 'GO'),
(3879, 'QUISSAMA', 'RJ'),
(3880, 'QUITANDINHA', 'PR'),
(3881, 'QUITERIANÓPOLIS', 'CE'),
(3882, 'QUIXABA', 'PB'),
(3883, 'QUIXABÁ', 'PE'),
(3884, 'QUIXABEIRA', 'BA'),
(3885, 'QUIXADA', 'CE'),
(3886, 'QUIXELO', 'CE'),
(3887, 'QUIXERAMOBIM', 'CE'),
(3888, 'QUIXERÉ', 'CE'),
(3889, 'RAFAEL FERNANDES', 'RN'),
(3890, 'RAFAEL GODEIRO', 'RN'),
(3891, 'RAFAEL JAMBEIRO', 'BA'),
(3892, 'RAFARD', 'SP'),
(3893, 'RAMILÂNDIA', 'PR'),
(3894, 'RANCHARIA', 'SP'),
(3895, 'RANCHO ALEGRE', 'PR'),
(3896, 'RANCHO ALEGRE DO OESTE', 'PR'),
(3897, 'RANCHO QUEIMADO', 'SC'),
(3898, 'RAPOSA', 'MA'),
(3899, 'RAPOSOS', 'MG'),
(3900, 'RAUL SOARES', 'MG'),
(3901, 'REALEZA', 'PR'),
(3902, 'REBOUCAS', 'PR'),
(3903, 'RECIFE', 'PE'),
(3904, 'RECREIO', 'MG'),
(3905, 'RECURSOLÂNDIA', 'TO'),
(3906, 'REDENCÃO', 'CE'),
(3907, 'REDENÇÃO', 'PA'),
(3908, 'REDENCÃO DA SERRA', 'SP'),
(3909, 'REDENCÃO DO GURGUELIA', 'PI'),
(3910, 'REDENTORA', 'RS'),
(3911, 'REGENERACÃO', 'PI'),
(3912, 'REGENTE FEIJO', 'SP'),
(3913, 'REGINÓPOLIS', 'SP'),
(3914, 'REGISTRO', 'SP'),
(3915, 'RELVADO', 'RS'),
(3916, 'REMANSO', 'BA'),
(3917, 'REMÍGIO', 'PB'),
(3918, 'RENASCENÇA', 'PR'),
(3919, 'RERIUTABA', 'CE'),
(3920, 'RESENDE', 'RJ'),
(3921, 'RESERVA', 'PR'),
(3922, 'RESERVA DO CABACAL', 'MT'),
(3923, 'RESPLENDOR', 'MG'),
(3924, 'RESSAQUINHA', 'MG'),
(3925, 'RESTINGA', 'SP'),
(3926, 'RESTINGA SECA', 'RS'),
(3927, 'RETIROLÂNDIA', 'BA'),
(3928, 'REZENDE COSTA', 'MG'),
(3929, 'RIACHAO', 'MA'),
(3930, 'RIACHAO DAS NEVES', 'BA'),
(3931, 'RIACHÃO DO DANTAS', 'SE'),
(3932, 'RIACHAO DO JAÇUIPE', 'BA'),
(3933, 'RIACHAO DO POÇO', 'PB'),
(3934, 'RIACHINHO', 'MG'),
(3935, 'RIACHINHO', 'TO'),
(3936, 'RIACHO DA CRUZ', 'RN'),
(3937, 'RIACHO DAS ALMAS', 'PE'),
(3938, 'RIACHO DE SANTANA', 'BA'),
(3939, 'RIACHO DE SANTO ANTÔNIO', 'PB'),
(3940, 'RIACHO DOS CAVALOS', 'PB'),
(3941, 'RIACHO DOS MACHADOS', 'MG'),
(3942, 'RIACHO FRIO', 'PI'),
(3943, 'RIACHUELO', 'RN'),
(3944, 'RIACHUELO', 'SE'),
(3945, 'RIALMA', 'GO'),
(3946, 'RIANÁPOLIS', 'GO'),
(3947, 'RIBAMAR FIQUENE', 'MA'),
(3948, 'RIBAS DO RIO PARDO', 'MS'),
(3949, 'RIBEIRA', 'SP'),
(3950, 'RIBEIRA DO AMPARO', 'BA'),
(3951, 'RIBEIRA DO PIAUÍ', 'PI'),
(3952, 'RIBEIRA DO POMBAL', 'BA'),
(3953, 'RIBEIRÃO', 'PE'),
(3954, 'RIBEIRÃO BONITO', 'SP'),
(3955, 'RIBEIRÃO BRANCO', 'SP'),
(3956, 'RIBEIRÃO CASCALHEIRA', 'MT'),
(3957, 'RIBEIRÃO CLARO', 'PR'),
(3958, 'RIBEIRÃO CORRENTE', 'SP'),
(3959, 'RIBEIRÃO DAS NEVES', 'MG'),
(3960, 'RIBEIRÃO DO LARGO', 'BA'),
(3961, 'RIBEIRÃO DO PINHAL', 'PR'),
(3962, 'RIBEIRÃO DO SUL', 'SP'),
(3963, 'RIBEIRÃO DOS ÍNDIOS', 'SP'),
(3964, 'RIBEIRÃO GRANDE', 'SP'),
(3965, 'RIBEIRÃO PIRES', 'SP'),
(3966, 'RIBEIRÃO PRETO', 'SP'),
(3967, 'RIBEIRÃO VERMELHO', 'MG'),
(3968, 'RIBEIRÃOZINHO', 'MT'),
(3969, 'RIBEIRO GONÇALVES', 'PI'),
(3970, 'RIBEIRÓPOLIS', 'SE'),
(3971, 'RIFAINA', 'SP'),
(3972, 'RINCÃO', 'SP'),
(3973, 'RINÓPOLIS', 'SP'),
(3974, 'RIO ACIMA', 'MG'),
(3975, 'RIO AZUL', 'PR'),
(3976, 'RIO BANANAL', 'ES'),
(3977, 'RIO BOM', 'PR'),
(3978, 'RIO BONITO', 'RJ'),
(3979, 'RIO BONITO DO IGUAÇU', 'PR'),
(3980, 'RIO BRANCO', 'AC'),
(3981, 'RIO BRANCO DO IVAÍ', 'PR'),
(3982, 'RIO BRANCO DO SUL', 'PR'),
(3983, 'RIO BRILHANTE', 'MS'),
(3984, 'RIO CASCA', 'MG'),
(3985, 'RIO CLARO', 'RJ'),
(3986, 'RIO CRESPO', 'RO'),
(3987, 'RIO DA CONCEIÇÃO', 'TO'),
(3988, 'RIO DAS ANTAS', 'SC'),
(3989, 'RIO DAS FLORES', 'RJ'),
(3990, 'RIO DAS OSTRAS', 'RJ'),
(3991, 'RIO DAS PEDRAS', 'SP'),
(3992, 'RIO DE CONTAS', 'BA'),
(3993, 'RIO DE JANEIRO', 'RJ'),
(3994, 'RIO DO ANTÔNIO', 'BA'),
(3995, 'RIO DO CAMPO', 'SC'),
(3996, 'RIO DO FOGO', 'RN'),
(3997, 'RIO DO OESTE', 'SC'),
(3998, 'RIO DO PIRES', 'BA'),
(3999, 'RIO DO PRADO', 'MG'),
(4000, 'RIO DO SUL', 'SC'),
(4001, 'RIO DOCE', 'MG'),
(4002, 'RIO DOS BOIS', 'TO'),
(4003, 'RIO DOS CEDROS', 'SC'),
(4004, 'RIO DOS ÍNDIOS', 'RS'),
(4005, 'RIO ESPERA', 'MG'),
(4006, 'RIO FORMOSO', 'PE'),
(4007, 'RIO FORTUNA', 'SC'),
(4008, 'RIO GRANDE', 'RS'),
(4009, 'RIO GRANDE DA SERRA', 'SP'),
(4010, 'RIO GRANDE DO PIAUÍ', 'PI'),
(4011, 'RIO LARGO', 'AL'),
(4012, 'RIO MANSO', 'MG'),
(4013, 'RIO MARIA', 'PA'),
(4014, 'RIO NEGRINHO', 'SC'),
(4015, 'RIO NEGRO', 'MS'),
(4016, 'RIO NEGRO', 'PR'),
(4017, 'RIO NOVO', 'MG'),
(4018, 'RIO NOVO DO SUL', 'ES'),
(4019, 'RIO PARANAÍBA', 'MG'),
(4020, 'RIO PARDO', 'RS'),
(4021, 'RIO PARDO DE MINAS', 'MG'),
(4022, 'RIO PIRACICABA', 'MG'),
(4023, 'RIO POMBA', 'MG'),
(4024, 'RIO PRETO', 'MG'),
(4025, 'RIO PRETO DA EVA', 'AM'),
(4026, 'RIO QUENTE', 'GO'),
(4027, 'RIO REAL', 'BA'),
(4028, 'RIO RUFINO', 'SC'),
(4029, 'RIO SONO', 'TO'),
(4030, 'RIO TINTO', 'PB'),
(4031, 'RIO VERDE', 'GO'),
(4032, 'RIO VERDE DE MATO GROSSO', 'MS'),
(4033, 'RIO VERMELHO', 'MG'),
(4034, 'RIOLÂNDIA', 'SP'),
(4035, 'RIOZINHO', 'RS'),
(4036, 'RIQUEZA', 'SC'),
(4037, 'RITAPOLIS', 'MG'),
(4038, 'RIVERSUL', 'SP'),
(4039, 'ROCA SALES', 'RS'),
(4040, 'ROCHEDO', 'MS'),
(4041, 'ROCHEDO DE MINAS', 'MG'),
(4042, 'RODEIO', 'SC'),
(4043, 'RODEIO BONITO', 'RS'),
(4044, 'RODEIRO', 'MG'),
(4045, 'RODELAS', 'BA'),
(4046, 'RODOLFO FERNANDES', 'RN'),
(4047, 'RODRIGUES ALVES', 'AC'),
(4048, 'ROLÂNDIA', 'PR'),
(4049, 'ROLANTE', 'RS'),
(4050, 'ROLIM DE MOURA', 'RO'),
(4051, 'ROMARIA', 'MG'),
(4052, 'ROMELÂNDIA', 'SC'),
(4053, 'RONCADOR', 'PR'),
(4054, 'RONDA ALTA', 'RS'),
(4055, 'RONDINHA', 'RS'),
(4056, 'RONDOLÂNDIA', 'MT'),
(4057, 'RONDON', 'PR'),
(4058, 'RONDON DO PARA', 'PA'),
(4059, 'RONDONÓPOLIS', 'MT'),
(4060, 'ROQUE GONZALES', 'RS'),
(4061, 'RORAINÓPOLIS', 'RR'),
(4062, 'ROSANA', 'SP'),
(4063, 'ROSÁRIO', 'MA'),
(4064, 'ROSARIO DA LIMEIRA', 'MG'),
(4065, 'ROSÁRIO DO CATETE', 'SE'),
(4066, 'ROSÁRIO DO IVAÍ', 'PR'),
(4067, 'ROSARIO DO SUL', 'RS'),
(4068, 'ROSARIO OESTE', 'MT'),
(4069, 'ROSEIRA', 'SP'),
(4070, 'ROTEIRO', 'AL'),
(4071, 'RUBELITA', 'MG'),
(4072, 'RUBIACEA', 'SP'),
(4073, 'RUBIATABA', 'GO'),
(4074, 'RUBIM', 'MG'),
(4075, 'RUBINÉIA', 'SP'),
(4076, 'RURÓPOLIS', 'PA'),
(4077, 'RUSSAS', 'CE'),
(4078, 'RUY BARBOSA', 'BA'),
(4079, 'SÃO DOMINGOS DE POMBAL', 'PB'),
(4080, 'SÃO SEBASTIÃO DO TOCANTINS', 'TO'),
(4081, 'SABARA', 'MG'),
(4082, 'SABAUDIA', 'PR'),
(4083, 'SABINO', 'SP'),
(4084, 'SABINÓPOLIS', 'MG'),
(4085, 'SABOEIRO', 'CE'),
(4086, 'SACRAMENTO', 'MG'),
(4087, 'SAGRADA FAMILIA', 'RS'),
(4088, 'SAGRES', 'SP'),
(4089, 'SAIRE', 'PE'),
(4090, 'SALDANHA MARINHO', 'RS'),
(4091, 'SALES', 'SP'),
(4092, 'SALES OLIVEIRA', 'SP'),
(4093, 'SALESÓPOLIS', 'SP'),
(4094, 'SALETE', 'SC'),
(4095, 'SALGADINHO', 'PB'),
(4096, 'SALGADO', 'SE'),
(4097, 'SALGADO DE SÃO FELIX', 'PB'),
(4098, 'SALGADO FILHO', 'PR'),
(4099, 'SALGUEIRO', 'PE'),
(4100, 'SALINAS', 'MG'),
(4101, 'SALINAS DA MARGARIDA', 'BA'),
(4102, 'SALINÓPOLIS', 'PA'),
(4103, 'SALITRE', 'CE'),
(4104, 'SALMOURÃO', 'SP'),
(4105, 'SALOÁ', 'PE'),
(4106, 'SALTINHO', 'SC'),
(4107, 'SALTO', 'SP'),
(4108, 'SALTO DA DIVISA', 'MG'),
(4109, 'SALTO DE PIRAPORA', 'SP'),
(4110, 'SALTO DO CÉU', 'MT'),
(4111, 'SALTO DO ITARARE', 'PR'),
(4112, 'SALTO DO JACUÍ', 'RS'),
(4113, 'SALTO DO LONTRA', 'PR'),
(4114, 'SALTO DO VELOSO', 'SC'),
(4115, 'SALTO GRANDE', 'SP'),
(4116, 'SALVADOR', 'BA'),
(4117, 'SALVADOR DAS MISSÕES', 'RS'),
(4118, 'SALVADOR DO SUL', 'RS'),
(4119, 'SALVATERRA', 'PA'),
(4120, 'SAMBAIBA', 'MA'),
(4121, 'SAMPAIO', 'TO'),
(4122, 'SANANDUVA', 'RS'),
(4123, 'SANCLERLÂNDIA', 'GO'),
(4124, 'SANDOLÂNDIA', 'TO'),
(4125, 'SANDOVALINA', 'SP'),
(4126, 'SANGAO', 'SC'),
(4127, 'SANHARO', 'PE'),
(4128, 'SANTA CRUZ DE SALINAS', 'MG'),
(4129, 'SANTA ADELIA', 'SP'),
(4130, 'SANTA ALBERTINA', 'SP'),
(4131, 'SANTA AMELIA', 'PR'),
(4132, 'SANTA BÁRBARA', 'BA'),
(4133, 'SANTA BÁRBARA', 'MG'),
(4134, 'SANTA BÁRBARA DE GOIÁS', 'GO'),
(4135, 'SANTA BÁRBARA DO LESTE', 'MG'),
(4136, 'SANTA BÁRBARA DO M. VERDE', 'MG'),
(4137, 'SANTA BÁRBARA DO PARÁ', 'PA'),
(4138, 'SANTA BÁRBARA DO SUL', 'RS'),
(4139, 'SANTA BÁRBARA DO TUGURIO', 'MG'),
(4140, 'SANTA BÁRBARA DO OESTE', 'SP'),
(4141, 'SANTA BRANCA', 'SP'),
(4142, 'SANTA BRIGIDA', 'BA'),
(4143, 'SANTA CARMEM', 'MT'),
(4144, 'SANTA CECILIA', 'SC'),
(4145, 'SANTA CECILIA DE UMBUZEIRO', 'PB'),
(4146, 'SANTA CECILIA DO PAVAO', 'PR'),
(4147, 'SANTA CLARA DO SUL', 'RS'),
(4148, 'SANTA CLARA DO OESTE', 'SP'),
(4149, 'SANTA CRISTO', 'RS'),
(4150, 'SANTA CRUZ', 'PB'),
(4151, 'SANTA CRUZ DO ESCALVADO', 'MG'),
(4152, 'SANTA CRUZ DA BAIXA VERDE', 'PE'),
(4153, 'SANTA CRUZ DA CABRALIA', 'BA'),
(4154, 'SANTA CRUZ DA CONCEIÇÃO', 'SP'),
(4155, 'SANTA CRUZ DA ESPERANÇA', 'SP'),
(4156, 'SANTA CRUZ DA VITÓRIA', 'BA'),
(4157, 'SANTA CRUZ DE GOIÁS', 'GO'),
(4158, 'SANTA CRUZ DO ARARI', 'PA'),
(4159, 'SANTA CRUZ DO CAPIBARIBE', 'PE'),
(4160, 'SANTA CRUZ DO MONTE CASTELO', 'PR'),
(4161, 'SANTA CRUZ DO PIAUÍ', 'PI'),
(4162, 'SANTA CRUZ DO SUL', 'BA'),
(4163, 'SANTA CRUZ DOS MILAGRES', 'PI'),
(4164, 'SANTA CRUZ DAS PALMEIRAS', 'SP'),
(4165, 'SANTA CRUZ DO RIO PARDO', 'SP'),
(4166, 'SANTA EFIGENIA DE MINAS', 'MG'),
(4167, 'SANTA ERNESTINA', 'SP'),
(4168, 'SANTA FÉ', 'PR'),
(4169, 'SANTA FÉ DE GOIÁS', 'GO'),
(4170, 'SANTA FÉ DE MINAS', 'MG'),
(4171, 'SANTA FÉ DO ARÁGUAIA', 'TO'),
(4172, 'SANTA FÉ DO SUL', 'SP'),
(4173, 'SANTA FILOMENA', 'PE'),
(4174, 'SANTA FILOMENA DO MARANHÃO', 'MA'),
(4175, 'SANTA GERTRUDES', 'SP'),
(4176, 'SANTA HELENA', 'MA'),
(4177, 'SANTA HELENA', 'PB'),
(4178, 'SANTA HELENA', 'PR'),
(4179, 'SANTA HELENA DE GOIÁS', 'GO'),
(4180, 'SANTA INES', 'BA'),
(4181, 'SANTA ISABEL', 'GO'),
(4182, 'SANTA ISABEL DO RIO NEGRO', 'AM'),
(4183, 'SANTA ISABEL DO IVAÍ', 'PR'),
(4184, 'SANTA IZABEL DO OESTE', 'PR'),
(4185, 'SANTA ISABEL DO PARÁ', 'PA'),
(4186, 'SANTA JULIANA', 'MG'),
(4187, 'SANTA LEOPOLDINA', 'ES'),
(4188, 'SANTA LUCIA', 'PR'),
(4189, 'SANTA LUCIA', 'SP'),
(4190, 'SANTA LUZ', 'BA'),
(4191, 'SANTA LUZIA', 'BA'),
(4192, 'SANTA LUZIA', 'MA'),
(4193, 'SANTA LUZIA DO ITANHY', 'SE'),
(4194, 'SANTA LUZIA DO NORTE', 'AL'),
(4195, 'SANTA LUZIA DO PARÁ', 'PA'),
(4196, 'SANTA LUZIA DO OESTE', 'RO'),
(4197, 'SANTA MARGARIDA', 'MG'),
(4198, 'SANTA MARIA', 'RN'),
(4199, 'SANTA MARIA DA BOA VISTA', 'PE'),
(4200, 'SANTA MARIA DA SERRA', 'SP'),
(4201, 'SANTA MARIA DA VITÓRIA', 'BA'),
(4202, 'SANTA MARIA DAS BARREIRAS', 'PA'),
(4203, 'SANTA MARIA DE ITABIRA', 'MG'),
(4204, 'SANTA MARIA DE JETIBÁ', 'ES'),
(4205, 'SANTA MARIA DO CAMBUCA', 'PE'),
(4206, 'SANTA MARIA DO HERVAL', 'RS'),
(4207, 'SANTA MARIA DO OESTE', 'PR'),
(4208, 'SANTA MARIA DO PARÁ', 'PA'),
(4209, 'SANTA MARIA DO SALTO', 'MG'),
(4210, 'SANTA MARIA DO SUAÇUÍ', 'MG'),
(4211, 'SANTA MARIA DO TOCANTINS', 'TO'),
(4212, 'SANTA MARIA MADALENA', 'RJ'),
(4213, 'SANTA MARIANA', 'PR'),
(4214, 'SANTA MERCEDES', 'SP'),
(4215, 'SANTA MÔNICA', 'PR'),
(4216, 'SANTA QUITERIA', 'CE'),
(4217, 'SANTA QUITERIA DO MARANHÃO', 'MA'),
(4218, 'SANTA RITA', 'MA'),
(4219, 'SANTA RITA DE CALDAS', 'MG'),
(4220, 'SANTA RITA DE CASSIA', 'BA'),
(4221, 'SANTA RITA DE JACUTINGA', 'MG'),
(4222, 'SANTA RITA DE MINAS', 'MG'),
(4223, 'SANTA RITA DO ARÁGUAIA', 'GO'),
(4224, 'SANTA RITA DO IBITIPOCA', 'MG'),
(4225, 'SANTA RITA DO ITUETO', 'MG'),
(4226, 'SANTA RITA DO NOVO DESTINO', 'GO'),
(4227, 'SANTA RITA DO PASSA QUATRO', 'SP'),
(4228, 'SANTA RITA DO PRADO', 'MS'),
(4229, 'SANTA RITA DO SAPUCAI', 'MG'),
(4230, 'SANTA RITA DO TOCANTINS', 'TO'),
(4231, 'SANTA RITA DO OESTE', 'SP'),
(4232, 'SANTA ROSA', 'AC'),
(4233, 'SANTA ROSA', 'RS'),
(4234, 'SANTA ROSA DA SERRA', 'MG'),
(4235, 'SANTA ROSA DE GOIÁS', 'GO'),
(4236, 'SANTA ROSA DE LIMA', 'SC'),
(4237, 'SANTA ROSA DE LIMA', 'SE'),
(4238, 'SANTA ROSA DO VITERBO', 'SP'),
(4239, 'SANTA ROSA DO PIAUÍ', 'PI'),
(4240, 'SANTA ROSA DO SUL', 'SC'),
(4241, 'SANTA ROSA DO TOCANTINS', 'TO'),
(4242, 'SANTA SALETE', 'SP'),
(4243, 'SANTA TERESA', 'ES'),
(4244, 'SANTA TEREZA', 'RS'),
(4245, 'SANTA TEREZA DE GOIÁS', 'GO'),
(4246, 'SANTA TEREZA DO OESTE', 'PR'),
(4247, 'SANTA TEREZA DO TOCANTINS', 'TO'),
(4248, 'SANTA TEREZINHA', 'BA'),
(4249, 'SANTA TEREZINHA', 'PB'),
(4250, 'SANTA TEREZINHA DE GOIÁS', 'GO'),
(4251, 'SANTA TEREZINHA DE ITAIPU', 'PR'),
(4252, 'SANTA TEREZINHA DO PROGRESSO', 'SC'),
(4253, 'SANTA TEREZINHA DO TOCANTINS', 'TO'),
(4254, 'SANTA VITÓRIA', 'MG'),
(4255, 'SANTA VITÓRIA DO PALMAR', 'RS'),
(4256, 'SANTANA', 'AP'),
(4257, 'SANTANA DA BOA VISTA', 'RS'),
(4258, 'SANTANA DA PONTE PENSA', 'SP'),
(4259, 'SANTANA DA VARGEM', 'MG'),
(4260, 'SANTANA DE CATÁGUASES', 'MG'),
(4261, 'SANTANA DE PIRAPAMA', 'MG'),
(4262, 'SANTANA DO ACARAÚ', 'CE'),
(4263, 'SANTANA DO ARAGUAIA', 'PA'),
(4264, 'SANTANA DO CARIRI', 'CE'),
(4265, 'SANTANA DO DESERTO', 'MG'),
(4266, 'SANTANA DO GARAMBEU', 'MG'),
(4267, 'SANTANA DO IPANEMA', 'AL'),
(4268, 'SANTANA DO ITARARÉ', 'PR'),
(4269, 'SANTANA DO JACARÉ', 'MG'),
(4270, 'SANTANA DO LIVRAMENTO', 'RS'),
(4271, 'SANTANA DO MANHUAÇU', 'MG'),
(4272, 'SANTANA DO MARANHÃO', 'MA'),
(4273, 'SANTANA DO MATOS', 'RN'),
(4274, 'SANTANA DO MUNDAU', 'AL'),
(4275, 'SANTANA DO PARAÍSO', 'MG'),
(4276, 'SANTANA DE PARNAÍBA', 'SP'),
(4277, 'SANTANA DO PIAUÍ', 'PI'),
(4278, 'SANTANA DO PIRAPAMA', 'MG'),
(4279, 'SANTANA DO RIACHO', 'MG'),
(4280, 'SANTANA DO SERIDO', 'RN'),
(4281, 'SANTANA DOS GARROTES', 'PB'),
(4282, 'SANTANA DOS MONTES', 'MG'),
(4283, 'SANTANA DO SÃO FRANCISCO', 'SE'),
(4284, 'SANTANÁPOLIS', 'BA'),
(4285, 'SANTAREM', 'PA'),
(4286, 'SANTARÉM NOVO', 'PA'),
(4287, 'SANTIAGO', 'RS'),
(4288, 'SANTO ANTÔNIO DO AVENTUREIRO', 'MG'),
(4289, 'SANTO ANTÔNIO DO RETIRO', 'MG'),
(4290, 'SANTO AFONSO', 'MT'),
(4291, 'SANTO AMARO', 'BA'),
(4292, 'SANTO AMARO DA IMPERATRIZ', 'SC'),
(4293, 'SANTO AMARO DAS BROTAS', 'SE'),
(4294, 'SANTO AMARO DO MARANHÃO', 'MA'),
(4295, 'SANTO ANASTÁCIO', 'SP'),
(4296, 'SANTO ANDRÉ', 'SP'),
(4297, 'SANTO ANGELO', 'RS'),
(4298, 'SANTO ANTÔNIO', 'RN'),
(4299, 'SANTO ANTÔNIO DO MONTE', 'MG'),
(4300, 'SANTO ANTÔNIO DA ALEGRIA', 'SP'),
(4301, 'SANTO ANTÔNIO DA BARRA', 'GO'),
(4302, 'SANTO ANTÔNIO DAS MISSÕES', 'RS'),
(4303, 'SANTO ANTÔNIO DE GOIÁS', 'GO'),
(4304, 'SANTO ANTÔNIO DE JESUS', 'BA'),
(4305, 'SANTO ANTÔNIO DE PÁDUA', 'RJ'),
(4306, 'SANTO ANTÔNIO DE POSSE', 'SP'),
(4307, 'SANTO ANTÔNIO DESCOBERTO', 'GO'),
(4308, 'SANTO ANTÔNIO DO CAIUA', 'PR'),
(4309, 'SANTO ANTÔNIO DO AMPARO', 'MG'),
(4310, 'SANTO ANTÔNIO DO ARAÇANGUA', 'SP'),
(4311, 'SANTO ANTÔNIO DO GRAMA', 'MG'),
(4312, 'SANTO ANTÔNIO DO ICA', 'AM'),
(4313, 'SANTO ANTÔNIO DO ITAMBÉ', 'MG'),
(4314, 'SANTO ANTÔNIO DO JARDIM', 'SP'),
(4315, 'SANTO ANTÔNIO DO LEVERGER', 'MT'),
(4316, 'SANTO ANTÔNIO DO PALMA', 'RS'),
(4317, 'SANTO ANTÔNIO DO PARAISO', 'PR'),
(4318, 'SANTO ANTÔNIO DO PINHAL', 'SP'),
(4319, 'SANTO ANTÔNIO DO PLANALTO', 'RS'),
(4320, 'SANTO ANTÔNIO DO SUDOESTE', 'PR'),
(4321, 'SANTO ANTÔNIO DO TAUÁ', 'PA'),
(4322, 'SANTO ANTÔNIO DOS LOPES', 'MA'),
(4323, 'SANTO ANTÔNIO DOS MILAGRES', 'PI'),
(4324, 'SANTO ANTÔNIO DO JACINTO', 'MG'),
(4325, 'SANTO ANTÔNIO DE LISBOA', 'PI'),
(4326, 'SANTO ANTÔNIO DA PLATINA', 'PR'),
(4327, 'SANTO ANTÔNIO DO RIO ABAIXO', 'MG'),
(4328, 'SANTO AUGUSTO', 'RS'),
(4329, 'SANTO CRISTO', 'RS'),
(4330, 'SANTO ESTEVAO', 'BA'),
(4331, 'SANTO EXPEDITO', 'SP'),
(4332, 'SANTO EXPEDITO DO SUL', 'RS'),
(4333, 'SANTO HIPOLITO', 'MG'),
(4334, 'SANTO INÁCIO', 'PR'),
(4335, 'SANTO INACIO DO PIAUÍ', 'PI'),
(4336, 'SANTÓPOLIS DO ÁGUAPEI', 'SP'),
(4337, 'SANTOS', 'SP'),
(4338, 'SANTOS DUMONT', 'MG'),
(4339, 'SÃO BENEDITO', 'CE'),
(4340, 'SÃO BENEDITO DO RIO PRETO', 'MA'),
(4341, 'SÃO BENEDITO DO SUL', 'PE'),
(4342, 'SÃO BENTO', 'MA'),
(4343, 'SÃO BENTO', 'PB'),
(4344, 'SÃO BENTO DO ABADE', 'MG'),
(4345, 'SÃO BENTO DO NORTE', 'RN'),
(4346, 'SÃO BENTO DO SAPUCAI', 'SP'),
(4347, 'SÃO BENTO DO SUL', 'SC'),
(4348, 'SÃO BENTO DO TRAIRI', 'RN'),
(4349, 'SÃO BENTO DO UNA', 'PE'),
(4350, 'SÃO BENTO DO TOCANTINS', 'TO'),
(4351, 'SÃO BERNARDINO', 'SC'),
(4352, 'SÃO BERNARDO', 'MA'),
(4353, 'SÃO BERNARDO DO CAMPO', 'SP'),
(4354, 'SÃO BONIFÁCIO', 'SC'),
(4355, 'SÃO BORJA', 'RS'),
(4356, 'SÃO BRAS', 'AL'),
(4357, 'SÃO BRAS DO PIAUÍ', 'PI'),
(4358, 'SÃO BRAS DO SUAÇUI', 'MG'),
(4359, 'SÃO CAETANO', 'PE'),
(4360, 'SÃO CAETANO DE ODIVELAS', 'PA'),
(4361, 'SÃO CAETANO DO SUL', 'SP'),
(4362, 'SÃO CARLOS', 'SC'),
(4363, 'SÃO CARLOS DO IVAI', 'PR'),
(4364, 'SÃO CRISTOVAO', 'SE'),
(4365, 'SÃO CRISTOVAO DO SUL', 'SC'),
(4366, 'SÃO DESIDERIO', 'BA'),
(4367, 'SÃO DOMINGOS', 'SC'),
(4368, 'SÃO DOMINGOS', 'SE'),
(4369, 'SÃO DOMINGOS DAS DORES', 'MG'),
(4370, 'SÃO DOMINGOS DO ARAGUAIA', 'PA'),
(4371, 'SÃO DOMINGOS DO AZEITÃO', 'MA'),
(4372, 'SÃO DOMINGOS DO CAPIM', 'PA'),
(4373, 'SÃO DOMINGOS DO CARIRI', 'PB'),
(4374, 'SÃO DOMINGOS DO MARANHÃO', 'MA'),
(4375, 'SÃO DOMINGOS DO NORTE', 'ES'),
(4376, 'SÃO DOMINGOS DO PRATA', 'MG'),
(4377, 'SÃO DOMINGOS DO SUL', 'RS'),
(4378, 'SÃO FELIPE', 'BA'),
(4379, 'SÃO FELIPE DO OESTE', 'RO'),
(4380, 'SÃO FELIX', 'BA'),
(4381, 'SÃO FELIX DE BALSAS', 'MA'),
(4382, 'SÃO FELIX DE MINAS', 'MG'),
(4383, 'SÃO FÉLIX DO ARAGUAIA', 'MT'),
(4384, 'SÃO FELIX DO CORIBE', 'BA'),
(4385, 'SÃO FELIX DO PIAUÍ', 'PI'),
(4386, 'SÃO FÉLIX DO TOCANTINS', 'TO'),
(4387, 'SÃO FELIX DO XINGU', 'PA'),
(4388, 'SÃO FERNANDO', 'RN'),
(4389, 'SÃO FIDÉLIS', 'RJ'),
(4390, 'SÃO FRANCISCO', 'MG'),
(4391, 'SÃO FRANCISCO DE ASSIS', 'RS'),
(4392, 'SÃO FRANCISCO DE ASSIS PIAUÍ', 'PI'),
(4393, 'SÃO FRANCISCO DE GOIÁS', 'GO'),
(4394, 'SÃO FRANCISCO DE PAULA', 'MG'),
(4395, 'SÃO FRANCISCO DE SALES', 'MG'),
(4396, 'SÃO FRANCISCO DO BREJÃO', 'MA'),
(4397, 'SÃO FRANCISCO DO CONDE', 'BA'),
(4398, 'SÃO FRANCISCO DO GLÓRIA', 'MG'),
(4399, 'SÃO FRANCISCO DO MARANHÃO', 'MA'),
(4400, 'SÃO FRANCISCO DO OESTE', 'RN'),
(4401, 'SÃO FRANCISCO DO PARÁ', 'PA'),
(4402, 'SÃO FRANCISCO DO PIAUÍ', 'PI'),
(4403, 'SÃO FRANCISCO DO SUL', 'SC'),
(4404, 'SÃO FRANCISCO ITABAPOANA', 'RJ'),
(4405, 'SÃO GABRIEL', 'BA'),
(4406, 'SÃO GABRIEL CACHOEIRA', 'AM'),
(4407, 'SÃO GABRIEL DA PALHA', 'ES'),
(4408, 'SÃO GABRIEL D''OESTE', 'MS'),
(4409, 'SÃO GERALDO', 'MG'),
(4410, 'SÃO GERALDO DA PIEDADE', 'MG'),
(4411, 'SÃO GERALDO DO ARAGUAIA', 'PA'),
(4412, 'SÃO GERALDO DO BAIXO', 'MG'),
(4413, 'SÃO GONCALO', 'RJ'),
(4414, 'SÃO GONCALO AMARANTE', 'RN'),
(4415, 'SÃO GONCALO DO ABAETÉ', 'MG'),
(4416, 'SÃO GONCALO DO AMARANTE', 'CE'),
(4417, 'SÃO GONCALO DO GURGUEIA', 'PI'),
(4418, 'SÃO GONCALO DO PARA', 'MG'),
(4419, 'SÃO GONCALO DO PIAUÍ', 'PI'),
(4420, 'SÃO GONCALO DO RIO PRETO', 'MG'),
(4421, 'SÃO GONCALO DO SAPUCAI', 'MG'),
(4422, 'SÃO GONCALO DOS CAMPOS', 'BA'),
(4423, 'SÃO GONÇALO RIO ABAIXO', 'MG'),
(4424, 'SÃO GOTARDO', 'MG'),
(4425, 'SÃO JERONIMO', 'RS'),
(4426, 'SÃO JERÔNIMO DA SERRA', 'PR'),
(4427, 'SÃO JOÃO', 'PR'),
(4428, 'SÃO JOÃO BATISTA', 'MA'),
(4429, 'SÃO JOÃO BATISTA GLÓRIA', 'MG'),
(4430, 'SÃO JOÃO DA BALIZA', 'RR'),
(4431, 'SÃO JOÃO DA BARRA', 'RJ'),
(4432, 'SÃO JOÃO DA BOA VISTA', 'SP'),
(4433, 'SÃO JOÃO DA CANABRAVA', 'PI'),
(4434, 'SÃO JOÃO DA FRONTEIRA', 'PI'),
(4435, 'SÃO JOÃO DA LAGOA', 'MG'),
(4436, 'SÃO JOÃO DA MATA', 'MG'),
(4437, 'SÃO JOÃO DA PARAUNA', 'GO'),
(4438, 'SÃO JOÃO DA PONTA', 'PA'),
(4439, 'SÃO JOÃO DA PONTE', 'MG'),
(4440, 'SÃO JOÃO DA SERRA', 'PI'),
(4441, 'SÃO JOÃO DA URTIGA', 'RS'),
(4442, 'SÃO JOÃO DA VARJOTA', 'PI'),
(4443, 'SÃO JOÃO D''ALIANÇA', 'GO'),
(4444, 'SÃO JOÃO DAS DUAS PONTES', 'SP'),
(4445, 'SÃO JOÃO DAS MISSÕES', 'MG'),
(4446, 'SÃO JOÃO DE IRACEMA', 'SP'),
(4447, 'SÃO JOÃO DE MERITI', 'RJ'),
(4448, 'SÃO JOÃO DE PIRABAS', 'PA'),
(4449, 'SÃO JOÃO DEL REI', 'MG'),
(4450, 'SÃO JOÃO DO ARÁGUAIA', 'PA'),
(4451, 'SÃO JOÃO DO ARRAIAL', 'PI'),
(4452, 'SÃO JOÃO DO CAIUA', 'PR'),
(4453, 'SÃO JOÃO DO CARIRI', 'PB'),
(4454, 'SÃO JOÃO DO CARU', 'MA'),
(4455, 'SÃO JOÃO DO ITAPERUI', 'SC'),
(4456, 'SÃO JOÃO DO IVAI', 'PR'),
(4457, 'SÃO JOÃO DO JÁGUARIBE', 'CE'),
(4458, 'SÃO JOÃO DO MANHUAÇU', 'MG'),
(4459, 'SÃO JOÃO DO MANTENINHA', 'MG'),
(4460, 'SÃO JOÃO DO OESTE', 'SC'),
(4461, 'SÃO JOÃO DO ORIENTE', 'MG'),
(4462, 'SÃO JOÃO DO PACUÍ', 'MG'),
(4463, 'SÃO JOÃO DO PARAISO', 'MA'),
(4464, 'SÃO JOÃO DO PAU DE ALHO', 'SP'),
(4465, 'SÃO JOÃO DO PIAUÍ', 'PI'),
(4466, 'SÃO JOÃO DO POLESINE', 'RS'),
(4467, 'SÃO JOÃO DO RIO DO PEIXE', 'PB'),
(4468, 'SÃO JOÃO DO SABUGI', 'PB'),
(4469, 'SÃO JOÃO DO SOTER', 'MA'),
(4470, 'SÃO JOÃO DO SUL', 'SC'),
(4471, 'SÃO JOÃO DO TIGRE', 'PB'),
(4472, 'SÃO JOÃO DO TRIUNFO', 'PR'),
(4473, 'SÃO JOÃO DOS PATOS', 'MA'),
(4474, 'SÃO JOÃO EVANGELISTA', 'MG'),
(4475, 'SÃO JOÃO NEPOMUCENO', 'MG'),
(4476, 'SÃO JOAQUIM', 'SC'),
(4477, 'SÃO JOAQUIM DA BARRA', 'SP'),
(4478, 'SÃO JOAQUIM DO MONTE', 'PE'),
(4479, 'SÃO JORGE', 'RS'),
(4480, 'SÃO JORGE DO IVAI', 'PR'),
(4481, 'SÃO JORGE DO OESTE', 'PR'),
(4482, 'SÃO JORGE DO PATROCÍNIO', 'PR'),
(4483, 'SÃO JOSÉ', 'SC'),
(4484, 'SÃO JOSÉ CAMPESTRE', 'RN'),
(4485, 'SÃO JOSÉ DA BARRA', 'MG'),
(4486, 'SÃO JOSÉ DA BELA VISTA', 'SP'),
(4487, 'SÃO JOSÉ DA BOA VISTA', 'PR'),
(4488, 'SÃO JOSÉ DA COROA GRANDE', 'PE'),
(4489, 'SÃO JOSÉ DA LAJE', 'AL'),
(4490, 'SÃO JOSÉ DA LAGOA TAPADA', 'PB'),
(4491, 'SÃO JOSÉ DA SAFIRA', 'MG'),
(4492, 'SÃO JOSÉ DA TAPERA', 'AL'),
(4493, 'SÃO JOSÉ DA VARGINHA', 'MG'),
(4494, 'SÃO JOSÉ DA VITÓRIA', 'BA'),
(4495, 'SÃO JOSÉ DAS MISSÕES', 'RS'),
(4496, 'SÃO JOSÉ DAS PALMEIRAS', 'PR'),
(4497, 'SÃO JOSÉ DE CAIANA', 'PB'),
(4498, 'SÃO JOSÉ DE ESPINHARAS', 'PB'),
(4499, 'SÃO JOSÉ DE MIPIBU', 'RN'),
(4500, 'SÃO JOSÉ DE PIRANHAS', 'PB'),
(4501, 'SÃO JOSÉ DE PRINCESA', 'PB'),
(4502, 'SÃO JOSÉ DE RIBAMAR', 'MA'),
(4503, 'SÃO JOSÉ DE UBA', 'RJ'),
(4504, 'SÃO JOSÉ DO ALEGRE', 'MG'),
(4505, 'SÃO JOSÉ DO BARREIRO', 'SP'),
(4506, 'SÃO JOSÉ DO BELMONTE', 'PE'),
(4507, 'SÃO JOSÉ DO BONFIM', 'PB'),
(4508, 'SÃO JOSÉ DO BREJO DO CRUZ', 'PB'),
(4509, 'SÃO JOSÉ DO CALCADO', 'ES'),
(4510, 'SÃO JOSÉ DO CEDRO', 'SC'),
(4511, 'SÃO JOSÉ DO CERRITO', 'SC'),
(4512, 'SÃO JOSÉ DO EGITO', 'PE'),
(4513, 'SÃO JOSÉ DO GOIABAL', 'MG'),
(4514, 'SÃO JOSÉ DO HERVAL', 'RS'),
(4515, 'SÃO JOSÉ DO HORTENCIO', 'RS'),
(4516, 'SÃO JOSÉ DO INHACORA', 'RS'),
(4517, 'SÃO JOSÉ DO JACURI', 'MG'),
(4518, 'SÃO JOSÉ DO LAPA', 'MG'),
(4519, 'SÃO JOSÉ DO MANTIMENTO', 'MG'),
(4520, 'SÃO JOSÉ DO NORTE', 'RS'),
(4521, 'SÃO JOSÉ DO OURO', 'RS'),
(4522, 'SÃO JOSÉ DO PEIXE', 'PI'),
(4523, 'SÃO JOSÉ DO PIAUÍ', 'PI'),
(4524, 'SÃO JOSÉ DO POVO', 'MT'),
(4525, 'SÃO JOSÉ DO RIO CLARO', 'MT'),
(4526, 'SÃO JOSÉ DO RIO PARDO', 'SP'),
(4527, 'SÃO JOSÉ DO RIO PRETO', 'SP'),
(4528, 'SÃO JOSÉ DO SABUGI', 'PB'),
(4529, 'SÃO JOSÉ DO XINGU', 'MT'),
(4530, 'SÃO JOSÉ DOS AUSENTES', 'RS'),
(4531, 'SÃO JOSÉ DOS BASILIOS', 'MA'),
(4532, 'SÃO JOSÉ DOS CAMPOS', 'SP'),
(4533, 'SÃO JOSÉ DOS CORDEIROS', 'PB'),
(4534, 'SÃO JOSÉ DOS PINHAIS', 'PR'),
(4535, 'SÃO JOSÉ DOS QUATRO MARCOS', 'MT'),
(4536, 'SÃO JOSÉ DOS RAMOS', 'PB'),
(4537, 'SÃO JOSÉ VALE RIO PRETO', 'RJ'),
(4538, 'SÃO JULIÃO', 'PI'),
(4539, 'SÃO LEOPOLDO', 'RS'),
(4540, 'SÃO LOURENCO', 'MG'),
(4541, 'SÃO LOURENCO DA MATA', 'PE'),
(4542, 'SÃO LOURENCO DA SERRA', 'SP'),
(4543, 'SÃO LOURENCO DO PIAUÍ', 'PI'),
(4544, 'SÃO LOURENCO DO SUL', 'RS'),
(4545, 'SÃO LOURENCO D''OESTE', 'SC'),
(4546, 'SÃO LUDGERO', 'SC'),
(4547, 'SÃO LUIS', 'MA'),
(4548, 'SÃO LUIS DE MONTES BELOS', 'GO'),
(4549, 'SÃO LUIS DO CURU', 'CE'),
(4550, 'SÃO LUÍS DO PIAUÍ', 'PI'),
(4551, 'SÃO LUIS DO QUITUNDE', 'AL'),
(4552, 'SÃO LUÍS GONZAGA DO MARANHÃO', 'MA'),
(4553, 'SÃO LUIZ', 'RR'),
(4554, 'SÃO LUIZ DO AMAUA', 'RR'),
(4555, 'SÃO LUIZ DO NORTE', 'GO'),
(4556, 'SÃO LUIZ DO PARAITINGA', 'SP'),
(4557, 'SÃO LUIZ GONZAGA', 'RS'),
(4558, 'SÃO MAMEDE', 'PB'),
(4559, 'SÃO MANOEL DO PARANÁ', 'PR'),
(4560, 'SÃO MANUEL', 'SP'),
(4561, 'SÃO MARCOS', 'RS'),
(4562, 'SÃO MARTINHO', 'RS'),
(4563, 'SÃO MARTINHO DA SERRA', 'RS'),
(4564, 'SÃO MATEUS', 'ES'),
(4565, 'SÃO MATEUS DO MARANHÃO', 'MA'),
(4566, 'SÃO MATEUS DO SUL', 'PR'),
(4567, 'SÃO MIGUEL', 'RN'),
(4568, 'SÃO MIGUEL DO ANTA', 'MG'),
(4569, 'SÃO MIGUEL ARCANJO', 'SP'),
(4570, 'SÃO MIGUEL DA BAIXA GRANDE', 'PI'),
(4571, 'SÃO MIGUEL DA BOA VISTA', 'SC'),
(4572, 'SÃO MIGUEL DAS MATAS', 'BA'),
(4573, 'SÃO MIGUEL DAS MISSÕES', 'RS'),
(4574, 'SÃO MIGUEL DE TAIPU', 'PB'),
(4575, 'SÃO MIGUEL DE TOUROS', 'RN'),
(4576, 'SÃO MIGUEL DO ALEIXO', 'SE'),
(4577, 'SÃO MIGUEL DO ARÁGUAIA', 'GO'),
(4578, 'SÃO MIGUEL DO FIDALGO', 'PI'),
(4579, 'SÃO MIGUEL DO GUAMA', 'PA'),
(4580, 'SÃO MIGUEL DO GUAPORÉ', 'RO'),
(4581, 'SÃO MIGUEL DO IGUAÇU', 'PR'),
(4582, 'SÃO MIGUEL DO OESTE', 'SC'),
(4583, 'SÃO MIGUEL DO TAPUIO', 'PI'),
(4584, 'SÃO MIGUEL DO TOCANTINS', 'TO'),
(4585, 'SÃO MIGUEL DOS CAMPOS', 'AL'),
(4586, 'SÃO MIGUEL DOS MILAGRES', 'AL'),
(4587, 'SÃO MIGUEL PASSA QUATRO', 'GO'),
(4588, 'SÃO NICILAU', 'RS'),
(4589, 'SÃO NICOLAU', 'RS'),
(4590, 'SÃO PATRÍCIO', 'GO'),
(4591, 'SÃO PAULO', 'SP'),
(4592, 'SÃO PAULO DAS MISSÕES', 'RS'),
(4593, 'SÃO PAULO DE OLIVENCA', 'AM'),
(4594, 'SÃO PAULO DO POTENGI', 'RN'),
(4595, 'SÃO PEDRO', 'RN'),
(4596, 'SÃO PEDRO DA ÁGUA BRANCA', 'MA'),
(4597, 'SÃO PEDRO DA ALDEIA', 'RJ'),
(4598, 'SÃO PEDRO DA CIPA', 'MT'),
(4599, 'SÃO PEDRO DA SERRA', 'RS'),
(4600, 'SÃO PEDRO DA UNIÃO', 'MG'),
(4601, 'SÃO PEDRO DE ALCANTARA', 'SC'),
(4602, 'SÃO PEDRO DO BUTIA', 'RS'),
(4603, 'SÃO PEDRO DO IGUAÇU', 'PR'),
(4604, 'SÃO PEDRO DO IVAÍ', 'PR'),
(4605, 'SÃO PEDRO DO PARANÁ', 'PR'),
(4606, 'SÃO PEDRO DO PIAUÍ', 'PI'),
(4607, 'SÃO PEDRO DO SUAÇUI', 'MG'),
(4608, 'SÃO PEDRO DO SUL', 'RS'),
(4609, 'SÃO PEDRO DO TURVO', 'SP'),
(4610, 'SÃO PEDRO DOS CRENTES', 'MA'),
(4611, 'SÃO PEDRO DOS FERROS', 'MG'),
(4612, 'SÃO RAFAEL', 'RN'),
(4613, 'SÃO RAIMUNDO DAS MANGABEIRA', 'MA'),
(4614, 'SÃO RAIMUNDO DOCA BEZERRA', 'MS'),
(4615, 'SÃO RAIMUNDO NONATO', 'PI'),
(4616, 'SÃO ROBERTO', 'MA'),
(4617, 'SÃO ROMAO', 'MG'),
(4618, 'SÃO ROQUE', 'SP'),
(4619, 'SÃO ROQUE DE MINAS', 'MG'),
(4620, 'SÃO ROQUE DO CANAÃ', 'ES'),
(4621, 'SÃO SALVADOR DO TOCANTINS', 'TO'),
(4622, 'SÃO SEBASTIÃO', 'AL'),
(4623, 'SÃO SEBASTIÃO DO RIO VERDE', 'MG'),
(4624, 'SÃO SEBASTIÃO DA AMOREIRA', 'PR'),
(4625, 'SÃO SEBASTIÃO BELA VISTA', 'MG'),
(4626, 'SÃO SEBASTIÃO DA BOA VISTA', 'PA'),
(4627, 'SÃO SEBASTIÃO DA GRAMA', 'SP'),
(4628, 'SÃO SEBASTIÃO DE LAGOA DE ROCA', 'PB'),
(4629, 'SÃO SEBASTIÃO DO ALTO', 'RJ'),
(4630, 'SÃO SEBASTIÃO DO CAÍ', 'RS'),
(4631, 'SÃO SEBASTIÃO DO MARANHÃO', 'MG'),
(4632, 'SÃO SEBASTIÃO DO OESTE', 'MG'),
(4633, 'SÃO SEBASTIÃO DO PARAÍSO', 'MG'),
(4634, 'SÃO SEBASTIÃO DO PASSÉ', 'BA'),
(4635, 'SÃO SEBASTIÃO DO UATUMA', 'AM'),
(4636, 'SÃO SEBASTIÃO DO UMBUZEIRO', 'PB'),
(4637, 'SÃO SEBASTIÃO DO RIO PRETO', 'MG'),
(4638, 'SÃO SEPE', 'RS'),
(4639, 'SÃO SIMÃO', 'GO'),
(4640, 'SÃO SIMÃO', 'SP'),
(4641, 'SÃO TOME DAS LETRAS', 'MG'),
(4642, 'SÃO TIAGO', 'MG'),
(4643, 'SÃO TOMAS DE AQUINO', 'MG'),
(4644, 'SÃO TOMÉ', 'PR'),
(4645, 'SÃO VALENTIM', 'RS'),
(4646, 'SÃO VALENTIM DO SUL', 'RS'),
(4647, 'SÃO VALERIO DA NATIVIDADE', 'TO'),
(4648, 'SÃO VALERIO DO SUL', 'RS'),
(4649, 'SÃO VENDELINO', 'RS'),
(4650, 'SÃO VICENTE', 'RN'),
(4651, 'SÃO VICENTE DE FERRER', 'MA'),
(4652, 'SÃO VICENTE DE MINAS', 'MG'),
(4653, 'SÃO VICENTE DO SERIDO', 'PB'),
(4654, 'SÃO VICENTE DO SUL', 'RS'),
(4655, 'SÃO VICENTE FERRER', 'PE'),
(4656, 'SAPÉ', 'PB'),
(4657, 'SAPEAÇU', 'BA'),
(4658, 'SAPEZAL', 'MT'),
(4659, 'SAPIRANGA', 'RS'),
(4660, 'SAPOPEMA', 'PR'),
(4661, 'SAPUCAIA', 'PA'),
(4662, 'SAPUCAIA', 'RJ'),
(4663, 'SAPUCAIA DO SUL', 'RS'),
(4664, 'SAPUCAI-MIRIM', 'MG'),
(4665, 'SAQUAREMA', 'RJ'),
(4666, 'SARANDI', 'PR'),
(4667, 'SARANDI', 'RS'),
(4668, 'SARAPUI', 'SP'),
(4669, 'SARDOA', 'MG'),
(4670, 'SARUTAIA', 'SP'),
(4671, 'SARZEDO', 'MG'),
(4672, 'SÁTIRO DIAS', 'BA'),
(4673, 'SATUBINHA', 'MA'),
(4674, 'SAUBARA', 'BA'),
(4675, 'SAUDADE DO IGUAÇU', 'PR'),
(4676, 'SAUDADES', 'SC'),
(4677, 'SAÚDE', 'BA'),
(4678, 'SCHROEDER', 'SC'),
(4679, 'SEABRA', 'BA'),
(4680, 'SEARA', 'SC'),
(4681, 'SEBASTIANÓPOLIS DO SUL', 'SP'),
(4682, 'SEBASTIÃO BARROS', 'PI'),
(4683, 'SEBASTIÃO LARANJEIRAS', 'BA'),
(4684, 'SEBASTIÃO LEAL', 'PI'),
(4685, 'SEBERI', 'RS'),
(4686, 'SEDE NOVA', 'RS'),
(4687, 'SEGREDO', 'RS'),
(4688, 'SELBACH', 'RS'),
(4689, 'SELVIRIA', 'MS'),
(4690, 'SENA MADUREIRA', 'AC'),
(4691, 'SENADOR ALEXANDRE COSTA', 'MA'),
(4692, 'SENADOR AMARAL', 'MG'),
(4693, 'SENADOR CANEDO', 'GO'),
(4694, 'SENADOR CORTES', 'MG'),
(4695, 'SENADOR ELOI DE SOUZA', 'RN'),
(4696, 'SENADOR FIRMINO', 'MG'),
(4697, 'SENADOR GEORGINO AVELINO', 'RN'),
(4698, 'SENADOR GUIOMARD', 'AC'),
(4699, 'SENADOR JOSÉ BENTO', 'MG'),
(4700, 'SENADOR JOSÉ PORFIRIO', 'PA'),
(4701, 'SENADOR LA  ROCQUE', 'MA'),
(4702, 'SENADOR MODESTINO GONÇALVES', 'MG'),
(4703, 'SENADOR POMPEU', 'CE'),
(4704, 'SENADOR RUI PALMEIRA', 'AL'),
(4705, 'SENADOR SA', 'CE'),
(4706, 'SENADOR SALGADO FILHO', 'RS'),
(4707, 'SENGES', 'PR'),
(4708, 'SENHOR DO BONFIM', 'BA'),
(4709, 'SENHORA DE OLIVEIRA', 'MG'),
(4710, 'SENHORA DO PORTO', 'MG'),
(4711, 'SENHORA DOS REMÉDIOS', 'MG'),
(4712, 'SENTINELA DO SUL', 'RS'),
(4713, 'SENTO SE', 'BA'),
(4714, 'SERAFINA CORRÊA', 'RS'),
(4715, 'SERICITA', 'MG'),
(4716, 'SERIDO', 'PB'),
(4717, 'SERINGUEIRAS', 'RO'),
(4718, 'SERIO', 'RS'),
(4719, 'SERITINGA', 'MG'),
(4720, 'SEROPEDICA', 'RJ'),
(4721, 'SERRA', 'ES'),
(4722, 'SERRA ALTA', 'SC'),
(4723, 'SERRA AZUL', 'SP'),
(4724, 'SERRA AZUL DE MINAS', 'MG'),
(4725, 'SERRA BRANCA', 'PB'),
(4726, 'SERRA CAIADA', 'RN'),
(4727, 'SERRA DA RAIZ', 'PB'),
(4728, 'SERRA DA SAUDADE', 'MG'),
(4729, 'SERRA DE SÃO BENTO', 'RN'),
(4730, 'SERRA DO MEL', 'RN'),
(4731, 'SERRA DO NAVIO', 'AP'),
(4732, 'SERRA DO RAMALHO', 'BA'),
(4733, 'SERRA DO SALITRE', 'MG'),
(4734, 'SERRA DOS AIMORES', 'MG'),
(4735, 'SERRA DOURADA', 'BA'),
(4736, 'SERRA GRANDE', 'PB'),
(4737, 'SERRA NEGRA', 'SP'),
(4738, 'SERRA NEGRA DO NORTE', 'RN'),
(4739, 'SERRA PRETA', 'BA'),
(4740, 'SERRA REDONDA', 'PB'),
(4741, 'SERRA TALHADA', 'PE'),
(4742, 'SERRANA', 'SP'),
(4743, 'SERRANIA', 'MG'),
(4744, 'SERRANO DO MARANHÃO', 'MA'),
(4745, 'SERRANÓPOLIS', 'GO'),
(4746, 'SERRANÓPOLIS DO IGUAÇU', 'PR'),
(4747, 'SERRANOS', 'MG'),
(4748, 'SERRARIA', 'PB'),
(4749, 'SERRINHA', 'BA'),
(4750, 'SERRINHA DOS PINTOS', 'RN'),
(4751, 'SERRITA', 'PE'),
(4752, 'SERRO', 'MG'),
(4753, 'SERROLÂNDIA', 'BA'),
(4754, 'SERTANEJA', 'PR'),
(4755, 'SERTANIA', 'PE'),
(4756, 'SERTANÓPOLIS', 'PR'),
(4757, 'SERTÃO', 'RS'),
(4758, 'SERTÃO SANTANA', 'RS'),
(4759, 'SERTÃOZINHO', 'PB'),
(4760, 'SETE BARRAS', 'SP'),
(4761, 'SETE DE SETEMBRO', 'RS'),
(4762, 'SETE LAGOAS', 'MG'),
(4763, 'SETE QUEDAS', 'MS'),
(4764, 'SETUBINHA', 'MG'),
(4765, 'SEVERIANO DE ALMEIDA', 'RS'),
(4766, 'SEVERIANO MELO', 'RN'),
(4767, 'SEVERINIA', 'SP'),
(4768, 'SIDERÓPOLIS', 'SC'),
(4769, 'SIDROLÂNDIA', 'MS'),
(4770, 'SIGEFREDO PACHECO', 'PI'),
(4771, 'SILVA JARDIM', 'RJ'),
(4772, 'SILVANIA', 'GO'),
(4773, 'SILVANÓPOLIS', 'TO'),
(4774, 'SILVEIRA MARTINS', 'RS'),
(4775, 'SILVEIRAS', 'SP'),
(4776, 'SILVEIRÂNIA', 'MG'),
(4777, 'SILVES', 'AM'),
(4778, 'SILVIANÓPOLIS', 'MG'),
(4779, 'SIMÃO DIAS', 'SE'),
(4780, 'SIMÃO PEREIRA', 'MG'),
(4781, 'SIMÕES', 'PI'),
(4782, 'SIMÕES FILHO', 'BA'),
(4783, 'SIMOLÂNDIA', 'GO'),
(4784, 'SIMONÉSIA', 'MG'),
(4785, 'SIMPLÍCIO MENDES', 'PI'),
(4786, 'SINOP', 'MT'),
(4787, 'SIQUEIRA CAMPOS', 'PR'),
(4788, 'SIRINHAÉM', 'PE'),
(4789, 'SIRIRI', 'SE'),
(4790, 'SITIO D''ABADIA', 'GO'),
(4791, 'SITIO DO MATO', 'BA'),
(4792, 'SITIO DO QUINTO', 'BA'),
(4793, 'SITIO NOVO', 'MA'),
(4794, 'SITIO NOVO DO TOCANTINS', 'TO'),
(4795, 'SOBRADINHO', 'BA'),
(4796, 'SOBRADINHO', 'RS'),
(4797, 'SOBRADO', 'PB'),
(4798, 'SOBRAL', 'CE'),
(4799, 'SOBRALIA', 'MG'),
(4800, 'SOCORRO', 'SP'),
(4801, 'SOCORRO DO PIAUÍ', 'PI'),
(4802, 'SOLANEA', 'PB'),
(4803, 'SOLEDADE', 'PB'),
(4804, 'SOLEDADE DE MINAS', 'MG'),
(4805, 'SOLIDÃO', 'PE'),
(4806, 'SOLONOPOLE', 'CE'),
(4807, 'SOMBRIO', 'SC'),
(4808, 'SONORA', 'MS'),
(4809, 'SOORETAMA', 'ES'),
(4810, 'SOROCABA', 'SP'),
(4811, 'SORRISO', 'MT'),
(4812, 'SOSSEGO', 'PB'),
(4813, 'SOURE', 'PA'),
(4814, 'SOUSA', 'PB'),
(4815, 'SOUTO SOARES', 'BA'),
(4816, 'SUCUPIRA', 'TO'),
(4817, 'SUCUPIRA DO NORTE', 'MA'),
(4818, 'SUCUPIRA DO RIACHÃO', 'MA'),
(4819, 'SUD MENNUCCI', 'SP'),
(4820, 'SUL BRASIL', 'SC'),
(4821, 'SULINA', 'PR'),
(4822, 'SUMARE', 'SP'),
(4823, 'SUMÉ', 'PB'),
(4824, 'SUMIDOURO', 'RJ'),
(4825, 'SURUBIM', 'PE'),
(4826, 'SUSSUAPARA', 'PI'),
(4827, 'SUZANÁPOLIS', 'SP'),
(4828, 'SUZANO', 'SP'),
(4829, 'TABAI', 'RS'),
(4830, 'TABAPORÃ', 'MT'),
(4831, 'TABAPUA', 'SP'),
(4832, 'TABATINGA', 'AM'),
(4833, 'TABIRA', 'PE'),
(4834, 'TABOAO DA SERRA', 'SP'),
(4835, 'TABOCAS DO BREJO VELHO', 'BA'),
(4836, 'TABOLEIRO GRANDE', 'RN'),
(4837, 'TABULEIRO', 'MG'),
(4838, 'TABULEIRO DO NORTE', 'CE'),
(4839, 'TACAIMBO', 'PE'),
(4840, 'TACARATU', 'PE'),
(4841, 'TACIBA', 'SP'),
(4842, 'TACIMA', 'PB'),
(4843, 'TACURU', 'MS'),
(4844, 'TÁGUAI', 'SP'),
(4845, 'TÁGUATINGA', 'TO'),
(4846, 'TAIAÇU', 'SP'),
(4847, 'TAILÂNDIA', 'PA'),
(4848, 'TAIO', 'SC'),
(4849, 'TAIOBEIRAS', 'MG'),
(4850, 'TAIPAS', 'TO'),
(4851, 'TAIPU', 'RN'),
(4852, 'TAIUVA', 'SP'),
(4853, 'TALISMA', 'TO'),
(4854, 'TAMANDARÉ', 'PE'),
(4855, 'TAMARANA', 'PR'),
(4856, 'TAMBAU', 'SP'),
(4857, 'TAMBOARA', 'PR'),
(4858, 'TAMBORIL', 'CE'),
(4859, 'TAMBORIL DO PIAUÍ', 'PI'),
(4860, 'TAMMAÇU', 'BA'),
(4861, 'TANABI', 'SP'),
(4862, 'TANGARA', 'RN'),
(4863, 'TANGARA DA SERRA', 'MT'),
(4864, 'TANGUÁ', 'RJ'),
(4865, 'TANQUE D''ARCA', 'AL'),
(4866, 'TANQUE DO PIAUÍ', 'PI'),
(4867, 'TANQUE NOVO', 'BA'),
(4868, 'TANQUINHO', 'BA'),
(4869, 'TAPARUBA', 'MG'),
(4870, 'TAPAUA', 'AM'),
(4871, 'TAPEJARA', 'PR'),
(4872, 'TAPERA', 'RS'),
(4873, 'TAPEROA', 'BA'),
(4874, 'TAPES', 'RS'),
(4875, 'TAPIRA', 'MG'),
(4876, 'TAPIRAÍ', 'MG'),
(4877, 'TAPIRAMUTA', 'BA'),
(4878, 'TAPIRATIBA', 'SP'),
(4879, 'TAPURAII', 'MT'),
(4880, 'TAQUARA', 'RS'),
(4881, 'TAQUARAÇU DE MINAS', 'MG'),
(4882, 'TAQUARAL', 'SP'),
(4883, 'TAQUARAL DE GOIÁS', 'GO'),
(4884, 'TAQUARI', 'RS'),
(4885, 'TAQUARITINGA', 'SP'),
(4886, 'TAQUARITINGA DO NORTE', 'PE'),
(4887, 'TAQUARITUBA', 'SP'),
(4888, 'TAQUARIVAÍ', 'SP'),
(4889, 'TAQUARUÇU DO SUL', 'RS'),
(4890, 'TAQUARUSSU', 'MS'),
(4891, 'TARABAI', 'SP'),
(4892, 'TARAUAÇA', 'AC'),
(4893, 'TARRAFAS', 'CE'),
(4894, 'TARTARUGALZINHO', 'AP'),
(4895, 'TARUMA', 'SP'),
(4896, 'TARUMIRIM', 'MG'),
(4897, 'TASSO FRAGOSO', 'MA'),
(4898, 'TATUI', 'SP'),
(4899, 'TAUA', 'CE'),
(4900, 'TAUBATE', 'SP'),
(4901, 'TAVARES', 'PB'),
(4902, 'TAVARES', 'RS'),
(4903, 'TEFÉ', 'AM'),
(4904, 'TEIXEIRA', 'PB'),
(4905, 'TEIXEIRA DE FREITAS', 'BA'),
(4906, 'TEIXEIRA SOARES', 'PR'),
(4907, 'TEIXEIRAS', 'MG'),
(4908, 'TEJUCUOCA', 'CE'),
(4909, 'TEJUPA', 'SP'),
(4910, 'TELEMACO BORBA', 'PR'),
(4911, 'TELHA', 'SE'),
(4912, 'TENENTE ANANIAS', 'RN'),
(4913, 'TENENTE LAURENTINO CRUZ', 'RN'),
(4914, 'TENENTE PORTELA', 'RS'),
(4915, 'TENÓRIO', 'PB'),
(4916, 'TEODORO SAMPAIO', 'BA'),
(4917, 'TEOFILÂNDIA', 'BA'),
(4918, 'TEOFILO OTONI', 'MG'),
(4919, 'TEOLÂNDIA', 'BA'),
(4920, 'TEOTÔNIO VILELA', 'AL'),
(4921, 'TERENOS', 'MS'),
(4922, 'TERESINA', 'PI'),
(4923, 'TERESINA DE GOIÁS', 'GO'),
(4924, 'TERESÓPOLIS', 'RJ'),
(4925, 'TEREZINHA', 'PE'),
(4926, 'TEREZÓPOLIS DE GOIÁS', 'GO'),
(4927, 'TERRA ALTA', 'PA'),
(4928, 'TERRA BOA', 'PR'),
(4929, 'TERRA DE AREIA', 'RS'),
(4930, 'TERRA NOVA', 'BA'),
(4931, 'TERRA NOVA DO NORTE', 'MT'),
(4932, 'TERRA RICA', 'PR'),
(4933, 'TERRA ROXA', 'PR'),
(4934, 'TERRA SANTA', 'PA'),
(4935, 'TESOURO', 'MT'),
(4936, 'TEUTÔNIA', 'RS'),
(4937, 'THEOBROMA', 'RO'),
(4938, 'TIANGUA', 'CE'),
(4939, 'TIBAGI', 'PR'),
(4940, 'TIBAU', 'RN'),
(4941, 'TIBAU DO SUL', 'RN'),
(4942, 'TIETÊ', 'SP'),
(4943, 'TIGRINHOS', 'SC'),
(4944, 'TIJUCAS', 'SC'),
(4945, 'TIJUCAS DO SUL', 'PR'),
(4946, 'TIMBAUBA', 'PE'),
(4947, 'TIMBAUBA BATISTAS', 'RN'),
(4948, 'TIMBÉ DO SUL', 'SC'),
(4949, 'TIMBIRAS', 'MA'),
(4950, 'TIMBO', 'SC'),
(4951, 'TIMBO GRANDE', 'SC'),
(4952, 'TIMBURI', 'SP'),
(4953, 'TIMON', 'MA'),
(4954, 'TIMOTEO', 'MG'),
(4955, 'TIRADENTES', 'MG'),
(4956, 'TIRADENTES DO SUL', 'RS'),
(4957, 'TIROS', 'MG'),
(4958, 'TOBIAS BARRETO', 'SE'),
(4959, 'TOCANTINIA', 'TO'),
(4960, 'TOCANTINÓPOLIS', 'TO'),
(4961, 'TOCANTINS', 'MG'),
(4962, 'TOLEDO', 'MG'),
(4963, 'TOMAR DO GERU', 'SE'),
(4964, 'TOMAZINA', 'PR'),
(4965, 'TOMBOS', 'MG'),
(4966, 'TOMÉ-AÇU', 'PA'),
(4967, 'TONANTINS', 'AM'),
(4968, 'TORITAMA', 'PE'),
(4969, 'TORIXORÉU', 'MT'),
(4970, 'TOROPI', 'RS'),
(4971, 'TORRE DE PEDRA', 'SP'),
(4972, 'TORRES', 'RS'),
(4973, 'TORRINHA', 'SP'),
(4974, 'TOUROS', 'RN'),
(4975, 'TRABIJU', 'SP'),
(4976, 'TRACUATEUA', 'PA'),
(4977, 'TRAÇUNHAEM', 'PE'),
(4978, 'TRAIPU', 'AL'),
(4979, 'TRAIRÃO', 'PA'),
(4980, 'TRAIRI', 'CE'),
(4981, 'TRAJANO DE MORAES', 'RJ'),
(4982, 'TRAMANDAI', 'RS'),
(4983, 'TRAVESSEIRO', 'RS'),
(4984, 'TREMEDAL', 'BA'),
(4985, 'TREMEMBÉ', 'SP'),
(4986, 'TRÊS ARROIOS', 'RS'),
(4987, 'TRÊS BARRAS', 'SC'),
(4988, 'TRÊS BARRAS DO PARANÁ', 'PR'),
(4989, 'TRÊS CACHOEIRAS', 'RS'),
(4990, 'TRÊS CORAÇÕES', 'MG'),
(4991, 'TRÊS COROAS', 'RS'),
(4992, 'TRÊS DE MAIO', 'RS'),
(4993, 'TRÊS FORQUILHAS', 'RS'),
(4994, 'TRÊS FRONTEIRAS', 'SP'),
(4995, 'TRÊS LAGOAS', 'MS'),
(4996, 'TRÊS MARIAS', 'MG'),
(4997, 'TRÊS PALMEIRAS', 'RS'),
(4998, 'TRÊS PASSOS', 'RS'),
(4999, 'TRÊS PONTAS', 'MG'),
(5000, 'TRÊS RANCHOS', 'GO'),
(5001, 'TRÊS RIOS', 'RJ'),
(5002, 'TREVISO', 'SC'),
(5003, 'TREZE DE MAIO', 'SC'),
(5004, 'TREZE TILIAS', 'SC'),
(5005, 'TRINDADE', 'GO'),
(5006, 'TRINDADE DO SUL', 'RS'),
(5007, 'TRIUNFO', 'PB'),
(5008, 'TRIUNFO POTIGUAR', 'RN'),
(5009, 'TRIZIDELA DO VALE', 'MA'),
(5010, 'TROMBAS', 'GO'),
(5011, 'TROMBUDO CENTRAL', 'SC'),
(5012, 'TUBARÃO', 'SC'),
(5013, 'TUCANO', 'BA'),
(5014, 'TUCUMA', 'PA'),
(5015, 'TUCUNDUVA', 'RS'),
(5016, 'TUCURUÍ', 'PA'),
(5017, 'TUFILÂNDIA', 'MA'),
(5018, 'TUIUTI', 'SP'),
(5019, 'TUMIRITINGA', 'MG'),
(5020, 'TUNÁPOLIS', 'SC'),
(5021, 'TUNAS', 'RS'),
(5022, 'TUNAS DO PARANÁ', 'PR'),
(5023, 'TUNEIRAS DO OESTE', 'PR'),
(5024, 'TUNTUM', 'MA'),
(5025, 'TUPA', 'SP'),
(5026, 'TUPACIGUARA', 'MG'),
(5027, 'TUPANATINGA', 'PE'),
(5028, 'TUPANCI DO SUL', 'RS'),
(5029, 'TUPANCIRETÃ', 'RS'),
(5030, 'TUPANDI', 'RS'),
(5031, 'TUPARENDI', 'RS'),
(5032, 'TUPARETAMA', 'PE'),
(5033, 'TUPASSI', 'PR'),
(5034, 'TUPI PAULISTA', 'SP'),
(5035, 'TUPIRAMA', 'TO'),
(5036, 'TUPIRATINS', 'TO'),
(5037, 'TURIAÇU', 'MA'),
(5038, 'TURILÂNDIA', 'MA'),
(5039, 'TURIÚBA', 'SP'),
(5040, 'TURMALINA', 'MG'),
(5041, 'TURMALINA', 'SP'),
(5042, 'TURUÇU', 'RS'),
(5043, 'TURURU', 'CE'),
(5044, 'TURVANIA', 'GO'),
(5045, 'TURVELÂNDIA', 'GO'),
(5046, 'TURVO', 'PR'),
(5047, 'TURVO', 'SC'),
(5048, 'TURVOLÂNDIA', 'MG'),
(5049, 'TUTOIA', 'MA'),
(5050, 'UARINI', 'AM'),
(5051, 'UAUÁ', 'BA'),
(5052, 'UBA', 'MG'),
(5053, 'UBAÍ', 'MG'),
(5054, 'UBAIRA', 'BA'),
(5055, 'UBAITABA', 'BA'),
(5056, 'UBAJARA', 'CE'),
(5057, 'UBAPORANGA', 'MG'),
(5058, 'UBARANA', 'SP'),
(5059, 'UBATÃ', 'BA'),
(5060, 'UBATUBA', 'SP'),
(5061, 'UBERABA', 'MG'),
(5062, 'UBERLÂNDIA', 'MG'),
(5063, 'UBIRAJARA', 'SP'),
(5064, 'UBIRATA', 'PR'),
(5065, 'UCHOA', 'SP'),
(5066, 'UIBAI', 'BA'),
(5067, 'UIRAPURU', 'GO'),
(5068, 'UIRAUNA', 'PB'),
(5069, 'ULIANÓPOLIS', 'PA'),
(5070, 'UMARI', 'CE'),
(5071, 'UMARIZAL', 'RN'),
(5072, 'UMBAUBA', 'SE'),
(5073, 'UMBURANAS', 'BA'),
(5074, 'UMBURATIBA', 'MG'),
(5075, 'UMBUZEIRO', 'PB'),
(5076, 'UMIRIM', 'CE'),
(5077, 'UMUARAMA', 'PR'),
(5078, 'UNA', 'BA'),
(5079, 'UNAI', 'MG'),
(5080, 'UNIÃO', 'PI'),
(5081, 'UNIÃO DA SERRA', 'RS'),
(5082, 'UNIÃO DA VITÓRIA', 'PR'),
(5083, 'UNIÃO DE MINAS', 'MG'),
(5084, 'UNIÃO DO OESTE', 'SC'),
(5085, 'UNIÃO DO SUL', 'MT'),
(5086, 'UNIÃO DOS PALMARES', 'AL'),
(5087, 'UNIÃO PAULISTA', 'SP'),
(5088, 'UNIFLOR', 'PR'),
(5089, 'UPANEMA', 'RN'),
(5090, 'URAI', 'PR'),
(5091, 'URANDI', 'BA'),
(5092, 'URANIA', 'SP'),
(5093, 'URBANO SANTOS', 'MA'),
(5094, 'URU', 'SP'),
(5095, 'URUAÇU', 'GO'),
(5096, 'URUANA', 'GO'),
(5097, 'URUANA DE MINAS', 'MG'),
(5098, 'URUARA', 'PA'),
(5099, 'URUBICI', 'SC'),
(5100, 'URUBURETAMA', 'CE'),
(5101, 'URUCÂNIA', 'MG'),
(5102, 'URUCARA', 'AM'),
(5103, 'URUÇUCA', 'BA'),
(5104, 'URUÇUÍ', 'PI'),
(5105, 'URUCUIA', 'MG'),
(5106, 'URUCURITUBA', 'AM'),
(5107, 'URUGUAIANA', 'RS'),
(5108, 'URUOCA', 'CE'),
(5109, 'URUPA', 'RO'),
(5110, 'URUPEMA', 'SC'),
(5111, 'URUPÊS', 'SP'),
(5112, 'URUSSANGA', 'SC'),
(5113, 'URUTAI', 'GO'),
(5114, 'UTINGA', 'BA'),
(5115, 'VACARIA', 'RS'),
(5116, 'VALE DO PARAISO', 'RO'),
(5117, 'VALE DO SOL', 'RS'),
(5118, 'VALE REAL', 'RS'),
(5119, 'VALE VERDE', 'RS'),
(5120, 'VALENCA', 'BA'),
(5121, 'VALENÇA', 'RJ'),
(5122, 'VALENCA DO PIAUÍ', 'PI'),
(5123, 'VALENTE', 'BA'),
(5124, 'VALENTIM GENTIL', 'SP'),
(5125, 'VALINHOS', 'SP'),
(5126, 'VALPARAISO', 'GO'),
(5127, 'VANINI', 'RS'),
(5128, 'VARGEÃO', 'SC'),
(5129, 'VARGEM', 'SC'),
(5130, 'VARGEM', 'SP'),
(5131, 'VARGEM ALTA', 'ES'),
(5132, 'VARGEM BONITA', 'MG'),
(5133, 'VARGEM BONITA', 'SC'),
(5134, 'VARGEM GRANDE', 'MA'),
(5135, 'VARGEM GRANDE DO SUL', 'SP'),
(5136, 'VARGEM GRANDE PAULISTA', 'SP'),
(5137, 'VARGINHA', 'MG'),
(5138, 'VARJÃO', 'GO'),
(5139, 'VARJÃO DE MINAS', 'MG'),
(5140, 'VARJOTA', 'CE'),
(5141, 'VARRE-SAI', 'RJ'),
(5142, 'VÁRZEA', 'PB'),
(5143, 'VARZEA ALEGRE', 'CE'),
(5144, 'VARZEA BRANCA', 'PI'),
(5145, 'VÁRZEA DA PALMA', 'MG'),
(5146, 'VÁRZEA DA ROÇA', 'BA'),
(5147, 'VARZEA DO POÇO', 'BA'),
(5148, 'VARZEA GRANDE', 'MT'),
(5149, 'VARZEA NOVA', 'BA'),
(5150, 'VARZEA PAULISTA', 'SP'),
(5151, 'VARZEDO', 'BA'),
(5152, 'VARZELÂNDIA', 'MG'),
(5153, 'VASSOURAS', 'RJ'),
(5154, 'VAZANTE', 'MG'),
(5155, 'VENÂNCIO AIRES', 'RS'),
(5156, 'VENDA NOVA DO IMIGRANTE', 'ES'),
(5157, 'VENHA VER', 'RN'),
(5158, 'VENTANIA', 'PR'),
(5159, 'VENTUROSA', 'PE'),
(5160, 'VERA', 'MT'),
(5161, 'VERA CRUZ', 'BA'),
(5162, 'VERA CRUZ DO OESTE', 'PR'),
(5163, 'VERA MENDES', 'PI'),
(5164, 'VERANÓPOLIS', 'RS'),
(5165, 'VERDEJANTE', 'PE'),
(5166, 'VERDELÂNDIA', 'MG'),
(5167, 'VERE', 'PR'),
(5168, 'VEREDA', 'BA'),
(5169, 'VEREDINHA', 'MG'),
(5170, 'VERÍSSIMO', 'MG'),
(5171, 'VERTENTE DO LÉRIO', 'PE'),
(5172, 'VERTENTES', 'PE'),
(5173, 'VESPASIANO', 'MG'),
(5174, 'VESPASIANO CORREA', 'RS'),
(5175, 'VIADUTOS', 'RS'),
(5176, 'VIAMÃO', 'RS'),
(5177, 'VIANA', 'ES'),
(5178, 'VIANÓPOLIS', 'GO'),
(5179, 'VICENCIA', 'PE'),
(5180, 'VICENTE DUTRA', 'RS'),
(5181, 'VICENTINA', 'MS'),
(5182, 'VICENTINÓPOLIS', 'GO'),
(5183, 'VICOSA', 'AL'),
(5184, 'VIÇOSA', 'MG'),
(5185, 'VICOSA DO CEARÁ', 'CE'),
(5186, 'VICTOR GRAEFF', 'RS'),
(5187, 'VIDAL RAMOS', 'SC'),
(5188, 'VIDEIRA', 'SC'),
(5189, 'VIEIRAS', 'MG'),
(5190, 'VIEIRÓPOLIS', 'PB'),
(5191, 'VIGIA', 'PA'),
(5192, 'ALTO PARAÍSO', 'PR'),
(5193, 'VILA BELA SS. TRINDADE', 'MT'),
(5194, 'VILA BOA', 'GO'),
(5195, 'VILA FLOR', 'RN'),
(5196, 'VILA FLORES', 'RS'),
(5197, 'VILA LANGARO', 'RS'),
(5198, 'VILA MARIA', 'RS'),
(5199, 'VILA NEWTON BELLO', 'MA'),
(5200, 'VILA NOVA DO PIAUÍ', 'PI'),
(5201, 'VILA NOVA DO SUL', 'RS'),
(5202, 'VILA NOVA DOS MARTIRIOS', 'MA'),
(5203, 'VILA PAULINO NEVES', 'MA'),
(5204, 'VILA PAVAO', 'ES'),
(5205, 'VILA PROPICIO', 'GO'),
(5206, 'VILA RICA', 'MT'),
(5207, 'VILA VALERIO', 'ES'),
(5208, 'VILA VELHA', 'ES'),
(5209, 'VILHENA', 'RO'),
(5210, 'VINHEDO', 'SP'),
(5211, 'VIRADOURO', 'SP'),
(5212, 'VIRGEM DA LAPA', 'MG'),
(5213, 'VIRGINIA', 'MG'),
(5214, 'VIRGINÓPOLIS', 'MG'),
(5215, 'VIRGOLÂNDIA', 'MG'),
(5216, 'VIRMOND', 'PR'),
(5217, 'VISCONDE DO RIO BRANCO', 'MG'),
(5218, 'VISEU', 'PA'),
(5219, 'VISTA ALEGRE', 'RS'),
(5220, 'VISTA ALEGRE DO ALTO', 'SP'),
(5221, 'VISTA ALEGRE DO PRATA', 'RS'),
(5222, 'VISTA ALEGRE DO PRETA', 'RS'),
(5223, 'VISTA GAÚCHA', 'RS'),
(5224, 'VISTA SERRANA', 'PB'),
(5225, 'VITOR MEIRELES', 'SC'),
(5226, 'VITÓRIA', 'ES'),
(5227, 'VITÓRIA BRASIL', 'SP'),
(5228, 'VITÓRIA DA CONQUISTA', 'BA'),
(5229, 'VITÓRIA DAS MISSÕES', 'RS'),
(5230, 'VITÓRIA DE SANTO ANTÃO', 'PE'),
(5231, 'VITÓRIA DO JARI', 'AP'),
(5232, 'VITÓRIA DO MEARIM', 'MA'),
(5233, 'VITÓRIA DO XINGU', 'PA'),
(5234, 'VITORINO', 'PR'),
(5235, 'VITORINO FREIRE', 'MA'),
(5236, 'VOLTA GRANDE', 'MG'),
(5237, 'VOLTA REDONDA', 'RJ'),
(5238, 'VOTORANTIM', 'SP'),
(5239, 'VOTUPORANGA', 'SP'),
(5240, 'WAGNER', 'BA'),
(5241, 'WALL FERRAZ', 'PI'),
(5242, 'WANDERLÂNDIA', 'TO'),
(5243, 'WANDERLEY', 'BA'),
(5244, 'WENCESLAU BRAS', 'PR'),
(5245, 'WENCESLAU BRAZ', 'MG'),
(5246, 'WENCESLAU GUIMARAES', 'BA'),
(5247, 'WITMARSUN', 'SC'),
(5248, 'XAMBIOA', 'TO'),
(5249, 'XAMBRÊ', 'PR'),
(5250, 'XANGRI-LA', 'RS'),
(5251, 'XANXERÊ', 'SC'),
(5252, 'XAPURI', 'AC'),
(5253, 'XAVANTINA', 'SC'),
(5254, 'XAXIM', 'SC'),
(5255, 'XEXEU', 'PE'),
(5256, 'XINGUARA', 'PA'),
(5257, 'XIQUE XIQUE', 'BA'),
(5258, 'ZABELE', 'PB'),
(5259, 'ZACARIAS', 'SP'),
(5260, 'ZE DOCA', 'MA'),
(5261, 'ZORTÉA', 'SC'),
(5262, 'SÍTIO NOVO', 'RN'),
(5263, 'ALTO ALEGRE DO PINDARÉ', 'MA'),
(5264, 'VARGEM ALEGRE', 'MG'),
(5265, 'SANTANA', 'BA'),
(5266, 'TRINDADE', 'PE'),
(5267, 'BONITO DE MINAS', 'MG'),
(5268, 'JUVENÍLIA', 'MG'),
(5269, 'NOVA NAZARÉ', 'MT'),
(5270, 'ITAOCARA', 'RJ'),
(5271, 'CUJUBIM', 'RO'),
(5272, 'ARARUNA', 'PB'),
(5273, 'SÃO FRANCISCO', 'PB'),
(5274, 'MACURURÉ', 'BA'),
(5275, 'SÃO BENTINHO', 'PB'),
(5276, 'BARCARENA', 'PA'),
(5277, 'ITAJÁ', 'RN'),
(5278, 'MONTEVIDIO DO NORTE', 'GO'),
(5279, 'ENTRE RIOS', 'SC'),
(5280, 'SERRINHA', 'RN'),
(5281, 'IRATI ', 'SC'),
(5282, 'PALMEIRINA', 'PE'),
(5283, 'VALE DE SÃO DOMINGOS', 'MT'),
(5284, 'NOVO HORIZONTE', 'SC'),
(5285, 'ARAGUAINHA', 'MT'),
(5286, 'ACRELÂNDIA', 'AC'),
(5287, 'BOM JARDIM', 'PE'),
(5288, 'SÃO SEBASTIÃO DA VARGEM ALEGRE', 'MG'),
(5289, 'SANTA LUZIA DO PARUÁ', 'MA'),
(5290, 'SERRA NOVA DOURADA', 'MT'),
(5291, 'SANTA RITA DO TRIVELATO', 'MT'),
(5292, 'TAPEROÁ', 'PB'),
(5293, 'JACUÍPE', 'AL'),
(5294, 'BONITO', 'PE'),
(5295, 'SALGADINHO', 'PE'),
(5296, 'TAPURAH', 'MT'),
(5297, 'PARAGUAÇU', 'SP'),
(5298, 'SANTA TEREZINHA', 'PE'),
(5299, 'ALTO ALEGRE DO PARECIS', 'RO'),
(5300, 'PIÇARRA', 'PA'),
(5301, 'INDAIABIRA', 'MG'),
(5302, 'ABADIA DE GOIÁS', 'GO'),
(5303, 'IGRAPIÚNA', 'BA'),
(5304, 'IGRAPIÚNA', 'BA'),
(5305, 'VALE DO ANARI', 'RO'),
(5306, 'BOM JESUS DO ARAGUAIA', 'MT'),
(5307, 'VIÇOSA', 'AL'),
(5308, 'SATUBA', 'AL'),
(5309, 'FÁTIMA', 'TO'),
(5310, 'ÁGUA BRANCA', 'PI'),
(5311, 'ALTO ALEGRE', 'RR'),
(5312, 'RIACHÃO', 'PB'),
(5313, 'PENDÊNCIAS', 'RN'),
(5314, 'BARRA DE ALCÂNTARA', 'PI'),
(5315, 'SÃO DOMINGOS', 'GO'),
(5316, 'ESTÂNCIA TURÍSTICA DE JOANÓPOLIS', 'SP'),
(5317, 'RESERVA DO IGUAÇU', 'PR'),
(5318, 'IBOTIRAMA', 'BA'),
(5319, 'CAFELÂNDIA', 'SP'),
(5320, 'SANTA CRUZ DO XINGU', 'MT'),
(5321, 'BANDEIRANTE', 'SC'),
(5322, 'CANTAGALO', 'MG'),
(5323, 'ARAGUAIANA', 'MT'),
(5324, 'CAPINZAL DO NORTE', 'MA'),
(5325, 'MOREILÂNDIA', 'PE'),
(5326, 'LAGOA DO ITAENGA', 'PE'),
(5327, 'TEIXEIRÓPOLIS', 'RO'),
(5328, 'CARAÚBAS', 'RN'),
(5329, 'PAÇO DO LUMIAR', 'MA'),
(5330, 'BOM JESUS', 'PI'),
(5331, 'CARACOL ', 'PI'),
(5332, 'JAGUAPITÃ', 'PR'),
(5333, 'BELA VISTA DO MARANHÃO', 'MA'),
(5334, 'SANTANA DE MANGUEIRA', 'PB'),
(5335, 'BOA ESPERANÇA', 'MG'),
(5336, 'CANÁPOLIS', 'MG'),
(5337, 'ALAGOA', 'MG'),
(5338, 'CONCEIÇÃO DA APARECIDA', 'MG'),
(5339, 'CONGONHAS', 'MG'),
(5340, 'OURO BRANCO', 'MG'),
(5341, 'PINTÓPOLIS', 'MG'),
(5342, 'RESENDE COSTA', 'MG'),
(5343, 'SÃO JOSÉ DA LAPA', 'MG'),
(5344, 'SÃO JOSÉ DO DIVINO', 'MG'),
(5345, 'SAO CARLOS', 'SP'),
(5346, 'ANCHIETA', 'SC'),
(5347, 'IRAQUARA', 'BA'),
(5348, 'CRUZEIRO DO SUL', 'PR'),
(5349, 'ITAMBÉ', 'PR'),
(5350, 'SANTA INÊS', 'PR'),
(5351, 'LUÍS EDUARDO MAGALHÃES', 'BA');

-- --------------------------------------------------------

--
-- Table structure for table `nfe_creditos`
--

CREATE TABLE IF NOT EXISTS `nfe_creditos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `credito` float(5,2) DEFAULT '0.00',
  `tipopessoa` varchar(4) DEFAULT 'PFPJ',
  `issretido` varchar(1) DEFAULT 'N',
  `valor` float(10,2) DEFAULT '0.00',
  `estado` char(1) DEFAULT 'A',
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=3 ;

--
-- Dumping data for table `nfe_creditos`
--

INSERT INTO `nfe_creditos` (`codigo`, `credito`, `tipopessoa`, `issretido`, `valor`, `estado`) VALUES
(2, 0.00, 'PFPJ', 'N', 0.00, 'A');

-- --------------------------------------------------------

--
-- Table structure for table `notas`
--

CREATE TABLE IF NOT EXISTS `notas` (
  `codigo` bigint(20) NOT NULL AUTO_INCREMENT,
  `numero` int(10) DEFAULT NULL,
  `codverificacao` varchar(9) DEFAULT NULL,
  `datahoraemissao` datetime DEFAULT NULL,
  `codemissor` int(10) DEFAULT NULL,
  `rps_numero` varchar(20) DEFAULT NULL,
  `rps_data` date DEFAULT NULL,
  `tomador_nome` varchar(100) NOT NULL DEFAULT '',
  `tomador_cnpjcpf` varchar(18) NOT NULL DEFAULT '',
  `tomador_inscrmunicipal` varchar(20) DEFAULT NULL,
  `tomador_endereco` varchar(100) DEFAULT NULL,
  `tomador_logradouro` varchar(80) DEFAULT NULL,
  `tomador_numero` int(11) DEFAULT NULL,
  `tomador_complemento` varchar(80) DEFAULT NULL,
  `tomador_bairro` varchar(80) DEFAULT NULL,
  `tomador_cep` varchar(9) DEFAULT NULL,
  `tomador_municipio` varchar(100) DEFAULT NULL,
  `tomador_uf` char(2) DEFAULT NULL,
  `tomador_email` varchar(100) DEFAULT NULL,
  `discriminacao` varchar(400) DEFAULT NULL,
  `observacao` text,
  `valortotal` float(10,2) DEFAULT NULL,
  `valordeducoes` float(10,2) DEFAULT NULL,
  `basecalculo` float(10,2) DEFAULT NULL,
  `valoriss` float(10,2) DEFAULT NULL,
  `issretido` float(10,2) DEFAULT NULL COMMENT 'Valor do iss retido',
  `valorinss` float(10,2) DEFAULT NULL,
  `aliqinss` float(10,2) DEFAULT NULL,
  `valorirrf` float(10,2) DEFAULT NULL,
  `aliqirrf` float(10,2) DEFAULT NULL,
  `deducao_irrf` float(11,2) DEFAULT NULL,
  `total_retencao` float(11,2) DEFAULT NULL,
  `credito` float(10,2) DEFAULT NULL,
  `estado` varchar(20) DEFAULT NULL COMMENT 'Estado da nfe, valores N  para normal, B boleto gerado, E nota escriturada',
  `tipoemissao` varchar(10) DEFAULT 'online' COMMENT 'Tipo da nfe emitida, "online" ou "importada"',
  `motivo_cancelamento` text CHARACTER SET utf8,
  PRIMARY KEY (`codigo`),
  KEY `emissor` (`codemissor`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `notas_servicos`
--

CREATE TABLE IF NOT EXISTS `notas_servicos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codnota` int(11) DEFAULT NULL,
  `codservico` bigint(11) DEFAULT NULL,
  `basecalculo` float(10,2) DEFAULT NULL,
  `issretido` float(10,2) DEFAULT NULL,
  `iss` float(10,2) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `noticias`
--

CREATE TABLE IF NOT EXISTS `noticias` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `titulo` varchar(100) DEFAULT '',
  `texto` varchar(500) DEFAULT NULL,
  `data` date DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `obras`
--

CREATE TABLE IF NOT EXISTS `obras` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `obra` varchar(100) DEFAULT NULL,
  `alvara` varchar(20) DEFAULT NULL,
  `iptu` varchar(20) DEFAULT NULL,
  `endereco` varchar(100) DEFAULT NULL,
  `proprietario` varchar(100) DEFAULT NULL,
  `proprietario_cnpjcpf` char(18) DEFAULT NULL,
  `dataini` date DEFAULT NULL,
  `datafim` date DEFAULT NULL,
  `listamateriais` text,
  `valormateriais` decimal(10,2) DEFAULT NULL,
  `estado` char(1) DEFAULT 'A' COMMENT 'A para aberto e C para concluido',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `operadoras_creditos`
--

CREATE TABLE IF NOT EXISTS `operadoras_creditos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL,
  `codbanco` int(11) DEFAULT NULL,
  `agencia` varchar(20) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `orgaospublicos`
--

CREATE TABLE IF NOT EXISTS `orgaospublicos` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codcadastro` int(11) DEFAULT NULL COMMENT 'codigo do orgao publico',
  `admpublica` char(1) DEFAULT NULL COMMENT 'D direta ou I indireta',
  `nivel` char(1) DEFAULT NULL COMMENT 'M municipal, E estadual, F federal',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais`
--

CREATE TABLE IF NOT EXISTS `processosfiscais` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codemissor` int(11) DEFAULT NULL,
  `nroprocesso` int(11) DEFAULT NULL,
  `anoprocesso` char(4) DEFAULT NULL,
  `abertura` char(1) DEFAULT NULL COMMENT 'I para individual e G para geral',
  `data_abertura` date DEFAULT NULL,
  `data_inicial` date DEFAULT NULL,
  `data_final` date DEFAULT NULL,
  `observacoes` text,
  `intimacao` char(1) DEFAULT 'S' COMMENT 'N para não e S para Sim',
  `situacao` char(1) DEFAULT 'A' COMMENT 'A para aberto e C para concluido',
  `cancelado` char(1) DEFAULT 'N' COMMENT 'S para sim N para não',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_apreensoes`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_apreensoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nroprocesso` int(11) DEFAULT NULL,
  `anoprocesso` char(4) DEFAULT NULL,
  `nroapreensao` int(11) DEFAULT NULL,
  `anoapreensao` char(4) DEFAULT NULL,
  `dataemissao` date DEFAULT NULL,
  `observacoes` text,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_apreensoes_docs`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_apreensoes_docs` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codapreensao` varchar(11) DEFAULT NULL,
  `coddoc` varchar(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_autuacoes`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_autuacoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nroautuacao` int(11) DEFAULT NULL,
  `anoautuacao` varchar(4) DEFAULT NULL,
  `nroprocesso` varchar(10) DEFAULT NULL,
  `anoprocesso` varchar(4) DEFAULT NULL,
  `codinfracao` int(11) DEFAULT NULL,
  `titulo` varchar(50) DEFAULT NULL,
  `tiposervico` char(1) DEFAULT 'P' COMMENT 'P para prestado e T para tomado',
  `historico` text,
  `obrigacao` char(1) DEFAULT 'P' COMMENT 'P para principal e A para acessória',
  `reincidencia` char(1) DEFAULT NULL COMMENT 'S ="sim, tem reicidencia" N ="nao tem reincidencia"',
  `quantidade` int(8) DEFAULT NULL COMMENT 'quantidade de reincidencia',
  `multa` decimal(10,2) DEFAULT NULL COMMENT 'porcentagem de multa',
  `valor` decimal(10,2) DEFAULT '0.00',
  `nroparcelas` int(11) NOT NULL DEFAULT '1' COMMENT 'Quantidade de parcelas',
  `situacao` char(1) DEFAULT 'E' COMMENT 'E para emitido e N para não emitido',
  `cancelado` char(1) DEFAULT 'A' COMMENT 'A para aberto C para cancelado',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='Quantidade de parcelas' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_autuacoes_docs`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_autuacoes_docs` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codautuacao` varchar(10) DEFAULT NULL,
  `coddoc` varchar(10) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_ciente_vencimento`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_ciente_vencimento` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codautuacao` int(11) DEFAULT NULL,
  `dataciente` date DEFAULT NULL,
  `datavencimento` date DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_competencias`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_competencias` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codautuacao` int(11) DEFAULT NULL,
  `competencia` varchar(7) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='competencias sao as contas vencidas e nao pagas' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_dividaativa`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_dividaativa` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codautuacao` varchar(11) DEFAULT NULL,
  `nrodivida` int(11) DEFAULT NULL,
  `anodivida` char(4) DEFAULT NULL,
  `datainscricao` date DEFAULT NULL,
  `datalimite` date DEFAULT NULL,
  `observacoes` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_docs`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_docs` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `nrodoc` varchar(50) DEFAULT NULL,
  `descricao` varchar(200) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_fiscais`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_fiscais` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codprocesso` int(11) DEFAULT NULL,
  `codfiscal` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_fundamentacaolegal`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_fundamentacaolegal` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `incidencia` text,
  `dispositivoinfringido` text,
  `atualizacaomonetaria` text,
  `juros` text,
  `multa` text,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_guias`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_guias` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codautuacao` int(11) DEFAULT NULL,
  `numero` int(11) DEFAULT NULL,
  `valor` decimal(10,2) DEFAULT NULL,
  `vencimento` date DEFAULT NULL,
  `datacientificacao` date DEFAULT NULL,
  `valorpago` decimal(10,2) DEFAULT NULL,
  `situacao` char(1) DEFAULT NULL COMMENT 'A para aberta e P para paga',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_homologacao`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_homologacao` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codservico` varchar(11) DEFAULT NULL,
  `nrodoc` varchar(11) DEFAULT NULL,
  `nroprocesso` int(11) DEFAULT NULL,
  `anoprocesso` char(4) DEFAULT NULL,
  `situacao_tributo` char(1) DEFAULT NULL COMMENT 'P para pago e N para nao pago',
  `valortotal` decimal(10,2) DEFAULT NULL,
  `iss` decimal(10,2) DEFAULT NULL,
  `issretido` decimal(10,2) DEFAULT NULL,
  `deducao` decimal(10,2) DEFAULT NULL,
  `lps` varchar(100) DEFAULT NULL COMMENT 'Local da Prestacao de Servico',
  `competencia` date DEFAULT NULL,
  `cpfcnpjtomador` varchar(25) DEFAULT NULL,
  `cpfcnpjprestador` varchar(25) DEFAULT NULL COMMENT 'informacao necessaria para os servicos tomados',
  `tipo` char(1) DEFAULT NULL COMMENT 'P para servicos prestados e T para servicos tomados',
  `homologado` char(1) DEFAULT 'N' COMMENT 'S para sim e N para nao',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_infracoes`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_infracoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nroinfracao` varchar(8) DEFAULT NULL,
  `anoinfracao` int(4) DEFAULT NULL,
  `tituloinfracao` text,
  `descricao` text,
  `fundamentacaolegal` text,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_infracoes_fundamentacaolegal`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_infracoes_fundamentacaolegal` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codinfracoes` int(11) DEFAULT NULL,
  `codfundamentacaolegal` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_intimacoes`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_intimacoes` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nroprocesso` int(11) DEFAULT NULL,
  `anoprocesso` varchar(4) DEFAULT NULL,
  `nrointimacao` int(11) DEFAULT NULL,
  `anointimacao` varchar(4) DEFAULT NULL,
  `dataemissao` date DEFAULT NULL,
  `prazo` int(3) DEFAULT NULL,
  `situacao` char(1) DEFAULT 'A' COMMENT 'A para aberto e C para concluido',
  `observacoes` text,
  `codlegislacao` int(11) DEFAULT NULL,
  `cancelado` char(1) DEFAULT 'N' COMMENT 'S para cancelado N para nao cancelado',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_intimacoes_docs`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_intimacoes_docs` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codintimacao` int(11) DEFAULT NULL,
  `coddoc` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_prorrogacao`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_prorrogacao` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nroprocesso` int(11) DEFAULT NULL,
  `anoprocesso` char(4) DEFAULT NULL,
  `nroprorrogacao` int(11) DEFAULT NULL,
  `anoprorrogacao` char(4) DEFAULT NULL,
  `dataemissao` date DEFAULT NULL,
  `diasprorrogacao` char(3) DEFAULT NULL,
  `dataprorrogada` date DEFAULT NULL,
  `observacoes` text,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_ted`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_ted` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nroted` int(11) DEFAULT NULL,
  `anoted` char(4) DEFAULT NULL,
  `nroprocesso` int(11) DEFAULT NULL,
  `anoprocesso` int(11) DEFAULT NULL,
  `dataemissao` date DEFAULT NULL,
  `observacoes` text,
  `situacao` char(1) DEFAULT 'A' COMMENT 'A para aberto e C para concluido',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='Tabela de registros do Termo de Entrega de Documentos' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_ted_docs`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_ted_docs` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codted` varchar(11) DEFAULT NULL,
  `coddoc` varchar(11) DEFAULT NULL,
  `estado` char(1) DEFAULT 'P' COMMENT 'E para enviado e P para pendente e T para temporario',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_tif`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_tif` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codlegislacao` int(11) DEFAULT NULL,
  `nrotif` int(11) DEFAULT NULL,
  `anotif` varchar(4) DEFAULT NULL,
  `nroprocesso` int(11) DEFAULT NULL,
  `anoprocesso` varchar(4) DEFAULT NULL,
  `codemissor` int(10) DEFAULT NULL,
  `datainicial` date DEFAULT NULL,
  `datafinal` date DEFAULT NULL,
  `dias` int(5) DEFAULT NULL,
  `observacoes` text,
  `intimacao` char(1) DEFAULT 'S',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC COMMENT='Tabela de registros do Termo de Inicio de Fiscalizacao' AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `processosfiscais_tif_docs`
--

CREATE TABLE IF NOT EXISTS `processosfiscais_tif_docs` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codtif` int(10) DEFAULT NULL,
  `coddoc` int(10) DEFAULT NULL,
  `estado` char(1) DEFAULT 'E' COMMENT 'E para enviado e P para pendente',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=FIXED AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `reclamacoes`
--

CREATE TABLE IF NOT EXISTS `reclamacoes` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `assunto` varchar(100) DEFAULT NULL,
  `descricao` text,
  `especificacao` varchar(200) DEFAULT '',
  `tomador_cnpj` varchar(20) DEFAULT NULL,
  `tomador_email` varchar(200) DEFAULT NULL,
  `rps_numero` varchar(100) DEFAULT NULL,
  `rps_data` date DEFAULT NULL,
  `rps_valor` float(10,2) DEFAULT NULL,
  `emissor_cnpjcpf` varchar(200) DEFAULT NULL,
  `datareclamacao` date DEFAULT NULL,
  `estado` varchar(10) DEFAULT NULL,
  `responsavel` varchar(100) DEFAULT NULL,
  `dataatendimento` date DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `servicos`
--

CREATE TABLE IF NOT EXISTS `servicos` (
  `codigo` bigint(11) NOT NULL AUTO_INCREMENT COMMENT 'Codigo do banco de dados para servicos',
  `codcategoria` int(11) DEFAULT NULL,
  `codservico` varchar(200) DEFAULT NULL COMMENT 'Codigo do servico pela prefeitura municipal',
  `descricao` text COMMENT 'Descricao do servico',
  `tipopessoa` varchar(10) DEFAULT NULL COMMENT 'Tipo de pessoa PJ Pessoa Juridica PF Pessoa Fisica',
  `aliquota` float(10,2) DEFAULT '0.00' COMMENT 'Porcentagem de aliquota para servicos',
  `aliquotair` float(10,2) DEFAULT '0.00' COMMENT 'Porcentagem de aliquota para servicos com iss retido',
  `aliquotainss` float(10,2) DEFAULT '0.00' COMMENT 'Porcentagem de aliquota de INSS',
  `aliquotairrf` float(10,2) DEFAULT '0.00' COMMENT 'Porcentagem de aliq. Imposto de Renda Retido na Fonte',
  `basecalculo` float(10,2) DEFAULT '0.00' COMMENT 'Base de calculo para ISS 0 igual ao preco servico',
  `incidencia` varchar(50) DEFAULT 'mensal' COMMENT 'incidencia do periodo do iss padrao eh mensal',
  `valor_rpa` float(10,2) DEFAULT '0.00',
  `datavenc` int(2) DEFAULT NULL COMMENT 'Data de vencimento do iss por dia',
  `docfiscal` varchar(50) DEFAULT NULL COMMENT 'Documento fiscal exigido',
  `estado` char(3) DEFAULT NULL COMMENT 'Estado do servico, valores A ou I',
  PRIMARY KEY (`codigo`),
  KEY `fkcategoria` (`codcategoria`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=202 ;

--
-- Dumping data for table `servicos`
--

INSERT INTO `servicos` (`codigo`, `codcategoria`, `codservico`, `descricao`, `tipopessoa`, `aliquota`, `aliquotair`, `aliquotainss`, `aliquotairrf`, `basecalculo`, `incidencia`, `valor_rpa`, `datavenc`, `docfiscal`, `estado`) VALUES
(1, 1, '1.01', 'Análise e desenvolvimento de sistemas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(2, 1, '1.02', 'Programação.', 'PJPF', 1.40, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(3, 1, '1.03', 'Processamento de dados e congêneres.', 'PJPF', 1.50, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(4, 1, '1.04', 'Elaboração de programas de computadores, inclusive de jogos eletrônicos.', 'PJPF', 1.75, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(5, 1, '1.05', 'Licenciamento ou cessão de direito de uso de programas de computação.', 'PJPF', 1.90, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(6, 1, '1.06', 'Assessoria e consultoria em informática.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(7, 1, '1.07', 'Suporte técnico em informática, inclusive instalação, configuração e manutenção de programas de computação e bancos de dados.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(8, 1, '1.08', 'Planejamento, confecção, manutenção e atualização de páginas eletrônicas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(9, 2, '2.01', 'Serviços de pesquisas e desenvolvimento de qualquer natureza.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(10, 3, '3.02', 'Cessão de direito de uso de marcas e de sinais de propaganda.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(11, 3, '3.03', 'Exploração de salões de festas, centro de convenções, escritórios virtuais, stands, quadras esportivas, estádios, ginásios, auditórios, casas de espetáculos, parques de diversões, canchas e congêneres, para realização de eventos ou negócios de qualquer natureza.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(12, 3, '3.04', 'Locação, sublocação, arrendamento, direito de passagem ou permissão de uso, compartilhado ou não, de ferrovia, rodovia, postes, cabos, dutos e condutos de qualquer natureza.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(13, 3, '3.05', 'Cessão de andaimes, palcos, coberturas e outras estruturas de uso temporário.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(14, 4, '4.01', 'Medicina e biomedicina.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(15, 4, '4.02', 'Análises clínicas, patologia, eletricidade médica, radioterapia, quimioterapia, ultra-sonografia, ressonância magnética, radiologia, tomografia e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(16, 4, '4.03', 'Hospitais, clínicas, laboratórios, sanatórios, manicômios, casas de saúde, prontos-socorros, ambulatórios e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(17, 4, '4.04', 'Instrumentação cirúrgica.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(18, 4, '4.05', 'Acupuntura.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(19, 4, '4.06', 'Enfermagem, inclusive serviços auxiliares.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(20, 4, '4.07', 'Serviços farmacêuticos.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(21, 4, '4.08', 'Terapia ocupacional, fisioterapia e fonoaudiologia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(22, 4, '4.09', 'Terapias de qualquer espécie destinadas ao tratamento físico, orgânico e mental.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(23, 4, '4.10', 'Nutrição.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(24, 4, '4.11', 'Obstetrícia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(25, 4, '4.12', 'Odontologia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(26, 4, '4.13', 'Ortóptica.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(27, 4, '4.14', 'Próteses sob encomenda.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(28, 4, '4.15', 'Serviços relativos a obras de arte sob encomenda.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(29, 4, '4.16', 'Psicologia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(30, 4, '4.17', 'Casas de repouso e de recuperação, creches, asilos e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(31, 4, '4.18', 'Inseminação artificial, fertilização in vitro e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(32, 4, '4.19', 'Bancos de sangue, leite, pele, olhos, óvulos, sêmen e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(33, 4, '4.20', 'Coleta de sangue, leite, tecidos, sêmen, órgãos e materiais biológicos de qualquer espécie.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(34, 4, '4.21', 'Unidade de atendimento, assistência ou tratamento móvel e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(35, 4, '4.22', 'Planos de medicina de grupo ou individual e convênios para prestação de assistência médica, hospitalar, odontológica e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(36, 4, '4.23', 'Outros planos de saúde que se cumpram através de serviços de terceiros contratados, credenciados, cooperados ou apenas pagos pelo operador do plano mediante indicação do beneficiário.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(37, 5, '5.01', 'Medicina veterinária e zootecnia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(38, 5, '5.02', 'Hospitais, clínicas, ambulatórios, prontos-socorros e congêneres, na área veterinária.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(39, 5, '5.03', 'Laboratórios de análise na área veterinária.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(40, 5, '5.04', 'Inseminação artificial, fertilização in vitro e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(41, 5, '5.05', 'Bancos de sangue e de órgãos e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(42, 5, '5.06', 'Coleta de sangue, leite, tecidos, sêmen, órgãos e materiais biológicos de qualquer espécie.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(43, 5, '5.07', 'Unidade de atendimento, assistência ou tratamento móvel e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(44, 5, '5.08', 'Guarda, tratamento, amestramento, embelezamento, alojamento e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(45, 5, '5.09', 'Planos de atendimento e assistência médico-veterinária.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(46, 6, '6.01', 'Barbearia, cabeleireiros, manicuros, pedicuros e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(47, 6, '6.02', 'Esteticistas, tratamento de pele, depilação e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(48, 6, '6.03', 'Banhos, duchas, sauna, massagens e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(49, 6, '6.04', 'Ginástica, dança, esportes, natação, artes marciais e demais atividades físicas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(50, 6, '6.05', 'Centros de emagrecimento, spa e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(51, 7, '7.01', 'Engenharia, agronomia, agrimensura, arquitetura, geologia, urbanismo, paisagismo e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(52, 7, '7.02', 'Execução, por administração, empreitada ou subempreitada, de obras de construção civil, hidráulica ou elétrica e de outras obras semelhantes, inclusive sondagem, perfuração de poços, escavação, drenagem e irrigação, terraplanagem, pavimentação, concretagem e a instalação e montagem de produtos, peças e equipamentos (exceto o fornecimento de mercadorias produzidas pelo prestador de serviços fora do local da prestação dos serviços, que fica sujeito ao ICMS).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(53, 7, '7.03', 'Elaboração de planos diretores, estudos de viabilidade, estudos organizacionais e outros, relacionados com obras e serviços de engenharia; elaboração de anteprojetos, projetos básicos e projetos executivos para trabalhos de engenharia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(54, 7, '7.04', 'Demolição.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(55, 7, '7.05', 'Reparação, conservação e reforma de edifícios, estradas, pontes, portos e congêneres (exceto o fornecimento de mercadorias produzidas pelo prestador dos serviços, fora do local da prestação dos serviços, que fica sujeito ao ICMS).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(56, 7, '7.06', 'Colocação e instalação de tapetes, carpetes, assoalhos, cortinas, revestimentos de parede, vidros, divisórias, placas de gesso e congêneres, com material fornecido pelo tomador do serviço.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(57, 7, '7.07', 'Recuperação, raspagem, polimento e lustração de pisos e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(58, 7, '7.08', 'Calafetação.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(59, 7, '7.09', 'Varrição, coleta, remoção, incineração, tratamento, reciclagem, separação e destinação final de lixo, rejeitos e outros resíduos quaisquer.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(60, 7, '7.10', 'Limpeza, manutenção e conservação de vias e logradouros públicos, imóveis, chaminés, piscinas, parques, jardins e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(61, 7, '7.11', 'Decoração e jardinagem, inclusive corte e poda de árvores.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(62, 7, '7.12', 'Controle e tratamento de efluentes de qualquer natureza e de agentes físicos, químicos e biológicos.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(63, 7, '7.13', 'Dedetização, desinfecção, desinsetização, imunização, higienização, desratização, pulverização e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(64, 7, '7.16', 'Florestamento, reflorestamento, semeadura, adubação e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(65, 7, '7.17', 'Escoramento, contenção de encostas e serviços congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(66, 7, '7.18', 'Limpeza e dragagem de rios, portos, canais, baías, lagos, lagoas, represas, açudes e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(67, 7, '7.19', 'Acompanhamento e fiscalização da execução de obras de engenharia, arquitetura e urbanismo.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(68, 7, '7.20', 'Aerofotogrametria (inclusive interpretação), cartografia, mapeamento, levantamentos topográficos, batimétricos, geográficos, geodésicos, geológicos, geofísicos e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(69, 7, '7.21', 'Pesquisa, perfuração, cimentação, mergulho, perfilagem, concretação, testemunhagem, pescaria, estimulação e outros serviços relacionados com a exploração e explotação de petróleo, gás natural e de outros recursos minerais.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(70, 7, '7.22', 'Nucleação e bombardeamento de nuvens e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(71, 8, '8.01', 'Ensino regular pré-escolar, fundamental, médio e superior.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(72, 8, '8.02', 'Instrução, treinamento, orientação pedagógica e educacional, avaliação de conhecimentos de qualquer natureza.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(73, 9, '9.01', 'Hospedagem de qualquer natureza em hotéis, apart-service condominiais, flat, apart-hotéis, hotéis residência, residence-service, suite service, hotelaria marítima, motéis, pensões e congêneres; ocupação por temporada com fornecimento de serviço (o valor da alimentação e gorjeta, quando incluído no preço da diária, fica sujeito ao Imposto Sobre Serviços).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(74, 9, '9.02', 'Agenciamento, organização, promoção, intermediação e execução de programas de turismo, passeios, viagens, excursões, hospedagens e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(75, 9, '9.03', 'Guias de turismo.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(76, 10, '10.01', 'Agenciamento, corretagem ou intermediação de câmbio, de seguros, de cartões de crédito, de planos de saúde e de planos de previdência privada.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(77, 10, '10.02', 'Agenciamento, corretagem ou intermediação de títulos em geral, valores mobiliários e contratos quaisquer.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(78, 10, '10.03', 'Agenciamento, corretagem ou intermediação de direitos de propriedade industrial, artística ou literária.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(79, 10, '10.04', 'Agenciamento, corretagem ou intermediação de contratos de arrendamento mercantil (leasing), de franquia (franchising) e de faturização (factoring).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(80, 10, '10.05', 'Agenciamento, corretagem ou intermediação de bens móveis ou imóveis, não abrangidos em outros itens ou subitens, inclusive aqueles realizados no âmbito de Bolsas de Mercadorias e Futuros, por quaisquer meios.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(81, 10, '10.06', 'Agenciamento marítimo.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(82, 10, '10.07', 'Agenciamento de notícias.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(83, 10, '10.08', 'Agenciamento de publicidade e propaganda, inclusive o agenciamento de veiculação por quaisquer meios.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(84, 10, '10.09', 'Representação de qualquer natureza, inclusive comercial.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(85, 10, '10.10', 'Distribuição de bens de terceiros.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(86, 11, '11.01', 'Guarda e estacionamento de veículos terrestres automotores, de aeronaves e de embarcações.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(87, 11, '11.02', 'Vigilância, segurança ou monitoramento de bens e pessoas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(88, 11, '11.03', 'Escolta, inclusive de veículos e cargas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(89, 11, '11.04', 'Armazenamento, depósito, carga, descarga, arrumação e guarda de bens de qualquer espécie.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(90, 12, '12.01', 'Espetáculos teatrais.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(91, 12, '12.02', 'Exibições cinematográficas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(92, 12, '12.03', 'Espetáculos circenses.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(93, 12, '12.04', 'Programas de auditório.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(94, 12, '12.05', 'Parques de diversões, centros de lazer e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(95, 12, '12.06', 'Boates, taxi-dancing e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(96, 12, '12.07', 'Shows, ballet, danças, desfiles, bailes, óperas, concertos, recitais, festivais e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(97, 12, '12.08', 'Feiras, exposições, congressos e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(98, 12, '12.09', 'Bilhares, boliches e diversões eletrônicas ou não.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(99, 12, '12.10', 'Corridas e competições de animais.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(100, 12, '12.11', 'Competições esportivas ou de destreza física ou intelectual, com ou sem a participação do espectador.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(101, 12, '12.12', 'Execução de música.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(102, 12, '12.13', 'Produção, mediante ou sem encomenda prévia, de eventos, espetáculos, entrevistas, shows, ballet, danças, desfiles, bailes, teatros, óperas, concertos, recitais, festivais e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(103, 12, '12.14', 'Fornecimento de música para ambientes fechados ou não, mediante transmissão por qualquer processo.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(104, 12, '12.15', 'Desfiles de blocos carnavalescos ou folclóricos, trios elétricos e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(105, 12, '12.16', 'Exibição de filmes, entrevistas, musicais, espetáculos, shows, concertos, desfiles, óperas, competições esportivas, de destreza intelectual ou congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(106, 12, '12.17', 'Recreação e animação, inclusive em festas e eventos de qualquer natureza.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(107, 13, '13.02', 'Fonografia ou gravação de sons, inclusive trucagem, dublagem, mixagem e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(108, 13, '13.03', 'Fotografia e cinematografia, inclusive revelação, ampliação, cópia, reprodução, trucagem e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(109, 13, '13.04', 'Reprografia, microfilmagem e digitalização.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(110, 13, '13.05', 'Composição gráfica, fotocomposição, clicheria, zincografia, litografia, fotolitografia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(119, 14, '14.01', 'Lubrificação, limpeza, lustração, revisão, carga e recarga, conserto, restauração, blindagem, manutenção e conservação de máquinas, veículos, aparelhos, equipamentos, motores, elevadores ou de qualquer objeto (exceto peças e partes empregadas, que ficam sujeitas ao ICMS).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(120, 14, '14.02', 'Assistência técnica.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(121, 14, '14.03', 'Recondicionamento de motores (exceto peças e partes empregadas, que ficam sujeitas ao ICMS).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(122, 14, '14.04', 'Recauchutagem ou regeneração de pneus.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(123, 14, '14.05', 'Restauração, recondicionamento, acondicionamento, pintura, beneficiamento, lavagem, secagem, tingimento, galvanoplastia, anodização, corte, recorte, polimento, plastificação e congêneres, de objetos quaisquer.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(124, 14, '14.06', 'Instalação e montagem de aparelhos, máquinas e equipamentos, inclusive montagem industrial, prestados ao usuário final, exclusivamente com material por ele fornecido.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(125, 14, '14.07', 'Colocação de molduras e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(126, 14, '14.08', 'Encadernação, gravação e douração de livros, revistas e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(127, 14, '14.09', 'Alfaiataria e costura, quando o material for fornecido pelo usuário final, exceto aviamento.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(128, 14, '14.10', 'Tinturaria e lavanderia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(129, 14, '14.11', 'Tapeçaria e reforma de estofamentos em geral.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(130, 14, '14.12', 'Funilaria e lanternagem.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(131, 14, '14.13', 'Carpintaria e serralheria.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(132, 15, '15.01', 'Administração de fundos quaisquer, de consórcio, de cartão de crédito ou débito e congêneres, de carteira de clientes, de cheques pré-datados e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(133, 15, '15.02', 'Abertura de contas em geral, inclusive conta-corrente, conta de investimentos e aplicação e caderneta de poupança, no País e no exterior, bem como a manutenção das referidas contas ativas e inativas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(134, 15, '15.03', 'Locação e manutenção de cofres particulares, de terminais eletrônicos, de terminais de atendimento e de bens e equipamentos em geral.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(135, 15, '15.04', 'Fornecimento ou emissão de atestados em geral, inclusive atestado de idoneidade, atestado de capacidade financeira e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(136, 15, '15.05', 'Cadastro, elaboração de ficha cadastral, renovação cadastral e congêneres, inclusão ou exclusão no Cadastro de Emitentes de Cheques sem Fundos – CCF ou em quaisquer outros bancos cadastrais.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(137, 15, '15.06', 'Emissão, reemissão e fornecimento de avisos, comprovantes e documentos em geral; abono de firmas; coleta e entrega de documentos, bens e valores; comunicação com outra agência ou com a administração central; licenciamento eletrônico de veículos; transferência de veículos; agenciamento fiduciário ou depositário; devolução de bens em custódia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(138, 15, '15.07', 'Acesso, movimentação, atendimento e consulta a contas em geral, por qualquer meio ou processo, inclusive por telefone, fac-símile, internet e telex, acesso a terminais de atendimento, inclusive vinte e quatro horas; acesso a outro banco e a rede compartilhada; fornecimento de saldo, extrato e demais informações relativas a contas em geral, por qualquer meio ou processo.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(139, 15, '15.08', 'Emissão, reemissão, alteração, cessão, substituição, cancelamento e registro de contrato de crédito; estudo, análise e avaliação de operações de crédito; emissão, concessão, alteração ou contratação de aval, fiança, anuência e congêneres; serviços relativos a abertura de crédito, para quaisquer fins.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(140, 15, '15.09', 'Arrendamento mercantil (leasing) de quaisquer bens, inclusive cessão de direitos e obrigações, substituição de garantia, alteração, cancelamento e registro de contrato, e demais serviços relacionados ao arrendamento mercantil (leasing).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(141, 15, '15.10', 'Serviços relacionados a cobranças, recebimentos ou pagamentos em geral, de títulos quaisquer, de contas ou carnês, de câmbio, de tributos e por conta de terceiros, inclusive os efetuados por meio eletrônico, automático ou por máquinas de atendimento; fornecimento de posição de cobrança, recebimento ou pagamento; emissão de carnês, fichas de compensação, impressos e documentos em geral.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(142, 15, '15.11', 'Devolução de títulos, protesto de títulos, sustação de protesto, manutenção de títulos, reapresentação de títulos, e demais serviços a eles relacionados.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(143, 15, '15.12', 'Custódia em geral, inclusive de títulos e valores mobiliários.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(144, 15, '15.13', 'Serviços relacionados a operações de câmbio em geral, edição, alteração, prorrogação, cancelamento e baixa de contrato de câmbio; emissão de registro de exportação ou de crédito; cobrança ou depósito no exterior; emissão, fornecimento e cancelamento de cheques de viagem; fornecimento, transferência, cancelamento e demais serviços relativos a carta de crédito de importação, exportação e garantias recebidas; envio e recebimento de mensagens em geral relacionadas a operações de câmbio.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(145, 15, '15.14', 'Fornecimento, emissão, reemissão, renovação e manutenção de cartão magnético, cartão de crédito, cartão de débito, cartão salário e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(146, 15, '15.15', 'Compensação de cheques e títulos quaisquer; serviços relacionados a depósito, inclusive depósito identificado, a saque de contas quaisquer, por qualquer meio ou processo, inclusive em terminais eletrônicos e de atendimento.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(147, 15, '15.16', 'Emissão, reemissão, liquidação, alteração, cancelamento e baixa de ordens de pagamento, ordens de crédito e similares, por qualquer meio ou processo; serviços relacionados à transferência de valores, dados, fundos, pagamentos e similares, inclusive entre contas em geral.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(148, 15, '15.17', 'Emissão, fornecimento, devolução, sustação, cancelamento e oposição de cheques quaisquer, avulso ou por talão.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(149, 15, '15.18', 'Serviços relacionados a crédito imobiliário, avaliação e vistoria de imóvel ou obra, análise técnica e jurídica, emissão, reemissão, alteração, transferência e renegociação de contrato, emissão e reemissão do termo de quitação e demais serviços relacionados a crédito imobiliário.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(150, 16, '16.01', 'Serviços de transporte de natureza municipal.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(151, 17, '17.01', 'Assessoria ou consultoria de qualquer natureza, não contida em outros itens desta lista; análise, exame, pesquisa, coleta, compilação e fornecimento de dados e informações de qualquer natureza, inclusive cadastro e similares.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(152, 17, '17.02', 'Datilografia, digitação, estenografia, expediente, secretaria em geral, resposta audível, redação, edição, interpretação, revisão, tradução, apoio e infra-estrutura administrativa e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(153, 17, '17.03', 'Planejamento, coordenação, programação ou organização técnica, financeira ou administrativa.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(154, 17, '17.04', 'Recrutamento, agenciamento, seleção e colocação de mão-de-obra.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(155, 17, '17.05', 'Fornecimento de mão-de-obra, mesmo em caráter temporário, inclusive de empregados ou trabalhadores, avulsos ou temporários contratados pelo prestador de serviço.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(156, 17, '17.06', 'Propaganda e publicidade, inclusive promoção de vendas, planejamento de campanhas ou sistemas de publicidade, elaboração de desenhos, textos e demais materiais publicitários.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(157, 17, '17.08', 'Franquia (franchising).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(158, 17, '17.09', 'Perícias, laudos, exames técnicos e análises técnicas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(159, 17, '17.10', 'Planejamento, organização e administração de feiras, exposições, congressos e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(160, 17, '17.11', 'Organização de festas e recepções; bufê (exceto o fornecimento de alimentação e bebidas, que fica sujeito ao ICMS).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(161, 17, '17.12', 'Administração em geral, inclusive de bens e negócios de terceiros.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(162, 17, '17.13', 'Leilão e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(163, 17, '17.14', 'Advocacia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(164, 17, '17.15', 'Arbitragem de qualquer espécie, inclusive jurídica.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(165, 17, '17.16', 'Auditoria.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(166, 17, '17.17', 'Análise de Organização e Métodos.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(167, 17, '17.18', 'Atuária e cálculos técnicos de qualquer natureza.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(168, 17, '17.19', 'Contabilidade, inclusive serviços técnicos e auxiliares.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(169, 17, '17.20', 'Consultoria e assessoria econômica ou financeira.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(170, 17, '17.21', 'Estatística.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(171, 17, '17.22', 'Cobrança em geral.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(172, 17, '17.23', 'Assessoria, análise, avaliação, atendimento, consulta, cadastro, seleção, gerenciamento de informações, administração de contas a receber ou a pagar e em geral, relacionados a operações de faturização (factoring).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(173, 17, '17.24', 'Apresentação de palestras, conferências, seminários e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(174, 18, '18.01', 'Serviços de regulação de sinistros vinculados a contratos de seguros; inspeção e avaliação de riscos para cobertura de contratos de seguros; prevenção e gerência de riscos seguráveis e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(175, 19, '19.01', 'Serviços de distribuição e venda de bilhetes e demais produtos de loteria, bingos, cartões, pules ou cupons de apostas, sorteios, prêmios, inclusive os decorrentes de títulos de capitalização e congêneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(176, 20, '20.01', 'Serviços portuários, ferroportuários, utilizaçao de porto, movimentaçao de passageiros, reboque de embarcaçoes, rebocador escoteiro, atracaçao, desatracaçao, serviços de praticagem, capatazia, armazenagem de qualquer natureza, serviços acessórios, movimentaçao de mercadorias, serviços de apoio marítimo, de movimentaçao ao largo, serviços de armadores, estiva, conferencia, logística e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(177, 20, '20.02', 'Serviços aeroportuários, utilizaçao de aeroporto, movimentaçao de passageiros, armazenagem de qualquer natureza, capatazia, movimentaçao de aeronaves, serviços de apoio aeroportuários, serviços acessórios, movimentaçao de mercadorias, logística e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(178, 20, '20.03', 'Serviços de terminais rodoviários, ferroviários, metroviários, movimentaçao de passageiros, mercadorias, inclusive suas operaçoes, logística e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(179, 21, '21.01', 'Serviços de registros públicos, cartorários e notariais.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(180, 22, '22.01', 'Serviços de exploraçao de rodovia mediante cobrança de preço ou pedágio dos usuários, envolvendo execuçao de serviços de conservaçao, manutençao, melhoramentos para adequaçao de capacidade e segurança de trânsito, operaçao, monitoraçao, assistencia aos usuários e outros serviços definidos em contratos, atos de concessao ou de permissao ou em normas oficiais.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(181, 23, '23.01', 'Serviços de programaçao e comunicaçao visual, desenho industrial e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(182, 24, '24.01', 'Serviços de chaveiros, confecçao de carimbos, placas, sinalizaçao visual, banners, adesivos e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(183, 25, '25.01', 'Funerais, inclusive fornecimento de caixao, urna ou esquifes, aluguel de capela, transporte do corpo cadavérico, fornecimento de flores, coroas e outros paramentos, desembaraço de certidao de óbito, fornecimento de véu, essa e outros adornos, embalsamento, embelezamento, conservaçao ou restauraçao de cadáveres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(184, 25, '25.02', 'Cremaçao de corpos e partes de corpos cadavéricos.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(185, 25, '25.03', 'Planos ou convenio funerários.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(186, 25, '25.04', 'Manutençao e conservaçao de jazigos e cemitérios.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(187, 26, '26.01', 'Serviços de coleta, remessa ou entrega de correspondencias, documentos, objetos, bens ou valores, inclusive pelos correios e suas agencias franqueadas, courrier e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(188, 27, '27.01', 'Serviços de coleta, remessa ou entrega de correspondencias, documentos, objetos, bens ou valores, inclusive pelos correios e suas agencias franqueadas; courrier e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(189, 28, '28.01', 'Serviços de avaliaçao de bens e serviços de qualquer natureza.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(190, 29, '29.01', 'Serviços de biblioteconomia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(191, 30, '30.01', 'Serviços de biologia, biotecnologia e química.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(192, 31, '31.01', 'Serviços técnicos em edificaçoes, eletrônica, eletrotécnica, mecânica, telecomunicaçoes e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(193, 32, '32.01', 'Serviços de desenhos técnicos.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(194, 33, '33.01', 'Serviços de desembaraço aduaneiro, comissários, despachantes e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(195, 34, '34.01', 'Serviços de investigaçoes particulares, detetives e congeneres.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(196, 35, '35.01', 'Serviços de reportagem, assessoria de imprensa, jornalismo e relaçoes públicas.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(197, 36, '36.01', 'Serviços de meteorologia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(198, 37, '37.01', 'Serviços de artistas, atletas, modelos e manequins.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(199, 38, '38.01', 'Serviços de museologia.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(200, 39, '39.01', 'Serviços de ourivesaria e lapidaçao (quando o material for fornecido pelo tomador do serviço).', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A'),
(201, 40, '40.01', 'Obras de arte sob encomenda.', 'PJPF', 1.00, 0.00, 0.00, 0.00, 0.00, 'mensal', 0.00, NULL, NULL, 'A');

-- --------------------------------------------------------

--
-- Table structure for table `servicos_categorias`
--

CREATE TABLE IF NOT EXISTS `servicos_categorias` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nome` text,
  `tipo` varchar(10) DEFAULT NULL COMMENT 'tipos: CNAE e LC116',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=41 ;

--
-- Dumping data for table `servicos_categorias`
--

INSERT INTO `servicos_categorias` (`codigo`, `nome`, `tipo`) VALUES
(1, 'Perícias, laudos, exames técnicos e análises técnicas.', 'LC116'),
(2, 'Serviços de pesquisas e desenvolvimento de qualquer natureza.', 'LC116'),
(3, 'Serviços prestados mediante locação, cessão de direito de uso e congêneres.', 'LC116'),
(4, 'Serviços de saúde, assistência médica e congêneres.', 'LC116'),
(5, 'Serviços de medicina e assistência veterinária e congêneres.', 'LC116'),
(6, 'Serviços de cuidados pessoais, estética, atividades físicas e congêneres.', 'LC116'),
(7, 'Serviços relativos a engenharia, arquitetura, geologia, urbanismo, construção civil, manutenção, limpeza, meio ambiente, saneamento e congêneres.', 'LC116'),
(8, 'Serviços de educação, ensino, orientação pedagógica e educacional, instrução, treinamento e avaliação pessoal de qualquer grau ou natureza.', 'LC116'),
(9, 'Serviços relativos a hospedagem, turismo, viagens e congêneres.', 'LC116'),
(10, 'Serviços de intermediação e congêneres.', 'LC116'),
(11, 'Serviços de guarda, estacionamento, armazenamento, vigilância e congêneres.', 'LC116'),
(12, 'Serviços de diversões, lazer, entretenimento e congêneres.', 'LC116'),
(13, 'Serviços relativos a fonografia, fotografia, cinematografia e reprografia.', 'LC116'),
(14, 'Serviços relativos a bens de terceiros.', 'LC116'),
(15, 'Serviços relacionados ao setor bancário ou financeiro, inclusive aqueles prestados por instituições financeiras autorizadas a funcionar pela União ou por quem de direito.', 'LC116'),
(16, 'Serviços de transporte de natureza municipal.', 'LC116'),
(17, 'Instrumentação cirúrgica.', 'LC116'),
(18, 'Serviços de regulação de sinistros vinculados a contratos de seguros; inspeção e avaliação de riscos para cobertura de contratos de seguros; prevenção e gerência de riscos seguráveis e congêneres.', 'LC116'),
(19, 'Agenciamento de publicidade e propaganda, inclusive o agenciamento de veiculação por quaisquer meios.', 'LC116'),
(20, 'Serviços portuários, aeroportuários, ferroportuários, de terminais rodoviários, ferroviários e metroviários.', 'LC116'),
(21, 'Serviços de registros públicos, cartorários e notariais.', 'LC116'),
(22, 'Serviços de exploraçao de rodovia.', 'LC116'),
(23, 'Serviços de programaçao e comunicaçao visual, desenho industrial e congeneres.', 'LC116'),
(24, 'Serviços de chaveiros, confecçao de carimbos, placas, sinalizaçao visual, banners, adesivos e congeneres.', 'LC116'),
(25, 'Serviços funerários.', 'LC116'),
(26, 'Serviços de coleta, remessa ou entrega de correspondencias, documentos, objetos, bens ou valores, inclusive pelos correios e suas agencias franqueadas, courrier e congeneres.', 'LC116'),
(27, 'Serviços de assistência social.', 'LC116'),
(28, 'Serviços de avaliaçao de bens e serviços de qualquer natureza.', 'LC116'),
(29, 'Serviços de biblioteconomia.', 'LC116'),
(30, 'Serviços de biologia, biotecnologia e química.', 'LC116'),
(31, 'Serviços técnicos em edificaçoes, eletrônica, eletrotécnica, mecânica, telecomunicaçoes e congeneres.', 'LC116'),
(32, 'Serviços de desenhos técnicos.', 'LC116'),
(33, 'Serviços de desembaraço aduaneiro, comissários, despachantes e congeneres.', 'LC116'),
(34, 'Serviços de investigaçoes particulares, detetives e congeneres.', 'LC116'),
(35, 'Serviços de reportagem, assessoria de imprensa, jornalismo e relaçoes públicas.', 'LC116'),
(36, 'Serviços de meteorologia.', 'LC116'),
(37, 'Serviços de artistas, atletas, modelos e manequins.', 'LC116'),
(38, 'Serviços de museologia.', 'LC116'),
(39, 'Serviços de ourivesaria e lapidaçao.', 'LC116'),
(40, 'Serviços relativos a obras de arte sob encomenda.', 'LC116');

-- --------------------------------------------------------

--
-- Table structure for table `simples_des`
--

CREATE TABLE IF NOT EXISTS `simples_des` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codemissor` int(11) DEFAULT NULL,
  `competencia` date DEFAULT NULL,
  `data_gerado` date DEFAULT NULL,
  `total` decimal(10,2) DEFAULT NULL,
  `tomador` char(1) DEFAULT 'N',
  `codverificacao` char(9) DEFAULT NULL,
  `estado` char(1) DEFAULT 'N' COMMENT 'N normal C cancelada B boleto E escriturada',
  `motivo_cancelamento` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `simples_des_servicos`
--

CREATE TABLE IF NOT EXISTS `simples_des_servicos` (
  `codigo` int(10) NOT NULL AUTO_INCREMENT,
  `codsimples_des` int(10) DEFAULT NULL,
  `codservico` bigint(11) DEFAULT NULL,
  `basedecalculo` float(10,2) DEFAULT NULL,
  `tomador_cnpjcpf` varchar(20) DEFAULT NULL,
  `nota_nro` int(11) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `submenus_prefeitura`
--

CREATE TABLE IF NOT EXISTS `submenus_prefeitura` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `menu` varchar(100) DEFAULT NULL,
  `link` varchar(100) DEFAULT NULL,
  `nivel_1` char(1) DEFAULT 'M' COMMENT 'A para alto ,M para medio e B para baixo',
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=62 ;

--
-- Dumping data for table `submenus_prefeitura`
--

INSERT INTO `submenus_prefeitura` (`codigo`, `menu`, `link`, `nivel_1`) VALUES
(1, 'Cadastro', 'cadastro.php', 'B'),
(3, 'Processos', 'processos.php', 'M'),
(4, 'Fiscais', 'fiscais.php', 'M'),
(5, 'Categoria', 'categoria.php', 'B'),
(6, 'Liberações', 'liberar.php', 'M'),
(8, 'Prest/Serv', 'des.php', 'M'),
(10, 'Manuais', 'manuais.php', 'B'),
(11, 'Consulta nfe', 'consulta.php', 'M'),
(12, 'Prestadores', 'prestadores/cadastro.php', 'M'),
(13, 'ISS Retido', 'issretido.php', 'M'),
(14, 'Inst Financeira', 'dif.php', 'M'),
(15, 'Op de Credito', 'doc.php', 'M'),
(16, 'AIDF', 'aidf.php', 'B'),
(17, 'AIDFe', 'aidfe.php', 'M'),
(20, 'Configurações', 'configuracoes.php', 'M'),
(21, 'Logs', 'logs.php', 'M'),
(22, 'Legislação', 'legislacao.php', 'M'),
(23, 'Usuários', 'usuarios.php', 'M'),
(24, 'Notícias', 'noticias.php', 'M'),
(26, 'Ouvidoria', 'ouvidoria.php', 'M'),
(27, 'Cartorios', 'dec.php', 'M'),
(28, 'Orgaos Publicos', 'dop.php', 'M'),
(29, 'Chat', 'chat.php', 'M'),
(30, 'Fórum', 'forum.php', 'M'),
(31, 'Auditoria', 'auditoria.php', 'M'),
(36, 'Tomadores', 'tomadores/cadastro.php', 'B'),
(37, 'Auto de Infração', 'infracao.php', 'M'),
(38, 'Empreiteiras', 'decc.php', 'M'),
(39, 'Escriturações', 'escrituracoes.php', 'M'),
(40, 'Serviço', 'servico.php', 'M'),
(41, 'Nfe', 'nfe.php', 'M'),
(42, 'Sobre', 'sobre.php', 'B'),
(43, 'Serviços', 'servicos.php', 'M'),
(44, 'Regras de Crédito', 'regras_credito.php', 'M'),
(45, 'Simples', 'simples.php', 'M'),
(46, 'Prestador x Contador', 'prestadores/cadastro_contador.php', 'M'),
(47, 'Cartório - Serviços', 'cartorio_cadastro.php', 'M'),
(48, 'Cartório - Categorias', 'cartorio_categoria.php', 'M'),
(49, 'Inst. Financeira', 'if_cadastro.php', 'M'),
(50, 'Op. de Créditos', 'oc_cadastro.php', 'A'),
(51, 'Obras', 'obras.php', 'M'),
(52, 'Prestadores', 'prestadores.php', 'M'),
(53, 'Serviços', 'servicos.php', 'M'),
(54, 'MEI', 'mei.php', 'M'),
(55, 'Regras de Multa', 'regras/regra_multa.php', 'M'),
(56, 'Escrituração', 'escrituracoes_nfe.php', 'M'),
(57, 'Notas Escrituradas', 'notas_escrituradas.php', 'M'),
(58, 'Declarar NF-e', 'declarar_nfe.php', 'M'),
(59, 'Consultar', 'sep_consultar.php', 'M'),
(60, 'Gerar', 'sep_gerar.php', 'M'),
(61, 'Atualizar', 'sep_atualizar.php', 'M');

-- --------------------------------------------------------

--
-- Table structure for table `tipo`
--

CREATE TABLE IF NOT EXISTS `tipo` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `tipo` varchar(30) DEFAULT NULL,
  `nome` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=13 ;

--
-- Dumping data for table `tipo`
--

INSERT INTO `tipo` (`codigo`, `tipo`, `nome`) VALUES
(1, 'prestador', 'Prestador'),
(3, 'simples', 'Simples Nacional'),
(9, 'grafica', 'Gráfica'),
(10, 'contador', 'Contador'),
(11, 'tomador', 'Tomador');

-- --------------------------------------------------------

--
-- Table structure for table `tomadores_pagamento`
--

CREATE TABLE IF NOT EXISTS `tomadores_pagamento` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `codnota` varchar(11) DEFAULT NULL,
  `cpfcnpj` varchar(200) DEFAULT NULL,
  `estado` char(1) DEFAULT NULL,
  `nropagamento` bigint(20) DEFAULT NULL,
  `data` varchar(255) DEFAULT NULL,
  `dadosconfirmacao` text,
  PRIMARY KEY (`codigo`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=1 ;

-- --------------------------------------------------------

--
-- Table structure for table `usuarios`
--

CREATE TABLE IF NOT EXISTS `usuarios` (
  `codigo` int(11) NOT NULL AUTO_INCREMENT,
  `nome` varchar(100) DEFAULT NULL,
  `login` varchar(100) DEFAULT NULL,
  `senha` varchar(255) DEFAULT NULL,
  `tipo` varchar(50) DEFAULT NULL,
  `ultlogin` datetime DEFAULT NULL,
  `nivel` char(1) DEFAULT 'A',
  PRIMARY KEY (`codigo`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT AUTO_INCREMENT=2 ;

--
-- Dumping data for table `usuarios`
--

INSERT INTO `usuarios` (`codigo`, `nome`, `login`, `senha`, `tipo`, `ultlogin`, `nivel`) VALUES
(1, 'prefeitura', 'prefeitura', '202cb962ac59075b964b07152d234b70', 'prefeitura', '2010-10-01 10:38:21', 'A');

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
